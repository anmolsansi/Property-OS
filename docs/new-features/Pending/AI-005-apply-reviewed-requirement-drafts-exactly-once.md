# AI-005 — Apply reviewed requirement drafts exactly once
...
Apply reviewed requirement drafts exactly once. Review is an authorized write. Reading a draft earlier does not guarantee the reviewer still has access when Apply is clicked.

## 2. Current behavior and evidence

AIJOB-v1 owns draft state; existing client createRequirement writes directly and does not participate in an AI review transaction.
...
Review is an authorized write. Reading a draft earlier does not guarantee the reviewer still has access when Apply is clicked.
...
Chosen approach: Review is an authorized write. Reading a draft earlier does not guarantee the reviewer still has access when Apply is clicked. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                      | Baseline state                                            |
| --------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/review.service.ts`                | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/review.service.spec.ts` | Proposed — create here or consume dependency-created file |
| `Backend/test/ai-apply-race.e2e-spec.ts`                  | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [REQ-002](../Pending/REQ-002-expose-atomic-brief-read-and-replacement-endpoints.md)

**Blocks:** [AI-006](../Pending/AI-006-build-source-side-by-side-draft-review-ui.md)
...
**Where:** `Backend/src/modules/ai/review.service.ts`

**Change:** Add POST /ai/intakes/:id/apply using strict BRIEF-v1 parsing and reviewed draftVersion; recheck actor/client/source status.

**Why:** The model output is not a trusted create payload.

**Verify immediately:** Unknown keys/foreign IDs fail before writes.
...
**Change:** Lock job and create Requirement, locality links, AiReview and applied status through one transaction; use domain validation with tx, not nested standalone service writes.

**Why:** Partial review would leave duplicate or untracked business records.

**Verify immediately:** Injected DB failure rolls back every effect.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/ai/review.service.ts`, `Backend/src/modules/ai/ai.controller.ts`

**Change:** On replay compare canonical reviewed fields and draftVersion to stored AiReview; exact replay returns prior result, changed replay409.

**Why:** Double-click must not create two client requirements.

**Verify immediately:** No provider call during apply.
...
**Purpose:** Prove the boundary named “Double apply” for apply reviewed requirement drafts exactly once.
...
**Setup:** One needs_review job.

**Action:** Two concurrent identical apply calls.

**Expected result:** One Requirement and AiReview, same result ID.
...
**Why this test matters:** Review is an authorized write. Reading a draft earlier does not guarantee the reviewer still has access when Apply is clicked. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check unique review and tx lock.

### Test 2 — Transaction failure

**Purpose:** Prove the boundary named “Transaction failure” for apply reviewed requirement drafts exactly once.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Failure after requirement insert before review write.

**Action:** Apply.

**Expected result:** No requirement/locality/review persists; job still needs_review.
...
### Test 3 — Revoked reviewer

**Purpose:** Prove the boundary named “Revoked reviewer” for apply reviewed requirement drafts exactly once.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Reviewer loses client access after viewing draft.

**Action:** Apply.

**Expected result:** 404/403; no business write or provider call.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Review is an authorized write. Reading a draft earlier does not guarantee the reviewer still has access when Apply is clicked. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Reauthorize at commit boundary.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/review.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/ai-apply-race.e2e-spec.ts
```
...
- [ ] Unknown keys/foreign IDs fail before writes.
- [ ] Injected DB failure rolls back every effect.
