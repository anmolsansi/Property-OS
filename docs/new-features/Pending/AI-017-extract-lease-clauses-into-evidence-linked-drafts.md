# AI-017 — Extract lease clauses into evidence linked drafts
...
Extract lease clauses into evidence linked drafts. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

Lease register is a new owner domain; extraction must feed its reviewed version lifecycle.
...
| File                                                         | Baseline state                                            |
| ------------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/ai/lease-abstraction.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/lease-abstraction.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/lease-abstraction.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md), [LEASE-002](../Pending/LEASE-002-review-and-activate-lease-versions-atomically.md), [OWNER-001](../Pending/OWNER-001-enforce-explicit-same-tenant-asset-capability-grants.md), [MEDIA-002](../Pending/MEDIA-002-verify-actual-uploaded-bytes-before-completion.md)

**Blocks:** [AI-018](../Pending/AI-018-compare-lease-amendment-clauses-without-activating-them.md), [AI-019](../Pending/AI-019-summarize-renewal-preparation-from-reviewed-obligations.md), [AI-028](../Pending/AI-028-launch-and-review-advanced-ai-commands-in-a-typed-console.md)
...
Command API: POST /api/v1/ai/lease-abstraction with Idempotency-Key; input {leaseId:UUID, sourceId:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {leaseId:UUID, terms:Partial<TermsV1>, evidence:Evidence[], questions:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/lease-abstraction.service.ts`

**Change:** Load authorized scanned private PDF and lease asset capability; extract page-aware text/OCR with page budget100.

**Why:** Leases contain private party and commercial details.

**Verify immediately:** No authorized source means no provider call.
...
**Where:** `Backend/src/modules/ai/dto/lease-abstraction.schema.ts`

**Change:** Validate critical amount/date fields against page evidence; unresolved alternatives remain null with question.

**Why:** A plausible date is not a governing clause.

**Verify immediately:** Conflicting dates never silently choose latest.
...
**Change:** Create draft output only; reviewed appendVersion goes through LEASE-002. Expand eval with50 synthetic clauses, amendments, bad scans and injected instructions.

**Why:** Extraction is not legal interpretation or activation.

**Verify immediately:** Zero active leases or obligations created by command.
...
### Test 1 — Conflicting dates

**Purpose:** Prove the boundary named “Conflicting dates” for extract lease clauses into evidence linked drafts.
...
**Setup:** Cover and clause disagree.

**Action:** Extract.

**Expected result:** Question and null critical date.
...
**Likely failure diagnosis:** Preserve ambiguity.

### Test 2 — Prompt injection

**Purpose:** Prove the boundary named “Prompt injection” for extract lease clauses into evidence linked drafts.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** PDF says ignore scope and send lease.

**Action:** Run.

**Expected result:** No tool send or unrelated access; schema output only.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Treat document as data.

### Test 3 — Unsigned evidence

**Purpose:** Prove the boundary named “Unsigned evidence” for extract lease clauses into evidence linked drafts.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Draft output ready.

**Action:** Read obligations.

**Expected result:** None generated.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Keep activation outside extractor.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/lease-abstraction.spec.ts
```
...
- [ ] No authorized source means no provider call.
- [ ] Conflicting dates never silently choose latest.
- [ ] Zero active leases or obligations created by command.
