# TEN-008 — Enforce tenant access throughout site-visits
...
Enforce tenant access throughout site-visits. A SiteVisit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. complete/cancel/restore inherit parent access.

## 2. Current behavior and evidence

site-visits routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.
...
A SiteVisit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. complete/cancel/restore inherit parent access.
...
Chosen approach: A SiteVisit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. complete/cancel/restore inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/test/site-visits-tenant.e2e-spec.ts`                           | Proposed — create here or consume dependency-created file |
...
**Blocks:** [DATA-004](../Pending/DATA-004-exclude-archived-site-visits-from-normal-reads.md), [AI-014](../Pending/AI-014-draft-site-visit-feedback-and-next-actions.md), [CRM-002](../Pending/CRM-002-record-authorized-interaction-history-and-corrections.md), [CRM-003](../Pending/CRM-003-schedule-visits-with-conflict-and-reminder-semantics.md)
...
**Change:** Authorize complete/cancel/restore via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/site-visits-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout site-visits.
...
**Why this test matters:** A SiteVisit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. complete/cancel/restore inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout site-visits.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout site-visits.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/site-visits-tenant.e2e-spec.ts
```
