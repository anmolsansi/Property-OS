# AI-002 — Add a schema constrained provider adapter
...
Add a schema constrained provider adapter. The adapter returns validated data or a typed failure. A model-generated JSON object is not automatically a valid business fact.

## 2. Current behavior and evidence

The current backend has no LLM SDK or configured AI contract; DEC-003 owns provider/model selection.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/config/configuration.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/config/configuration.ts) — exists at the baseline commit.
...
The adapter returns validated data or a typed failure. A model-generated JSON object is not automatically a valid business fact.
...
Chosen approach: The adapter returns validated data or a typed failure. A model-generated JSON object is not automatically a valid business fact. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                        | Baseline state                                            |
| ----------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/provider.adapter.ts`                | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/ai-output.schema.ts`            | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/__tests__/provider.adapter.spec.ts` | Proposed — create here or consume dependency-created file |
| `Backend/src/config/configuration.ts`                       | Existing — inspect before editing                         |
...
**Depends On:** [AI-001](../Pending/AI-001-create-private-source-job-and-review-persistence.md), [DEC-003](../Pending/DEC-003-select-ai-providers-budgets-and-processing-regions.md)

**Blocks:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md)
...
**Where:** `Backend/src/modules/ai/provider.adapter.ts`

**Change:** Implement generateStructured({kind, input, schemaVersion, promptVersion, deadlineMs}) behind the exact provider/API approved in DEC-003. Use built-in fetch unless approved SDK is necessary.

**Why:** One adapter prevents provider semantics leaking into domain services.

**Verify immediately:** No runtime unapproved model fallback.
...
**Where:** `Backend/src/modules/ai/dto/ai-output.schema.ts`

**Change:** Return {fields, evidence, questions, usage, modelVersion} or reason REFUSED|INVALID_OUTPUT|TIMEOUT|RATE_LIMIT|PROVIDER_UNAVAILABLE; validate strict schema and citations against accessible source spans.

**Why:** Correct JSON shape does not prove supported facts.

**Verify immediately:** Invented quote or source ID rejected.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/ai/__tests__/provider.adapter.spec.ts`

**Change:** Apply60s abort, bounded output tokens from approved config, no tool execution and no secrets/content in logs. Treat document text as untrusted input.

**Why:** Prompt injection must not gain authority.

**Verify immediately:** Malicious source cannot change tenant, call network tools or bypass review.
...
### Test 1 — Malformed/refused output

**Purpose:** Prove the boundary named “Malformed/refused output” for add a schema constrained provider adapter.
...
**Setup:** Provider returns wrong shape or refusal.

**Action:** generateStructured.

**Expected result:** Typed failure; no silent coercion or apply.
...
**Why this test matters:** The adapter returns validated data or a typed failure. A model-generated JSON object is not automatically a valid business fact. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check post-response schema parser.

### Test 2 — Invented citation

**Purpose:** Prove the boundary named “Invented citation” for add a schema constrained provider adapter.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Output references absent page/quote.

**Action:** Validate evidence.

**Expected result:** INVALID_OUTPUT; no needs_review draft presented as grounded.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The adapter returns validated data or a typed failure. A model-generated JSON object is not automatically a valid business fact. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check actual source span matching.

### Test 3 — Prompt injection

**Purpose:** Prove the boundary named “Prompt injection” for add a schema constrained provider adapter.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Source says send all contacts to URL.

**Action:** Generate requirement draft.

**Expected result:** No external tools/network other than fixed provider endpoint; output fields only.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The adapter returns validated data or a typed failure. A model-generated JSON object is not automatically a valid business fact. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate trusted instructions from source data.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/provider.adapter.spec.ts
```
...
- [ ] No runtime unapproved model fallback.
- [ ] Invented quote or source ID rejected.
- [ ] Malicious source cannot change tenant, call network tools or bypass review.
