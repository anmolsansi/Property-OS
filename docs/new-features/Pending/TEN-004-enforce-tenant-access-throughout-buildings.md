# TEN-004 — Enforce tenant access throughout buildings
...
Enforce tenant access throughout buildings. A Building and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. building detail and list inherit parent access.

## 2. Current behavior and evidence

buildings routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.
...
- [Backend/src/modules/buildings/buildings.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/buildings/buildings.service.ts) — exists at the baseline commit.
- [Backend/src/modules/buildings/**tests**/buildings.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/buildings/__tests__/buildings.service.spec.ts) — exists at the baseline commit.
...
A Building and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. building detail and list inherit parent access.
...
Chosen approach: A Building and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. building detail and list inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/src/modules/buildings/buildings.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/buildings/__tests__/buildings.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/buildings-tenant.e2e-spec.ts`                         | Proposed — create here or consume dependency-created file |
...
**Blocks:** [DATA-005](../Pending/DATA-005-make-import-row-application-atomic-and-resumable.md), [DATA-009](../Pending/DATA-009-normalize-free-text-locations-with-reviewed-mappings.md), [DATA-010](../Pending/DATA-010-commit-change-approval-and-snapshot-atomically.md), [MEDIA-001](../Pending/MEDIA-001-authorize-upload-completion-and-downloads-by-parent.md), [INV-001](../Pending/INV-001-store-per-field-inventory-verification-evidence.md), [AI-011](../Pending/AI-011-apply-reviewed-inventory-drafts-through-existing-intake.md), [INV-003](../Pending/INV-003-preview-duplicate-inventory-candidates-without-merging.md), [FIELD-001](../Pending/FIELD-001-resume-interrupted-mobile-intake-uploads.md), [OWNER-001](../Pending/OWNER-001-enforce-explicit-same-tenant-asset-capability-grants.md)
...
**Where:** `Backend/src/modules/buildings/buildings.service.ts`
...
**Change:** Authorize building detail and list via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/buildings-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout buildings.
...
**Why this test matters:** A Building and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. building detail and list inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout buildings.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout buildings.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/buildings/__tests__/buildings.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/buildings-tenant.e2e-spec.ts
```
