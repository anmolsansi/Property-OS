# PRODUCT-001 — Guide onboarding to the first useful shortlist
...
Guide onboarding to the first useful shortlist. Onboarding is complete when persisted work is usable, not when a user dismisses a checklist.

## 2. Current behavior and evidence

Import, property intake and requirements exist but do not provide a durable outcome-based onboarding checklist.
...
Onboarding is complete when persisted work is usable, not when a user dismisses a checklist.
...
Chosen approach: Onboarding is complete when persisted work is usable, not when a user dismisses a checklist. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                            | Baseline state                                            |
| --------------------------------------------------------------- | --------------------------------------------------------- |
| `Frontend/src/hooks/use-onboarding.ts`                          | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(v2)/onboarding/page.tsx`                     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/organizations/onboarding.service.ts`       | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/organizations/organizations.controller.ts` | Proposed — create here or consume dependency-created file |
| `Frontend/e2e/onboarding.spec.ts`                               | Proposed — create here or consume dependency-created file |
...
**Depends On:** [DATA-005](../Pending/DATA-005-make-import-row-application-atomic-and-resumable.md), [REQ-003](../Pending/REQ-003-build-a-requirement-editor-that-preserves-unknown-values.md), [MATCH-002](../Pending/MATCH-002-expose-match-runs-and-comparison-ui.md), [DEC-001](../Pending/DEC-001-approve-the-first-pilot-and-measurement-baseline.md), [METRIC-001](../Pending/METRIC-001-measure-the-intake-to-visit-funnel-without-private-payloads.md), [SAAS-001](../Pending/SAAS-001-invite-and-offboard-organization-members-explicitly.md)
...
Register onboarding service in organizations.module.ts. Paths must follow the actual target branch routes; use existing route links recorded during READ checkpoint.
...
**Where:** `Frontend/src/hooks/use-onboarding.ts`

**Change:** GET /organizations/current/onboarding derives steps from permitted counts: membership ready, at least one valid property, import errors reconciled or explicitly skipped, confirmed requirement, manual shortlist. Return statuses with links.

**Why:** Local checkboxes cannot represent shared business setup.

**Verify immediately:** Reload and another authorized device show same derived progress.
...
**Where:** `Frontend/src/app/(v2)/onboarding/page.tsx`

**Change:** Build page linking existing import/intake/brief/matching routes with sample template and error reconciliation summary. Allow skip import with reason but never mark data validation passed.

**Why:** Teams may start manually; they still need truthful completion state.

**Verify immediately:** No synthetic real inventory inserted by onboarding.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/organizations/onboarding.service.ts`

**Change:** Track first meaningful shortlist timestamp once via METRIC-001, show manual fallback if AI disabled.

**Why:** Activation should work before paid AI is enabled.

**Verify immediately:** No AI job automatically runs on mount.
...
**Purpose:** Prove the boundary named “Manual start” for guide onboarding to the first useful shortlist.
...
**Setup:** User creates property/brief without CSV.

**Action:** Open onboarding.

**Expected result:** Manual path valid; import explicitly skipped.
...
**Why this test matters:** Onboarding is complete when persisted work is usable, not when a user dismisses a checklist. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not require unnecessary import.

### Test 2 — Partial import

**Purpose:** Prove the boundary named “Partial import” for guide onboarding to the first useful shortlist.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Rows failed.

**Action:** Refresh checklist.

**Expected result:** Needs reconciliation, not complete.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Onboarding is complete when persisted work is usable, not when a user dismisses a checklist. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use row outcomes.

### Test 3 — Cross-device

**Purpose:** Prove the boundary named “Cross-device” for guide onboarding to the first useful shortlist.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Checklist opened elsewhere.

**Action:** Read.

**Expected result:** Same server-derived progress.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Onboarding is complete when persisted work is usable, not when a user dismisses a checklist. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** No browser-only completion flags.
...
npm --prefix Frontend test -- e2e/onboarding.spec.ts --project=chromium
```
...
- [ ] Reload and another authorized device show same derived progress.
- [ ] No synthetic real inventory inserted by onboarding.
- [ ] No AI job automatically runs on mount.
