# CRM-003 — Schedule visits with conflict and reminder semantics
...
Schedule visits with conflict and reminder semantics. A visit is a versioned time interval. Rescheduling creates a new schedule identity so old reminders can be discarded.

## 2. Current behavior and evidence

SiteVisitsService supports visits but does not establish reliable participant conflicts or reminder version cancellation.
...
- [Backend/src/modules/site-visits/site-visits.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/site-visits/site-visits.service.ts) — exists at the baseline commit.
- [Backend/src/modules/site-visits/site-visits.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/site-visits/site-visits.controller.ts) — exists at the baseline commit.
- [Backend/src/modules/site-visits/dto/site-visits.schema.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/site-visits/dto/site-visits.schema.ts) — exists at the baseline commit.
...
A visit is a versioned time interval. Rescheduling creates a new schedule identity so old reminders can be discarded.
...
Chosen approach: A visit is a versioned time interval. Rescheduling creates a new schedule identity so old reminders can be discarded. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915001600_visit_schedules/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/site-visits/site-visits.service.ts`                 | Existing — inspect before editing                         |
| `Backend/src/modules/site-visits/site-visits.controller.ts`              | Existing — inspect before editing                         |
| `Backend/src/modules/site-visits/dto/site-visits.schema.ts`              | Existing — inspect before editing                         |
| `Backend/test/visit-scheduling.e2e-spec.ts`                              | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-008](../Pending/TEN-008-enforce-tenant-access-throughout-site-visits.md), [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md)

**Blocks:** [CRM-006](../Pending/CRM-006-show-timelines-visit-calendar-and-saved-searches.md)
...
**Change:** Add startsAt/endsAt UTC instants, displayTimeZone IANA default Asia/Kolkata, scheduleVersion Int default1, participantIds join table. Keep legacy date fields readable. Duration15..480min; max20 participants.

**Why:** Time zones describe display; stored intervals compare consistently.

**Verify immediately:** Invalid IANA zone or end<=start400.
...
**Where:** `Backend/src/modules/site-visits/site-visits.service.ts`, `Backend/src/modules/site-visits/site-visits.controller.ts`

**Change:** PUT /site-visits/:id/schedule {expectedVersion, startsAt, endsAt, timeZone, participantIds} locks participant IDs in lexical order and checks active overlapping intervals; return409 conflict without private visit details.

**Why:** Two concurrent schedules must not both pass a preflight availability query.

**Verify immediately:** Adjacent intervals allowed; overlapping schedules cannot commit.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/site-visits/site-visits.service.ts`

**Change:** Reschedule/cancel atomically increments version and emits outbox event; worker rechecks active scheduleVersion before reminder delivery.

**Why:** A queued reminder can outlive its schedule.

**Verify immediately:** Cancelled or superseded reminder sends nothing and records suppression.
...
**Purpose:** Prove the boundary named “Concurrent overlap” for schedule visits with conflict and reminder semantics.
...
**Setup:** Two visits same participant/time.

**Action:** Schedule concurrently.

**Expected result:** One success one409.
...
**Why this test matters:** A visit is a versioned time interval. Rescheduling creates a new schedule identity so old reminders can be discarded. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use transactional locks on participant schedule boundary.

### Test 2 — Timezone boundary

**Purpose:** Prove the boundary named “Timezone boundary” for schedule visits with conflict and reminder semantics.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** 23:30IST visit crossing UTC date.

**Action:** Schedule/read.

**Expected result:** Same intended local instant and correct conflict result.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A visit is a versioned time interval. Rescheduling creates a new schedule identity so old reminders can be discarded. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Compare instants rather than formatted dates.

### Test 3 — Old reminder

**Purpose:** Prove the boundary named “Old reminder” for schedule visits with conflict and reminder semantics.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** v1 reminder pending; reschedule to v2.

**Action:** Execute old job.

**Expected result:** Suppressed, no send; v2 reminder remains.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A visit is a versioned time interval. Rescheduling creates a new schedule identity so old reminders can be discarded. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Recheck version at delivery.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/visit-scheduling.e2e-spec.ts
...
- [ ] Invalid IANA zone or end<=start400.
- [ ] Adjacent intervals allowed; overlapping schedules cannot commit.
- [ ] Cancelled or superseded reminder sends nothing and records suppression.
