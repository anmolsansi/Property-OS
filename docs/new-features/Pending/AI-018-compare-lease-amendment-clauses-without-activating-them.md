# AI-018 — Compare lease amendment clauses without activating them
...
Compare lease amendment clauses without activating them. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

LEASE-001 preserves versions, but there is no cited comparative review of governing clause changes.
...
| File                                                        | Baseline state                                            |
| ----------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/lease-comparison.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/lease-comparison.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/lease-comparison.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-017](../Pending/AI-017-extract-lease-clauses-into-evidence-linked-drafts.md), [LEASE-002](../Pending/LEASE-002-review-and-activate-lease-versions-atomically.md)
...
Command API: POST /api/v1/ai/lease-comparison with Idempotency-Key; input {leaseId:UUID, baseVersionId:UUID, candidateVersionId:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {changes:[{field:string, before:unknown, after:unknown, baseEvidence:Evidence[], candidateEvidence:Evidence[], status:changed|unchanged|ambiguous}], questions:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/lease-comparison.service.ts`

**Change:** Require same lease and view_lease/review_lease capabilities for both versions; compare normalized structured terms with deterministic diff first.

**Why:** A document from another lease must not enter the comparison.

**Verify immediately:** Version mismatch400; forbidden source404.
...
**Where:** `Backend/src/modules/ai/dto/lease-comparison.schema.ts`

**Change:** Ask AI only for clause summaries tied to both source versions, preserving exact amounts/dates as structured fields.

**Why:** Generated prose must not supersede deterministic values.

**Verify immediately:** Unsupported diff statements rejected.
...
**Change:** Store draft comparison and show ambiguous supersession explicitly; LEASE-002 remains sole activation boundary.

**Why:** Which clause governs requires explicit reviewer judgment.

**Verify immediately:** No obligation cancellation or activeVersion mutation.
...
**Purpose:** Prove the boundary named “Other lease” for compare lease amendment clauses without activating them.
...
**Setup:** Candidate belongs another lease.

**Action:** Compare.

**Expected result:** 400 and no provider call.
...
**Likely failure diagnosis:** Validate relationship.

### Test 2 — Unchanged number

**Purpose:** Prove the boundary named “Unchanged number” for compare lease amendment clauses without activating them.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model claims rent increase on equal decimals.

**Action:** Validate.

**Expected result:** Rejected claim; deterministic unchanged retained.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Compare decimal values before narrative.

### Test 3 — Ambiguous precedence

**Purpose:** Prove the boundary named “Ambiguous precedence” for compare lease amendment clauses without activating them.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Both versions contain conflicting effective clauses.

**Action:** Generate.

**Expected result:** Questions; no governing-version choice.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not infer legal priority.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/lease-comparison.spec.ts
```
...
- [ ] Version mismatch400; forbidden source404.
- [ ] Unsupported diff statements rejected.
- [ ] No obligation cancellation or activeVersion mutation.
