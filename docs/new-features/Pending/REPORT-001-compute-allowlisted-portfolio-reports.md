# REPORT-001 — Compute allowlisted portfolio reports
...
**Priority:** P1  
**Area:** Reporting  
**Assessment:** A15, A17, A20, P16, P30  
...
Compute allowlisted portfolio reports. The model may choose a report definition. Code computes every total from permissioned records.

## 2. Current behavior and evidence

Dashboard and rent-roll queries exist in distinct domains; a report interface must preserve their scope and calculation contracts.
...
The model may choose a report definition. Code computes every total from permissioned records.
...
Chosen approach: The model may choose a report definition. Code computes every total from permissioned records. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                  | Baseline state                                            |
| --------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/analytics/report.service.ts`                     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/analytics/report.controller.ts`                  | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/analytics/analytics.module.ts`                   | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/analytics/report.schema.ts`                      | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/analytics/__tests__/reports.integration.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [FIN-002](../Pending/FIN-002-export-reconciled-rent-rolls-with-source-identities.md), [OWNER-003](../Pending/OWNER-003-display-lease-register-obligations-and-occupancy.md), [METRIC-001](../Pending/METRIC-001-measure-the-intake-to-visit-funnel-without-private-payloads.md)

**Blocks:** [AI-022](../Pending/AI-022-translate-report-questions-into-approved-definitions.md), [AI-024](../Pending/AI-024-validate-permissioned-comparable-data-before-summaries.md), [AI-027](../Pending/AI-027-explain-deterministic-portfolio-anomalies-with-source-evidence.md), [AI-028](../Pending/AI-028-launch-and-review-advanced-ai-commands-in-a-typed-console.md)
...
### REPORT-v1 — computed metrics, bounded explanations

ReportRequest={definitionId:'pipeline_v1'|'occupancy_v1'|'rent_roll_v1', asOf:'YYYY-MM-DD', fromDate?:date, toDate?:date, buildingIds?:UUID[]} with max100 granted IDs and max366-day period. Code executes prewritten queries scoped by AccessContext and asset grants. Output includes definitionVersion, filters, asOf, generatedAt, rows, totals, missingCounts, sourceIds. Decimal strings for money. No arbitrary SQL, Prisma objects, expressions or user-provided formulas. Unknown definition400; unauthorized asset404; missing owner capability403. A narrative may reference returned rows; it cannot modify totals or infer causation.
...
Add ReportSnapshot(id, organizationId, actorId, definitionId, definitionVersion, filtersJson, resultJson, sourceIdsJson, createdAt) via Backend/prisma/schema.prisma and migration20260915002800_report_snapshots; these are explicitly allowed extra files.
...
**Where:** `Backend/src/modules/analytics/report.service.ts`

**Change:** Implement POST /reports/run with REPORT-v1; map definitions to pipeline events, owner dashboard and rent-roll services.

**Why:** Reuse verified calculations rather than duplicate SQL.

**Verify immediately:** Unknown definition/field400, no query execution.
...
**Where:** `Backend/src/modules/analytics/report.controller.ts`

**Change:** Persist report snapshot ID/hash and exact query definition/filter versions with sourceIds; protect snapshots under same live access.

**Why:** An explanation needs reproducible input.

**Verify immediately:** Cross-tenant or revoked grant returns no report.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/analytics/report.service.ts`, `Backend/src/modules/analytics/report.controller.ts`

**Change:** Return missing counts and data timestamp; refuse over-cap output rather than silently truncate financial totals.

**Why:** A number without coverage can mislead owners.

**Verify immediately:** Totals reconcile to included rows exactly.
...
### Test 1 — SQL injection intent

**Purpose:** Prove the boundary named “SQL injection intent” for compute allowlisted portfolio reports.
...
**Setup:** definitionId contains SQL.

**Action:** Run.

**Expected result:** 400, no arbitrary query.
...
**Why this test matters:** The model may choose a report definition. Code computes every total from permissioned records. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Allowlist exact definitions.

### Test 2 — Totals

**Purpose:** Prove the boundary named “Totals” for compute allowlisted portfolio reports.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Known decimal rent-roll.

**Action:** Report.

**Expected result:** Same total as FIN-002.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The model may choose a report definition. Code computes every total from permissioned records. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Reuse domain calculator.

### Test 3 — Revoked snapshot

**Purpose:** Prove the boundary named “Revoked snapshot” for compute allowlisted portfolio reports.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Grant lost after report saved.

**Action:** Read.

**Expected result:** Denied, no cached totals.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The model may choose a report definition. Code computes every total from permissioned records. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Reauthorize source set.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/analytics/__tests__/reports.integration.spec.ts
```
...
- [ ] Unknown definition/field400, no query execution.
- [ ] Cross-tenant or revoked grant returns no report.
- [ ] Totals reconcile to included rows exactly.
