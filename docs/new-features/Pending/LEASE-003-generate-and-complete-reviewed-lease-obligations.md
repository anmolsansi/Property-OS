# LEASE-003 — Generate and complete reviewed lease obligations
...
Generate and complete reviewed lease obligations. Code computes deadlines from reviewed terms. AI may summarize the source but cannot select the governing date.

## 2. Current behavior and evidence

Reviewed active versions exist after LEASE-002; ordinary task due dates do not prove lease clause deadlines.
...
Code computes deadlines from reviewed terms. AI may summarize the source but cannot select the governing date.
...
Chosen approach: Code computes deadlines from reviewed terms. AI may summarize the source but cannot select the governing date. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                   | Baseline state                                            |
| ---------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/leases/obligation.service.ts`                     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/obligation.worker.ts`                      | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/leases/__tests__/obligations.integration.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [LEASE-002](../Pending/LEASE-002-review-and-activate-lease-versions-atomically.md), [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md)

**Blocks:** [OWNER-003](../Pending/OWNER-003-display-lease-register-obligations-and-occupancy.md), [AI-019](../Pending/AI-019-summarize-renewal-preparation-from-reviewed-obligations.md), [WORKFLOW-001](../Pending/WORKFLOW-001-specify-versioned-broker-and-owner-workflow-templates.md)
...
**Where:** `Backend/src/modules/leases/obligation.service.ts`

**Change:** Consume activated version event; derive notice=endDate minus noticeDays, renewal=endDate, escalation only from explicit paired rate/period. Persist unique version/type/dueDate obligations with approved assignee.

**Why:** Replayed events must not create duplicate calendar duties.

**Verify immediately:** Null escalation pair creates no escalation event.
...
**Where:** `Backend/src/modules/leases/obligation.worker.ts`

**Change:** On amendment cancel only scheduled obligations of superseded version, preserve completed ones; worker checks current active version before each delivery.

**Why:** An old queued reminder must not survive a changed clause.

**Verify immediately:** No old-version message after cancellation commits.
...
**Change:** GET /leases/:id/obligations and POST /lease-obligations/:id/complete {expectedStatus:scheduled, evidenceSourceId, note<=1000}; authorized assignee/reviewer only. Completion records actor/time/evidence and is idempotent.

**Why:** Ticking a box is not proof that a contractual obligation was fulfilled.

**Verify immediately:** Missing evidence rejected; system never auto-completes on email send.
...
**Purpose:** Prove the boundary named “Leap date” for generate and complete reviewed lease obligations.
...
**Setup:** Lease ends2028-03-01 noticeDays1.

**Action:** Generate.

**Expected result:** Notice due2028-02-29 in lease timezone.
...
**Why this test matters:** Code computes deadlines from reviewed terms. AI may summarize the source but cannot select the governing date. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use date arithmetic, not milliseconds per day.

### Test 2 — Amendment

**Purpose:** Prove the boundary named “Amendment” for generate and complete reviewed lease obligations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Old notice completed; old escalation pending.

**Action:** Activate replacement.

**Expected result:** Completed history retained; pending cancelled; new schedule unique.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Code computes deadlines from reviewed terms. AI may summarize the source but cannot select the governing date. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Filter cancellation by status.

### Test 3 — Retry

**Purpose:** Prove the boundary named “Retry” for generate and complete reviewed lease obligations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Activation event delivered3 times.

**Action:** Generate.

**Expected result:** One obligation per unique tuple.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Code computes deadlines from reviewed terms. AI may summarize the source but cannot select the governing date. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** DB uniqueness plus idempotent consumer.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/leases/__tests__/obligations.integration.spec.ts
```
...
- [ ] Null escalation pair creates no escalation event.
- [ ] No old-version message after cancellation commits.
- [ ] Missing evidence rejected; system never auto-completes on email send.
