# LEASE-002 — Review and activate lease versions atomically

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Lease lifecycle  
**Assessment:** P17, P18, A11, A12  
...
Review and activate lease versions atomically. Review checks evidence. Activation is a separate authorized act that makes a version govern obligations.

## 2. Current behavior and evidence

LEASE-001 stores drafts and history; no generated term is active yet.
...
Review checks evidence. Activation is a separate authorized act that makes a version govern obligations.
...
Chosen approach: Review checks evidence. Activation is a separate authorized act that makes a version govern obligations. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                    | Baseline state                                            |
| ----------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/leases/lease-review.service.ts`                    | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/leases.controller.ts`                       | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/dto/leases.schema.ts`                       | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/leases.module.ts`                           | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/__tests__/lease-review.integration.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [LEASE-001](../Pending/LEASE-001-create-the-lease-register-and-immutable-term-versions.md), [OWNER-001](../Pending/OWNER-001-enforce-explicit-same-tenant-asset-capability-grants.md), [OPS-002](../Pending/OPS-002-persist-transactional-outbox-events.md)

**Blocks:** [LEASE-003](../Pending/LEASE-003-generate-and-complete-reviewed-lease-obligations.md), [AI-017](../Pending/AI-017-extract-lease-clauses-into-evidence-linked-drafts.md), [AI-018](../Pending/AI-018-compare-lease-amendment-clauses-without-activating-them.md), [FIN-001](../Pending/FIN-001-version-leasing-offers-and-enforce-approval-limits.md), [FIN-002](../Pending/FIN-002-export-reconciled-rent-rolls-with-source-identities.md), [AI-028](../Pending/AI-028-launch-and-review-advanced-ai-commands-in-a-typed-console.md)
...
**Where:** `Backend/src/modules/leases/lease-review.service.ts`

**Change:** POST /leases/:id/versions/:versionId/review {expectedVersion, terms, fieldEvidence} requires review_lease capability; critical dates/amounts require source citations and explicit confirmation.

**Why:** A model confidence value is not approval.

**Verify immediately:** Missing or unsupported critical fields400; no reviewedAt.
...
**Change:** POST /leases/:id/versions/:versionId/activate {expectedVersion, expectedCurrentVersionId} requires ADMIN plus activate_lease; apply DEC-007 self-approval rule. Lock lease, validate chronology and supersession, activate exactly one effective timeline.

**Why:** Concurrent amendments cannot both silently replace current terms.

**Verify immediately:** Stale expectation409 and no obligations changed.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/leases/dto/leases.schema.ts`

**Change:** Emit version-activated outbox event in transaction; retain superseded reviewed terms and expose complete version history.

**Why:** Downstream reminders must reconcile from durable committed version.

**Verify immediately:** Outbox failure rolls back activation; retry uses same event key.
...
### Test 1 — Unreviewed version

**Purpose:** Prove the boundary named “Unreviewed version” for review and activate lease versions atomically.
...
**Setup:** Extracted draft lacks confirmed dates.

**Action:** Activate.

**Expected result:** 409 NOT_REVIEWED; no event.
...
**Why this test matters:** Review checks evidence. Activation is a separate authorized act that makes a version govern obligations. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate needs_review and active.

### Test 2 — Two amendments

**Purpose:** Prove the boundary named “Two amendments” for review and activate lease versions atomically.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Two activations share currentVersion.

**Action:** Run concurrently.

**Expected result:** One succeeds, other409.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Review checks evidence. Activation is a separate authorized act that makes a version govern obligations. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Lock parent lease and compare current version.

### Test 3 — Revoked approver

**Purpose:** Prove the boundary named “Revoked approver” for review and activate lease versions atomically.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Grant revoked after review.

**Action:** Activate.

**Expected result:** Denied; reviewed history remains draft.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Review checks evidence. Activation is a separate authorized act that makes a version govern obligations. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Reauthorize at activation.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/leases/__tests__/lease-review.integration.spec.ts
```
...
- [ ] Missing or unsupported critical fields400; no reviewedAt.
- [ ] Stale expectation409 and no obligations changed.
- [ ] Outbox failure rolls back activation; retry uses same event key.
