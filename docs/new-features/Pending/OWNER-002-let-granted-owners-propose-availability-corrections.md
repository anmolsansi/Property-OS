# OWNER-002 — Let granted owners propose availability corrections
...
Let granted owners propose availability corrections. An owner may report a change on a granted asset. Verification remains a distinct staff review step.

## 2. Current behavior and evidence

Existing worker intake/change approval is the write path; owner portal must not bypass it.
...
An owner may report a change on a granted asset. Verification remains a distinct staff review step.
...
Chosen approach: An owner may report a change on a granted asset. Verification remains a distinct staff review step. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                | Baseline state                                            |
| ------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/asset-grants/owner-availability.controller.ts` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/asset-grants/asset-grants.module.ts`           | Proposed — create here or consume dependency-created file |
| `Frontend/src/hooks/use-owner-assets.ts`                            | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(v2)/owner/assets/page.tsx`                       | Proposed — create here or consume dependency-created file |
| `Frontend/e2e/owner-availability.spec.ts`                           | Proposed — create here or consume dependency-created file |
...
**Depends On:** [OWNER-001](../Pending/OWNER-001-enforce-explicit-same-tenant-asset-capability-grants.md), [INV-002](../Pending/INV-002-expose-the-verification-queue-and-review-actions.md), [DATA-010](../Pending/DATA-010-commit-change-approval-and-snapshot-atomically.md)
...
**Where:** `Backend/src/modules/asset-grants/owner-availability.controller.ts`

**Change:** GET /owner/assets returns only granted view_availability assets with public availability facts; POST /owner/assets/:id/availability {expectedVersion, value, sourceId?, note} requires propose_availability.

**Why:** A portal is an action-specific projection, not full building serialization.

**Verify immediately:** No client, commission, private lease or contact fields.
...
**Change:** Persist proposal using existing ChangeRequest with source attribution; require DATA-010 atomic approval to append new VerificationFact.

**Why:** Self-report is useful evidence but not automatically verified inventory.

**Verify immediately:** Owner submit leaves canonical availability and verifiedAt unchanged.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/hooks/use-owner-assets.ts`

**Change:** Render granted asset list, proposed/pending/approved/rejected history and explicit recorded confirmation. Clear data on revocation.

**Why:** Submitted must not be labeled verified.

**Verify immediately:** Reload preserves pending request and denied assets disappear.
...
**Purpose:** Prove the boundary named “Own report” for let granted owners propose availability corrections.
...
**Setup:** Granted owner submits availability.

**Action:** Reload asset.

**Expected result:** Pending correction; old verified fact unchanged.
...
**Why this test matters:** An owner may report a change on a granted asset. Verification remains a distinct staff review step. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not invoke admin appendFact.

### Test 2 — Unassigned asset

**Purpose:** Prove the boundary named “Unassigned asset” for let granted owners propose availability corrections.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Owner guesses other buildingID.

**Action:** POST.

**Expected result:** 404; zero change requests.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An owner may report a change on a granted asset. Verification remains a distinct staff review step. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Enforce grant before payload access.

### Test 3 — Revocation

**Purpose:** Prove the boundary named “Revocation” for let granted owners propose availability corrections.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Portal open then grant revoked.

**Action:** Refresh/submit.

**Expected result:** Denied and cached asset cleared.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An owner may report a change on a granted asset. Verification remains a distinct staff review step. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Apply policy to every endpoint.
...
npm --prefix Frontend test -- e2e/owner-availability.spec.ts --project=chromium
```
...
- [ ] No client, commission, private lease or contact fields.
- [ ] Owner submit leaves canonical availability and verifiedAt unchanged.
- [ ] Reload preserves pending request and denied assets disappear.
