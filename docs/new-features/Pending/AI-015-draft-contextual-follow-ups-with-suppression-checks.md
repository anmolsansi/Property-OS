# AI-015 — Draft contextual follow ups with suppression checks
...
Draft contextual follow ups with suppression checks. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

Tasks/follow-ups and email providers exist; approved interaction history and suppression rules are not a generated-message contract.
...
| File                                                | Baseline state                                            |
| --------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/followup.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/followup.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/followup.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [CRM-002](../Pending/CRM-002-record-authorized-interaction-history-and-corrections.md), [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md)
...
Command API: POST /api/v1/ai/followup with Idempotency-Key; input {dealId:UUID, channel:email|sms}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {subject:string|null, body:string<=2000, basisEventIds:UUID[], suppressionReason:string|null}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/followup.service.ts`

**Change:** Load permitted interaction timeline and deal state; suppress closed/lost/deleted deals or opted-out contacts.

**Why:** A queued draft can already be obsolete.

**Verify immediately:** Suppressed result contains no send action.
...
**Where:** `Backend/src/modules/ai/dto/followup.schema.ts`

**Change:** Generate one draft grounded in last approved interaction with no invented promises/prices; do not invoke notification send method.

**Why:** Draft permission is not messaging authority.

**Verify immediately:** Provider text cannot schedule another outreach.
...
**Change:** Display last-contact time, reason and mandatory human review; future sending must recheck suppression through OPS-004 and a separate authorized send action.

**Why:** A client reply between draft and send should stop stale follow-up.

**Verify immediately:** This ticket ends at a draft; no autonomous campaign.
...
### Test 1 — Opted-out contact

**Purpose:** Prove the boundary named “Opted-out contact” for draft contextual follow ups with suppression checks.
...
**Setup:** Timeline marks contact opted out.

**Action:** Request draft.

**Expected result:** Suppressed; no message queued.
...
**Likely failure diagnosis:** Check suppression before provider.

### Test 2 — Closed deal

**Purpose:** Prove the boundary named “Closed deal” for draft contextual follow ups with suppression checks.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Deal closes after request.

**Action:** Read/action draft.

**Expected result:** Stale/suppressed; no send.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Revalidate deal state.

### Test 3 — Unsupported promise

**Purpose:** Prove the boundary named “Unsupported promise” for draft contextual follow ups with suppression checks.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model invents guaranteed discount.

**Action:** Validate/review.

**Expected result:** Flag or reject; no automatic send.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Ground price claims against structured facts.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/followup.spec.ts
```
...
- [ ] Suppressed result contains no send action.
- [ ] Provider text cannot schedule another outreach.
- [ ] This ticket ends at a draft; no autonomous campaign.
