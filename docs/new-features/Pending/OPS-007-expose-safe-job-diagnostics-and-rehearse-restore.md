# OPS-007 — Expose safe job diagnostics and rehearse restore
...
Expose safe job diagnostics and rehearse restore. Health distinguishes process alive, dependencies ready, and customer operation failed. Support should diagnose by ID without reading private documents.

## 2. Current behavior and evidence

Runtime services have basic health behavior; silent notification degradation and job failures need an operator-visible support path.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/health/health.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/health/health.controller.ts) — exists at the baseline commit.
- [Backend/src/health/health.module.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/health/health.module.ts) — exists at the baseline commit.
...
Health distinguishes process alive, dependencies ready, and customer operation failed. Support should diagnose by ID without reading private documents.
...
Chosen approach: Health distinguishes process alive, dependencies ready, and customer operation failed. Support should diagnose by ID without reading private documents. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                              | Baseline state                                            |
| ------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/health/health.controller.ts`         | Existing — inspect before editing                         |
| `Backend/src/health/health.service.ts`            | Proposed — create here or consume dependency-created file |
| `docs/new-features/runbooks/support-triage.md`    | Proposed — create here or consume dependency-created file |
| `docs/new-features/runbooks/restore-rehearsal.md` | Proposed — create here or consume dependency-created file |
| `Backend/test/health-redaction.e2e-spec.ts`       | Proposed — create here or consume dependency-created file |
| `Backend/src/health/health.module.ts`             | Existing — inspect before editing                         |
| `Backend/src/health/admin-health.controller.ts`   | Proposed — create here or consume dependency-created file |
...
**Depends On:** [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md), [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [MEDIA-003](../Pending/MEDIA-003-serve-private-media-with-short-lived-authorized-urls.md)
...
HealthController currently has class-level @Public(). Keep public liveness in that controller; put private capability diagnostics in a new Backend/src/health/admin-health.controller.ts with ordinary auth/ADMIN guards and register it in health.module.ts. Never put a private method under an inherited public controller decorator.
...
**Where:** `Backend/src/health/health.controller.ts`

**Change:** Expose public liveness without topology/secrets; authenticated ADMIN capability health with DB/queue/storage/provider status and counts of oldest pending/failed jobs.

**Why:** Readiness and business failures are different signals.

**Verify immediately:** Unavailable Redis shown degraded/required-fail according to OPS-001.
...
**Where:** `Backend/src/health/health.service.ts`

**Change:** Write triage runbook by requestId/jobId/organizationId with safe error codes, authorized retry, incident owner and escalation severity.

**Why:** Support needs a concrete recovery action rather than raw payload logs.

**Verify immediately:** No tokens, document text, contacts or signedURLs in screenshots/log fixtures.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `docs/new-features/runbooks/support-triage.md`

**Change:** Rehearse DB+private-object restore into isolated environment using sanitized data; verify source/job/release references, deny-all access and replay behavior. Record measured RPO/RTO and gaps.

**Why:** Backup existence does not prove recovery.

**Verify immediately:** No production restore or send; restored workers initially disabled.
...
### Test 1 — Provider outage

**Purpose:** Prove the boundary named “Provider outage” for expose safe job diagnostics and rehearse restore.
...
**Setup:** AI provider down, DB healthy.

**Action:** Inspect health.

**Expected result:** AI degraded; app process live, manual workflow available.
...
**Why this test matters:** Health distinguishes process alive, dependencies ready, and customer operation failed. Support should diagnose by ID without reading private documents. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate capabilities.

### Test 2 — Sensitive error

**Purpose:** Prove the boundary named “Sensitive error” for expose safe job diagnostics and rehearse restore.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Worker exception includes token/source text.

**Action:** Read diagnostics.

**Expected result:** Redacted safe code/IDs only.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Health distinguishes process alive, dependencies ready, and customer operation failed. Support should diagnose by ID without reading private documents. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Serialize approved fields.

### Test 3 — Restore rehearsal

**Purpose:** Prove the boundary named “Restore rehearsal” for expose safe job diagnostics and rehearse restore.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Isolated backup fixture.

**Action:** Restore and verify refs.

**Expected result:** Counts reconcile, no outbound delivery, broken refs reported.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Health distinguishes process alive, dependencies ready, and customer operation failed. Support should diagnose by ID without reading private documents. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Start workers disabled.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/health-redaction.e2e-spec.ts
```
...
- [ ] Unavailable Redis shown degraded/required-fail according to OPS-001.
- [ ] No tokens, document text, contacts or signedURLs in screenshots/log fixtures.
- [ ] No production restore or send; restored workers initially disabled.
