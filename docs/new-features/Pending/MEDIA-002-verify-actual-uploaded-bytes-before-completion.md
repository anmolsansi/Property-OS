# MEDIA-002 — Verify actual uploaded bytes before completion
...
Verify actual uploaded bytes before completion. The server verifies bytes in storage. File names and client Content-Type are claims, not evidence.

## 2. Current behavior and evidence

completeUpload checks object existence but records client-provided size; it does not validate actual content before completion.
...
The server verifies bytes in storage. File names and client Content-Type are claims, not evidence.
...
Chosen approach: The server verifies bytes in storage. File names and client Content-Type are claims, not evidence. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/test/media-validation.e2e-spec.ts`                 | Proposed — create here or consume dependency-created file |
...
**Depends On:** [MEDIA-001](../Pending/MEDIA-001-authorize-upload-completion-and-downloads-by-parent.md), [DEC-003](../Pending/DEC-003-select-ai-providers-budgets-and-processing-regions.md)

**Blocks:** [MEDIA-004](../Pending/MEDIA-004-show-real-upload-progress-and-retry-states.md), [MEDIA-005](../Pending/MEDIA-005-clean-expired-pending-uploads-without-deleting-valid-files.md), [AI-010](../Pending/AI-010-extract-brochure-fields-with-page-level-evidence.md), [AI-013](../Pending/AI-013-transcribe-consented-voice-notes-into-editable-drafts.md), [AI-017](../Pending/AI-017-extract-lease-clauses-into-evidence-linked-drafts.md), [AI-023](../Pending/AI-023-suggest-photo-categories-and-quality-issues.md)
...
**Change:** Permit pilot JPEG/PNG/PDF only for new AI-source intake; keep existing video upload compatibility outside the intake capability until its separate policy is ratified.

**Why:** Do not silently break the existing media product.

**Verify immediately:** Unsupported AI source type fails before URL creation.
...
**Change:** Read HEAD ContentLength and bounded magic bytes; verify JPEG/PNG/PDF signatures, size limits and MIME agreement before marking completed. Ignore reported fileSizeBytes as authority.

**Why:** Existence alone does not prove a safe file.

**Verify immediately:** Oversized or mismatched file remains unavailable with reason code.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/media/__tests__/media.service.spec.ts`

**Change:** Timeout storage reads after10 seconds; fail closed, preserve prior media on replacements and allow a repeat completion only for the same verified object version/etag.

**Why:** Partial uploads must not become publishable.

**Verify immediately:** Retry is stable; no old valid media deleted on failure.
...
### Test 1 — Forged PDF type

**Purpose:** Prove the boundary named “Forged PDF type” for verify actual uploaded bytes before completion.
...
**Setup:** Stored executable bytes named brochure.pdf.

**Action:** Complete as PDF.

**Expected result:** 400 MEDIA_TYPE_MISMATCH; not completed.
...
**Why this test matters:** The server verifies bytes in storage. File names and client Content-Type are claims, not evidence. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check actual signature rather than extension.

### Test 2 — False size

**Purpose:** Prove the boundary named “False size” for verify actual uploaded bytes before completion.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Client claims100 bytes; actual object21MiB PDF.

**Action:** Complete intake upload.

**Expected result:** 400 MEDIA_TOO_LARGE; authoritative size used.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The server verifies bytes in storage. File names and client Content-Type are claims, not evidence. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Read HEAD ContentLength.

### Test 3 — Storage unavailable

**Purpose:** Prove the boundary named “Storage unavailable” for verify actual uploaded bytes before completion.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** HEAD times out; existing valid prior media.

**Action:** Complete replacement.

**Expected result:** 503; pending upload retained/failed; prior media intact.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The server verifies bytes in storage. File names and client Content-Type are claims, not evidence. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect replacement cleanup and timeout.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/media-validation.e2e-spec.ts
```
...
- [ ] Unsupported AI source type fails before URL creation.
- [ ] Oversized or mismatched file remains unavailable with reason code.
- [ ] Retry is stable; no old valid media deleted on failure.
