# DATA-007 — Migrate financial values through shadow decimal columns

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Money migration  
**Assessment:** D12  
...
Migrate financial values through shadow decimal columns. A migration changes persisted meaning, not only TypeScript types. Shadow values allow reconciliation before existing consumers move.

## 2. Current behavior and evidence

Unit, Requirement, Deal, Commission and Invoice monetary fields use Float in the inspected schema.
...
A migration changes persisted meaning, not only TypeScript types. Shadow values allow reconciliation before existing consumers move.
...
Chosen approach: A migration changes persisted meaning, not only TypeScript types. Shadow values allow reconciliation before existing consumers move. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915000400_shadow_money/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/scripts/reconcile-money.ts`                                  | Proposed — create here or consume dependency-created file |
| `Backend/test/money-migration.e2e-spec.ts`                            | Proposed — create here or consume dependency-created file |
...
**Blocks:** [DATA-008](../Pending/DATA-008-adopt-decimal-money-on-new-financial-contracts.md), [REQ-001](../Pending/REQ-001-persist-a-versioned-commercial-requirement-brief.md), [LEASE-001](../Pending/LEASE-001-create-the-lease-register-and-immutable-term-versions.md)
...
This is intentionally a schema/data ticket. MONEY-v1 consumer adoption is DATA-008; do not claim this ticket alone replaces Float arithmetic.
...
**Where:** `Backend/scripts/reconcile-money.ts`

**Change:** Read the DEC-004 field inventory and emit counts/min/max/rounding discrepancies for each field without customer values in console logs.

**Why:** Do not assume all numeric fields are amounts with two decimals.

**Verify immediately:** Audit identifies out-of-range and non-finite values and blocks apply.
...
**Where:** `Backend/prisma/schema.prisma`, `Backend/prisma/migrations/20260915000400_shadow_money/migration.sql`

**Change:** Add nullable <existingField>Decimal columns at the approved scale plus currency where absent; backfill in batches using approved round-half-up. Keep original columns.

**Why:** Additive changes preserve rollback and old clients.

**Verify immediately:** Counts and null/zero distinctions match approved reconciliation.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/test/money-migration.e2e-spec.ts`

**Change:** Exercise fractional, large, null, zero and negative-invalid fixtures across migration and reverse application compatibility.

**Why:** Silent value drift cannot be inferred from schema success.

**Verify immediately:** No original values dropped; new APIs remain disabled until dual-write/read adoption ticket.
...
### Test 1 — Fractional reconciliation

**Purpose:** Prove the boundary named “Fractional reconciliation” for migrate financial values through shadow decimal columns.
...
**Setup:** Values 0, null, 0.1+0.2 style legacy values and approved boundary amounts.

**Action:** Run shadow migration.

**Expected result:** Approved Decimal string values and preserved original columns.
...
**Why this test matters:** A migration changes persisted meaning, not only TypeScript types. Shadow values allow reconciliation before existing consumers move. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check rounding and casting, not JSON formatting.

### Test 2 — Dirty value blocks conversion

**Purpose:** Prove the boundary named “Dirty value blocks conversion” for migrate financial values through shadow decimal columns.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Out-of-range legacy value.

**Action:** Run audit/apply.

**Expected result:** Apply stops with safe count; no destructive conversion.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A migration changes persisted meaning, not only TypeScript types. Shadow values allow reconciliation before existing consumers move. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not clamp silently.

### Test 3 — Rerun is safe

**Purpose:** Prove the boundary named “Rerun is safe” for migrate financial values through shadow decimal columns.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Already reconciled shadow data.

**Action:** Rerun reconciliation.

**Expected result:** No value drift or duplicate metadata.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A migration changes persisted meaning, not only TypeScript types. Shadow values allow reconciliation before existing consumers move. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check repeat rounding and resume key.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/money-migration.e2e-spec.ts
...
- [ ] Audit identifies out-of-range and non-finite values and blocks apply.
- [ ] Counts and null/zero distinctions match approved reconciliation.
- [ ] No original values dropped; new APIs remain disabled until dual-write/read adoption ticket.
