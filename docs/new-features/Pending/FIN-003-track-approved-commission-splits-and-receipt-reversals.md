# FIN-003 — Track approved commission splits and receipt reversals

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Broker receivables  
**Assessment:** P21  
...
Track approved commission splits and receipt reversals. An entitlement states what is owed; a receipt states what arrived. They cannot be one mutable paid flag.

## 2. Current behavior and evidence

Current Commission/Invoice Float records do not define split entitlement, partial allocation or reversals.
...
An entitlement states what is owed; a receipt states what arrived. They cannot be one mutable paid flag.
...
Chosen approach: An entitlement states what is owed; a receipt states what arrived. They cannot be one mutable paid flag. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915002200_commission_ledger/migration.sql`  | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/deals/commission-ledger.service.ts`                    | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/deals/deals.controller.ts`                             | Existing — inspect before editing                         |
| `Backend/src/modules/deals/__tests__/commission-ledger.integration.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [DEC-008](../Pending/DEC-008-choose-lease-offer-approval-and-accounting-boundaries.md), [DATA-008](../Pending/DATA-008-adopt-decimal-money-on-new-financial-contracts.md), [TEN-007](../Pending/TEN-007-enforce-tenant-access-throughout-deals.md)

**Blocks:** [FIN-004](../Pending/FIN-004-show-offer-approvals-rent-roll-and-commission-balances.md)
...
Wire service via deals.module.ts; do not rewrite historical Float records as reconciled without DATA-007 reconciliation evidence.
...
**Change:** Add CommissionEntitlement(id, organizationId, dealId, version, total, currency, approvedBy), Split(id, entitlementId, payeeUserId, amount), Receipt(id, organizationId, amount, currency, externalRef, recordedBy), Allocation(id, receiptId, entitlementId, amount, reversalOfId nullable); scoped externalRef unique.

**Why:** A ledger preserves partial payments and correction history.

**Verify immediately:** Splits sum exactly total; no negative ordinary allocations.
...
**Where:** `Backend/src/modules/deals/commission-ledger.service.ts`, `Backend/src/modules/deals/deals.controller.ts`

**Change:** POST /deals/:id/commission-entitlements ADMIN creates approved version under DEC-008; POST /commission-receipts records verified receipt then allocations in one transaction with Idempotency-Key.

**Why:** No double-counting when receipt import retries.

**Verify immediately:** Allocated amount cannot exceed receipt or entitlement outstanding.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/deals/commission-ledger.service.ts`

**Change:** Reverse by append linked reversal within remaining unreversed amount; return derived outstanding and full audit trail.

**Why:** Deletion would erase financial reconciliation.

**Verify immediately:** No bank/payment API invoked; historical approved records immutable.
...
**Purpose:** Prove the boundary named “Partial receipt” for track approved commission splits and receipt reversals.
...
**Setup:** Entitlement1000 receipt300.

**Action:** Allocate.

**Expected result:** Outstanding700 exactly.
...
**Why this test matters:** An entitlement states what is owed; a receipt states what arrived. They cannot be one mutable paid flag. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Sum ledger amounts, not paid boolean.

### Test 2 — Concurrent allocation

**Purpose:** Prove the boundary named “Concurrent allocation” for track approved commission splits and receipt reversals.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Two allocate same remaining balance.

**Action:** Commit concurrently.

**Expected result:** One exceeding operation409; totals bounded.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An entitlement states what is owed; a receipt states what arrived. They cannot be one mutable paid flag. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Lock balances in transaction.

### Test 3 — Duplicate reversal

**Purpose:** Prove the boundary named “Duplicate reversal” for track approved commission splits and receipt reversals.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same allocation reversed twice.

**Action:** Retry.

**Expected result:** One reversal or409; outstanding not inflated.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An entitlement states what is owed; a receipt states what arrived. They cannot be one mutable paid flag. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Enforce remaining reversible balance.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/deals/__tests__/commission-ledger.integration.spec.ts
...
- [ ] Splits sum exactly total; no negative ordinary allocations.
- [ ] Allocated amount cannot exceed receipt or entitlement outstanding.
- [ ] No bank/payment API invoked; historical approved records immutable.
