# AI-019 — Summarize renewal preparation from reviewed obligations
...
Summarize renewal preparation from reviewed obligations. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

LEASE-003 computes authoritative obligations; AI can help explain preparation work.
...
| File                                                        | Baseline state                                            |
| ----------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/renewal-briefing.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/renewal-briefing.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/renewal-briefing.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-017](../Pending/AI-017-extract-lease-clauses-into-evidence-linked-drafts.md), [LEASE-003](../Pending/LEASE-003-generate-and-complete-reviewed-lease-obligations.md)
...
Command API: POST /api/v1/ai/renewal-briefing with Idempotency-Key; input {leaseId:UUID, asOf:YYYY-MM-DD}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {leaseId:UUID, obligationIds:UUID[], summary:string<=3000, evidence:Evidence[], draftActions:[{title:string<=200, obligationId:UUID}]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/renewal-briefing.service.ts`

**Change:** Retrieve only active reviewed terms and scheduled obligations in next90 days; preserve overdue flags from code.

**Why:** A model cannot invent a notice deadline.

**Verify immediately:** Every date rendered from obligation row.
...
**Where:** `Backend/src/modules/ai/dto/renewal-briefing.schema.ts`

**Change:** Generate cited preparation summary and optional task drafts, flag missing source/context.

**Why:** Draft actions must not imply legal advice or delivered notice.

**Verify immediately:** No generated deadline value accepted as canonical.
...
**Change:** Recheck active lease version at output retrieval and task-apply time; stale briefing marked superseded.

**Why:** A lease amendment can invalidate a prepared message.

**Verify immediately:** No task creation or outbound notice without separate explicit action.
...
### Test 1 — Amended lease

**Purpose:** Prove the boundary named “Amended lease” for summarize renewal preparation from reviewed obligations.
...
**Setup:** Briefing generated onv1;v2 activated.

**Action:** Open briefing.

**Expected result:** Superseded warning and no apply.
...
**Likely failure diagnosis:** Compare active version.

### Test 2 — Invented deadline

**Purpose:** Prove the boundary named “Invented deadline” for summarize renewal preparation from reviewed obligations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model changes notice date.

**Action:** Validate.

**Expected result:** Rejected field; canonical date retained.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use structured date rendering.

### Test 3 — No obligations

**Purpose:** Prove the boundary named “No obligations” for summarize renewal preparation from reviewed obligations.
...
**Expected result:** No pending action, no invented renewal urgency.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Handle empty query.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/renewal-briefing.spec.ts
```
...
- [ ] Every date rendered from obligation row.
- [ ] No generated deadline value accepted as canonical.
- [ ] No task creation or outbound notice without separate explicit action.
