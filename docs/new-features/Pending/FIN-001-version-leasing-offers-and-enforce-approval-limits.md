# FIN-001 — Version leasing offers and enforce approval limits

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Offer approvals  
**Assessment:** P19  
...
Version leasing offers and enforce approval limits. An internal approval records permitted commercial terms. It does not accept a lease on behalf of either party.

## 2. Current behavior and evidence

Proposal releases disclose listings; they are not signed leasing offers or delegated approval records.
...
An internal approval records permitted commercial terms. It does not accept a lease on behalf of either party.
...
Chosen approach: An internal approval records permitted commercial terms. It does not accept a lease on behalf of either party. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915002100_lease_offers/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/offer.service.ts`                         | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/leases.controller.ts`                     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/leases/__tests__/offers.integration.spec.ts`     | Proposed — create here or consume dependency-created file |
...
**Depends On:** [DEC-008](../Pending/DEC-008-choose-lease-offer-approval-and-accounting-boundaries.md), [LEASE-002](../Pending/LEASE-002-review-and-activate-lease-versions-atomically.md), [DATA-008](../Pending/DATA-008-adopt-decimal-money-on-new-financial-contracts.md)
...
Wire offer service via leases.module.ts; UI comparison and approval controls are FIN-004.
...
**Change:** Add LeaseOffer(id, leaseId, organizationId, status draft|approved|withdrawn, version) and immutable OfferVersion(offerId, version, terms, createdBy), OfferApproval(versionId, approverId, policyVersion, createdAt); unique version/approver.

**Why:** Approval must bind exact terms.

**Verify immediately:** Editing approved offer creates a new unapproved revision.
...
**Where:** `Backend/src/modules/leases/offer.service.ts`, `Backend/src/modules/leases/leases.controller.ts`

**Change:** POST /leases/:id/offers stores strict Terms-v1 plus expectedLeaseVersion; POST /lease-offers/:id/approve requires expectedOfferVersion and DEC-008 approved limits.

**Why:** Comparing amount alone can hide a changed lease term.

**Verify immediately:** Missing policy denies approval, no permissive fallback.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/leases/offer.service.ts`

**Change:** GET comparison returns same-basis decimal amounts, term lengths, deposit separately and excluded taxes/escalation warning; withdrawing never deletes approval history.

**Why:** Internal selection must remain auditable.

**Verify immediately:** No email, signature or lease activation side effect.
...
### Test 1 — Changed revision

**Purpose:** Prove the boundary named “Changed revision” for version leasing offers and enforce approval limits.
...
**Setup:** v1 approved, v2 changes term.

**Action:** Read v2.

**Expected result:** Unapproved.
...
**Why this test matters:** An internal approval records permitted commercial terms. It does not accept a lease on behalf of either party. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Bind approval to revision.

### Test 2 — Limit breach

**Purpose:** Prove the boundary named “Limit breach” for version leasing offers and enforce approval limits.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Approver below required limit.

**Action:** Approve.

**Expected result:** 403; no approval.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An internal approval records permitted commercial terms. It does not accept a lease on behalf of either party. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Enforce limits on server.

### Test 3 — No policy

**Purpose:** Prove the boundary named “No policy” for version leasing offers and enforce approval limits.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Org lacks signed threshold config.

**Action:** Approve.

**Expected result:** 409 POLICY_REQUIRED.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An internal approval records permitted commercial terms. It does not accept a lease on behalf of either party. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Fail closed on missing setup.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/leases/__tests__/offers.integration.spec.ts
...
- [ ] Editing approved offer creates a new unapproved revision.
- [ ] Missing policy denies approval, no permissive fallback.
- [ ] No email, signature or lease activation side effect.
