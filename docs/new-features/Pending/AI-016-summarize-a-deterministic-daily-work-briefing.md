# AI-016 — Summarize a deterministic daily work briefing
...
Summarize a deterministic daily work briefing. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

Dashboard and task services exist; no cited daily AI briefing is implemented.
...
| File                                                     | Baseline state                                            |
| -------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/work-briefing.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/work-briefing.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/work-briefing.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [TEN-006](../Pending/TEN-006-enforce-tenant-access-throughout-tasks.md), [INV-001](../Pending/INV-001-store-per-field-inventory-verification-evidence.md)
...
Command API: POST /api/v1/ai/work-briefing with Idempotency-Key; input {date:YYYY-MM-DD}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {date:YYYY-MM-DD, summary:string<=3000, items:[{entityType:task|deal|verification, entityId:UUID, reason:string<=500}]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/work-briefing.service.ts`

**Change:** Compute overdue tasks, stalled assigned deals and verification backlog via scoped deterministic queries; cap50 items.

**Why:** AI should describe a query result, not invent workload.

**Verify immediately:** Counts use DB predicates and requested local day boundary.
...
**Where:** `Backend/src/modules/ai/dto/work-briefing.schema.ts`

**Change:** Generate short summary referencing only supplied IDs; render counts/dates directly from structured results.

**Why:** A generated number is not an analytics source.

**Verify immediately:** Output cannot include another user/organization item.
...
**Change:** When provider fails show the deterministic list with a brief unavailable message.

**Why:** Daily work should continue without model availability.

**Verify immediately:** No task priority or due date is mutated.
...
**Purpose:** Prove the boundary named “No work” for summarize a deterministic daily work briefing.
...
**Setup:** No overdue items.

**Action:** Request briefing.

**Expected result:** Empty state and zero verified counts; no invented tasks.
...
**Likely failure diagnosis:** Handle empty input without prompting for creative content.

### Test 2 — Unknown ID

**Purpose:** Prove the boundary named “Unknown ID” for summarize a deterministic daily work briefing.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model cites nonexistent task.

**Action:** Validate.

**Expected result:** INVALID_OUTPUT; deterministic list retained.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check ID allowlist.

### Test 3 — Outage fallback

**Purpose:** Prove the boundary named “Outage fallback” for summarize a deterministic daily work briefing.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Valid workload, provider timeout.

**Action:** Open briefing.

**Expected result:** Work list usable; no lost task state.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Keep query independent of generation.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/work-briefing.spec.ts
```
...
- [ ] Counts use DB predicates and requested local day boundary.
- [ ] Output cannot include another user/organization item.
- [ ] No task priority or due date is mutated.
