# TEN-005 — Enforce tenant access throughout units
...
Enforce tenant access throughout units. A Unit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. unit detail and writes inherit parent access.

## 2. Current behavior and evidence

units routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/units/units.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/units/units.controller.ts) — exists at the baseline commit.
- [Backend/src/modules/units/units.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/units/units.service.ts) — exists at the baseline commit.
- [Backend/src/modules/units/**tests**/units.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/units/__tests__/units.service.spec.ts) — exists at the baseline commit.
...
A Unit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. unit detail and writes inherit parent access.
...
Chosen approach: A Unit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. unit detail and writes inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                        | Baseline state                                            |
| ----------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/units/units.controller.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/units/units.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/units/__tests__/units.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/units-tenant.e2e-spec.ts`                     | Proposed — create here or consume dependency-created file |
...
**Blocks:** [PROP-002](../Pending/PROP-002-validate-proposal-item-hierarchy-before-writes.md), [DATA-005](../Pending/DATA-005-make-import-row-application-atomic-and-resumable.md), [DATA-006](../Pending/DATA-006-enforce-shortlist-entity-relationships.md), [MEDIA-001](../Pending/MEDIA-001-authorize-upload-completion-and-downloads-by-parent.md), [MATCH-001](../Pending/MATCH-001-persist-and-compute-authorized-match-runs.md), [INV-004](../Pending/INV-004-merge-reviewed-building-duplicates-transactionally.md), [CRM-005](../Pending/CRM-005-save-and-evaluate-access-scoped-searches.md), [LEASE-001](../Pending/LEASE-001-create-the-lease-register-and-immutable-term-versions.md)
...
**Where:** `Backend/src/modules/units/units.controller.ts`
...
**Where:** `Backend/src/modules/units/units.service.ts`

**Change:** Conjoin organization and existing role/geography restrictions for list/count/detail through building. Resolve each supplied parent before create or reparent.
...
**Change:** Authorize unit detail and writes via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/units-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout units.
...
**Why this test matters:** A Unit and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. unit detail and writes inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout units.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout units.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/units/__tests__/units.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/units-tenant.e2e-spec.ts
```
