# PROP-002 — Validate proposal item hierarchy before writes
...
Validate proposal item hierarchy before writes. A floor or unit ID must be resolved to its real parent. Client-supplied parent IDs cannot lend permission to a different asset.

## 2. Current behavior and evidence

AddProposalItemSchema checks ID presence. addItem verifies building geography only when entityType is building, not floor/unit lineage.
...
- [Backend/src/modules/proposals/dto/proposals.schema.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/proposals/dto/proposals.schema.ts) — exists at the baseline commit.
...
A floor or unit ID must be resolved to its real parent. Client-supplied parent IDs cannot lend permission to a different asset.
...
Chosen approach: A floor or unit ID must be resolved to its real parent. Client-supplied parent IDs cannot lend permission to a different asset. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/src/modules/proposals/dto/proposals.schema.ts`             | Existing — inspect before editing |
...
**Depends On:** [PROP-001](../Pending/PROP-001-make-proposal-creator-and-geography-access-explicit.md), [TEN-005](../Pending/TEN-005-enforce-tenant-access-throughout-units.md)

**Blocks:** [PROP-004](../Pending/PROP-004-enforce-proposal-item-uniqueness-under-concurrency.md)
...
**Where:** `Backend/src/modules/proposals/dto/proposals.schema.ts`

**Change:** Reject extra IDs on building items; floor requires buildingId/floorId only; unit requires buildingId/unitId and permits floorId null for genuinely floorless units.

**Why:** The Prisma Unit.floorId is nullable; do not invent a floor requirement.

**Verify immediately:** Malformed combinations return 400 before mutation.
...
**Change:** Resolve unit/floor with actual building and floor; compare supplied parents; reject mismatches with 400 ITEM_PARENT_MISMATCH. Require active parent and same tenant/geography before create or restoration.

**Why:** A valid UUID alone is not a valid relationship.

**Verify immediately:** Cross-linked fixtures fail with no proposalItem write.
...
**Change:** Validate soft-deleted item restoration against current parents and access rather than blindly clearing removedAt.

**Why:** Permissions and asset state may have changed since removal.

**Verify immediately:** Forbidden restored item stays removed.
...
### Test 1 — Unit from foreign building

**Purpose:** Prove the boundary named “Unit from foreign building” for validate proposal item hierarchy before writes.
...
**Setup:** Unit belongs to B; request claims permitted A.

**Action:** addItem unit with building A.

**Expected result:** 400 or 404 if B inaccessible; zero create/update.
...
**Why this test matters:** A floor or unit ID must be resolved to its real parent. Client-supplied parent IDs cannot lend permission to a different asset. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Resolve actual parent before supplied parent authorization.

### Test 2 — Floorless unit remains supported

**Purpose:** Prove the boundary named “Floorless unit remains supported” for validate proposal item hierarchy before writes.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same-tenant unit.floorId=null.

**Action:** Add unit with no floorId.

**Expected result:** Success with null floor; no invented floor record.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A floor or unit ID must be resolved to its real parent. Client-supplied parent IDs cannot lend permission to a different asset. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check refined DTO and service expectations.

### Test 3 — Restoration rechecks access

**Purpose:** Prove the boundary named “Restoration rechecks access” for validate proposal item hierarchy before writes.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Removed item parent now deleted or outside scope.

**Action:** Add same item to restore.

**Expected result:** 404; removedAt unchanged.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A floor or unit ID must be resolved to its real parent. Client-supplied parent IDs cannot lend permission to a different asset. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect early duplicate/restore branch.
...
- [ ] Malformed combinations return 400 before mutation.
- [ ] Cross-linked fixtures fail with no proposalItem write.
- [ ] Forbidden restored item stays removed.
