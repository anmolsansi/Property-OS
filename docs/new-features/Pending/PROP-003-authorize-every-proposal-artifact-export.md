# PROP-003 — Authorize every proposal artifact export
...
Authorize every proposal artifact export. Download is a read of every disclosed record, not an exception to API authorization. Field permission and object permission must both pass.

## 2. Current behavior and evidence

XLSX export loads a proposal by ID and checks field role restrictions but does not use userId for object authorization. Legacy PDF reads by ID too.
...
- [Backend/src/modules/proposals/proposal-export.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/proposals/proposal-export.service.ts) — exists at the baseline commit.
...
- [Backend/src/modules/proposals/**tests**/proposal-export.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/proposals/__tests__/proposal-export.service.spec.ts) — exists at the baseline commit.
...
Download is a read of every disclosed record, not an exception to API authorization. Field permission and object permission must both pass.
...
Chosen approach: Download is a read of every disclosed record, not an exception to API authorization. Field permission and object permission must both pass. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/src/modules/proposals/proposal-export.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/proposals/proposals.service.ts`                      | Existing — inspect before editing                         |
| `Backend/src/modules/proposals/__tests__/proposal-export.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/proposal-export-access.e2e-spec.ts`                         | Proposed — create here or consume dependency-created file |
...
**Depends On:** [PROP-001](../Pending/PROP-001-make-proposal-creator-and-geography-access-explicit.md)

**Blocks:** [MEDIA-003](../Pending/MEDIA-003-serve-private-media-with-short-lived-authorized-urls.md), [PROP-006](../Pending/PROP-006-create-immutable-reviewed-proposal-releases.md), [OPS-005](../Pending/OPS-005-replace-superficial-search-and-proposal-acceptance-checks.md)
...
**Change:** Apply context to XLSX and PDF routes, forwarding it to exporters.

**Why:** Unmarked export routes bypass geography setup.

**Verify immediately:** The service sees current actor/org on both routes.
...
**Where:** `Backend/src/modules/proposals/proposal-export.service.ts`

**Change:** Invoke proposal and all-active-item authorization before loading export values or fetching images. Keep restricted field checks.

**Why:** Rejecting after artifact construction is too late.

**Verify immediately:** Denied export makes no image fetch or status update.
...
**Change:** Route legacy PDF through the same authorization check; do not implement document layout here.

**Why:** The stub must not remain an unauthorized metadata query.

**Verify immediately:** Cross-tenant PDF and XLSX return identical inaccessible-object behavior.
...
### Test 1 — Forbidden export does no work

**Purpose:** Prove the boundary named “Forbidden export does no work” for authorize every proposal artifact export.
...
**Setup:** OrgA worker requests orgB proposal.

**Action:** POST export with public fields.

**Expected result:** 404; zero artifact/image/provider calls and zero status writes.
...
**Why this test matters:** Download is a read of every disclosed record, not an exception to API authorization. Field permission and object permission must both pass. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Ensure authorization precedes data gathering.

### Test 2 — Restricted fields remain forbidden

**Purpose:** Prove the boundary named “Restricted fields remain forbidden” for authorize every proposal artifact export.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Permitted proposal; WORKER requests internalNotes.

**Action:** Export XLSX.

**Expected result:** 403; no output buffer delivered.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Download is a read of every disclosed record, not an exception to API authorization. Field permission and object permission must both pass. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not confuse same-tenant access with field permission.

### Test 3 — Same-tenant allowed artifact works

**Purpose:** Prove the boundary named “Same-tenant allowed artifact works” for authorize every proposal artifact export.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Permitted items and public fields.

**Action:** Download current XLSX.

**Expected result:** Valid XLSX with expected values only.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Download is a read of every disclosed record, not an exception to API authorization. Field permission and object permission must both pass. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect retained projection and binary response envelope.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/proposals/__tests__/proposal-export.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/proposal-export-access.e2e-spec.ts
```
...
- [ ] The service sees current actor/org on both routes.
- [ ] Denied export makes no image fetch or status update.
- [ ] Cross-tenant PDF and XLSX return identical inaccessible-object behavior.
