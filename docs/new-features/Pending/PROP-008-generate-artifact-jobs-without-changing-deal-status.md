# PROP-008 — Generate artifact jobs without changing deal status
...
Generate artifact jobs without changing deal status. The file job can fail without changing what the client accepted. A ready artifact means the exact file is durably stored.

## 2. Current behavior and evidence

Current exporter sets proposal exported before serialization completes; artifact state is mixed with proposal commercial status.
...
The file job can fail without changing what the client accepted. A ready artifact means the exact file is durably stored.
...
Chosen approach: The file job can fail without changing what the client accepted. A ready artifact means the exact file is durably stored. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                       | Baseline state                                            |
| ---------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/proposals/artifact.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/proposals/artifact.worker.ts`         | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/proposals/__tests__/artifact.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [PROP-007](../Pending/PROP-007-render-pdf-and-xlsx-from-immutable-release-data.md), [OPS-003](../Pending/OPS-003-dispatch-outbox-events-with-retry-safe-claims.md)

**Blocks:** [PROP-009](../Pending/PROP-009-add-review-release-and-download-ui.md)
...
**Where:** `Backend/src/modules/proposals/artifact.service.ts`

**Change:** Implement artifact create/read endpoints and transactionally publish render intent; unique releaseId/format deduplicates requests.

**Why:** Double-click generates one logical artifact.

**Verify immediately:** Pending response is202, not a download success.
...
**Where:** `Backend/src/modules/proposals/artifact.worker.ts`

**Change:** Worker reads release, reauthorizes stored actor, renders and uploads private object; marks ready only after storage confirmation.

**Why:** Serialization/upload can fail independently.

**Verify immediately:** No ready flag or business-status update before durable object.
...
**Change:** Return signed download only to currently authorized caller; retry uses same release/hash, preserves failure reason; do not regress accepted/sent Proposal.status.

**Why:** An export is not a sales lifecycle transition.

**Verify immediately:** Late retry never rewrites accepted proposal status.
...
### Test 1 — Serialization failure

**Purpose:** Prove the boundary named “Serialization failure” for generate artifact jobs without changing deal status.
...
**Setup:** Renderer throws.

**Action:** Run job.

**Expected result:** failed; no ready/download URL; proposal status unchanged.
...
**Why this test matters:** The file job can fail without changing what the client accepted. A ready artifact means the exact file is durably stored. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate job state from commercial status.

### Test 2 — Duplicate request

**Purpose:** Prove the boundary named “Duplicate request” for generate artifact jobs without changing deal status.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Two artifact requests same release/format.

**Action:** Execute together.

**Expected result:** One job; same artifact ID.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The file job can fail without changing what the client accepted. A ready artifact means the exact file is durably stored. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check unique constraint.

### Test 3 — Revoked before download

**Purpose:** Prove the boundary named “Revoked before download” for generate artifact jobs without changing deal status.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Ready artifact, actor removed.

**Action:** GET status/download.

**Expected result:** 404/403; no signed URL.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The file job can fail without changing what the client accepted. A ready artifact means the exact file is durably stored. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Recheck access at read time.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/proposals/__tests__/artifact.spec.ts
```
...
- [ ] Pending response is202, not a download success.
- [ ] No ready flag or business-status update before durable object.
- [ ] Late retry never rewrites accepted proposal status.
