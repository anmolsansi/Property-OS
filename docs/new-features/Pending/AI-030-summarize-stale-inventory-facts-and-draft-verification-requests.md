# AI-030 — Summarize stale inventory facts and draft verification requests
...
Summarize stale inventory facts and draft verification requests. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

INV-001/002 provide deterministic missing/expired facts; AI can summarize discrepancies but cannot refresh them.
...
| File                                                         | Baseline state                                            |
| ------------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/ai/freshness-summary.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/freshness-summary.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/freshness-summary.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [INV-002](../Pending/INV-002-expose-the-verification-queue-and-review-actions.md), [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md)
...
Command API: POST /api/v1/ai/freshness-summary with Idempotency-Key; input {buildingId:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {buildingId:UUID, issues:[{fieldKey:string, verificationFactId:UUID|null, summary:string}], requestDraft:string<=2000, evidence:Evidence[]}. Register freshness_summary in the closed AI-001 command registry and worker dispatcher; add held-out grounding cases. No unrelated command shape changes.
...
**Where:** `Backend/src/modules/ai/freshness-summary.service.ts`

**Change:** Load only scoped current/history verification facts and explicit expiry reasons; exclude private contacts from provider input.

**Why:** Staleness is a rule result, not a model opinion.

**Verify immediately:** Issue IDs match fact ledger; no cross-asset facts.
...
**Where:** `Backend/src/modules/ai/dto/freshness-summary.schema.ts`

**Change:** Generate concise verification request draft distinguishing last confirmed value from proposed conflicting value; label unknown source.

**Why:** Conflicting claims must not be merged into a new fact.

**Verify immediately:** No invented current rent/availability.
...
**Change:** Return copyable draft; require user-controlled existing communication workflow for any send. Generating request never updates verifiedAt/reviewDueAt.

**Why:** Asking for verification does not complete it.

**Verify immediately:** Provider failure leaves queue usable.
...
**Purpose:** Prove the boundary named “Expired rent” for summarize stale inventory facts and draft verification requests.
...
**Setup:** Old fact and newer unapproved request.

**Action:** Summarize.

**Expected result:** Distinguishes approved history from pending claim.
...
**Likely failure diagnosis:** Do not promote pending value.

### Test 2 — False freshness

**Purpose:** Prove the boundary named “False freshness” for summarize stale inventory facts and draft verification requests.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Provider says verified today.

**Action:** Validate.

**Expected result:** Reject unsupported date; ledger unchanged.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Dates rendered from structured facts.

### Test 3 — Outage

**Purpose:** Prove the boundary named “Outage” for summarize stale inventory facts and draft verification requests.
...
**Expected result:** Deterministic stale list still works.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Keep AI optional.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/freshness-summary.spec.ts
```
...
- [ ] Issue IDs match fact ledger; no cross-asset facts.
- [ ] No invented current rent/availability.
- [ ] Provider failure leaves queue usable.
