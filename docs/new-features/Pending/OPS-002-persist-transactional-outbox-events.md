# OPS-002 — Persist transactional outbox events
...
Persist transactional outbox events. Database commit and Redis enqueue cannot be one transaction. Commit the event with the business record and publish it later.

## 2. Current behavior and evidence

Existing queues accept notifications directly; no OutboxEvent model exists in the inspected schema.
...
Database commit and Redis enqueue cannot be one transaction. Commit the event with the business record and publish it later.
...
Chosen approach: Database commit and Redis enqueue cannot be one transaction. Commit the event with the business record and publish it later. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915000500_outbox/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/shared/services/outbox.service.ts`                 | Proposed — create here or consume dependency-created file |
| `Backend/src/shared/services/outbox.service.spec.ts`            | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-001](../Pending/TEN-001-create-an-explicit-server-derived-access-context.md)

**Blocks:** [OPS-003](../Pending/OPS-003-dispatch-outbox-events-with-retry-safe-claims.md), [AI-001](../Pending/AI-001-create-private-source-job-and-review-persistence.md), [LEASE-002](../Pending/LEASE-002-review-and-activate-lease-versions-atomically.md), [METRIC-001](../Pending/METRIC-001-measure-the-intake-to-visit-funnel-without-private-payloads.md)
...
**Change:** Add OutboxEvent exactly as OUTBOX-v1 with unique dedupe key and pending scan index.

**Why:** The event is durable even if Redis is unavailable.

**Verify immediately:** Migration succeeds on clean data and leaves domain schemas compatible.
...
**Where:** `Backend/src/shared/services/outbox.service.ts`

**Change:** Implement enqueueInTransaction(tx, event) requiring an existing Prisma transaction client; validate payload identifiers and required tenant.

**Why:** Do not accidentally open a second transaction.

**Verify immediately:** Rollback of caller transaction removes its event.
...
**Change:** Reject same dedupe key with different payload hash; identical replay returns existing ID.

**Why:** A key collision must not silently substitute another operation.

**Verify immediately:** Same-event repeats have one row.
...
### Test 1 — Domain rollback

**Purpose:** Prove the boundary named “Domain rollback” for persist transactional outbox events.
...
**Setup:** Transaction inserts entity then outbox, then throws.

**Action:** Commit attempt.

**Expected result:** Neither entity nor outbox persists.
...
**Why this test matters:** Database commit and Redis enqueue cannot be one transaction. Commit the event with the business record and publish it later. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Ensure passed tx is used.

### Test 2 — Idempotency

**Purpose:** Prove the boundary named “Idempotency” for persist transactional outbox events.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same key and payload twice.

**Action:** Enqueue twice.

**Expected result:** One event; same ID. Different payload409.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Database commit and Redis enqueue cannot be one transaction. Commit the event with the business record and publish it later. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect unique boundary and payload comparison.

### Test 3 — No private payload

**Purpose:** Prove the boundary named “No private payload” for persist transactional outbox events.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Event contains document text/token field.

**Action:** Validate event.

**Expected result:** Rejected; no row.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Database commit and Redis enqueue cannot be one transaction. Commit the event with the business record and publish it later. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use strict ID-only payload schema.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/shared/services/outbox.service.spec.ts
...
- [ ] Migration succeeds on clean data and leaves domain schemas compatible.
- [ ] Rollback of caller transaction removes its event.
- [ ] Same-event repeats have one row.
