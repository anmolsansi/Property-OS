# AI-009 — Draft proposal rationale from approved public facts
...
Draft proposal rationale from approved public facts. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

Proposal creation/export exists; no evidence-grounded narrative generation is implemented.
...
| File                                                          | Baseline state                                            |
| ------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/proposal-narrative.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/proposal-narrative.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/proposal-narrative.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md), [PROP-006](../Pending/PROP-006-create-immutable-reviewed-proposal-releases.md)
...
Command API: POST /api/v1/ai/proposal-narrative with Idempotency-Key; input {proposalId:UUID, expectedUpdatedAt:ISO8601}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {proposalId:UUID, summary:string<=2000, itemRationales:[{itemId:UUID, text:string<=1000, evidence:Evidence[]}]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/proposal-narrative.service.ts`

**Change:** Build provider input from public allowlisted proposal values after PROP-001 access checks.

**Why:** Internal notes and commissions must never be in a client narrative prompt.

**Verify immediately:** Snapshot records exact IDs/versions used.
...
**Where:** `Backend/src/modules/ai/dto/proposal-narrative.schema.ts`

**Change:** Generate prose with placeholders for figures; render monetary/area/date figures deterministically from approved values, rejecting unsupported amenity claims.

**Why:** Fluent text cannot invent financial facts.

**Verify immediately:** Critical numbers agree exactly with source snapshot.
...
**Change:** Store narrative draft for review only; user may paste approved narrative into draft proposal, then normal release review applies.

**Why:** Generation does not mean release or send.

**Verify immediately:** Zero ShareGrant or artifact created by this command.
...
### Test 1 — Sensitive data excluded

**Purpose:** Prove the boundary named “Sensitive data excluded” for draft proposal rationale from approved public facts.
...
**Setup:** Proposal has internal notes/commission.

**Action:** Capture provider request.

**Expected result:** No private field appears.
...
**Likely failure diagnosis:** Inspect projection before prompt building.

### Test 2 — Unsupported claim

**Purpose:** Prove the boundary named “Unsupported claim” for draft proposal rationale from approved public facts.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Provider states fully leased/safe/guaranteed returns.

**Action:** Validate narrative.

**Expected result:** Rejected or flagged unsupported; no public release.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Constrain generated claim vocabulary and evidence.

### Test 3 — Stale proposal

**Purpose:** Prove the boundary named “Stale proposal” for draft proposal rationale from approved public facts.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Timestamp changed after request.

**Action:** Review/use draft.

**Expected result:** Conflict warning; cannot silently apply old narrative to new facts.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check base version on apply.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/proposal-narrative.spec.ts
```
...
- [ ] Snapshot records exact IDs/versions used.
- [ ] Critical numbers agree exactly with source snapshot.
- [ ] Zero ShareGrant or artifact created by this command.
