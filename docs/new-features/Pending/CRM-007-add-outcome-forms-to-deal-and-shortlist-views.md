# CRM-007 — Add outcome forms to deal and shortlist views
...
Add outcome forms to deal and shortlist views. Unknown is a valid answer. Requiring a guess would pollute later product analytics and model evaluation.

## 2. Current behavior and evidence

CRM-001 provides atomic outcome recording; current views need a deliberate reason selection.
...
Unknown is a valid answer. Requiring a guess would pollute later product analytics and model evaluation.
...
Chosen approach: Unknown is a valid answer. Requiring a guess would pollute later product analytics and model evaluation. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                              | Baseline state                                            |
| ------------------------------------------------- | --------------------------------------------------------- |
| `Frontend/src/hooks/use-outcomes.ts`              | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/deals/OutcomeDialog.tsx` | Proposed — create here or consume dependency-created file |
...
| `Frontend/e2e/outcomes.spec.ts`                   | Proposed — create here or consume dependency-created file |
...
**Depends On:** [CRM-001](../Pending/CRM-001-capture-explicit-lost-deal-and-rejected-property-reasons.md)
...
**Where:** `Frontend/src/hooks/use-outcomes.ts`

**Change:** Mount loss/rejection dialog on explicit actions; require category selection with no preselected reason.

**Why:** The first dropdown option must not become accidental ground truth.

**Verify immediately:** Cancel closes dialog with no mutation.
...
**Where:** `Frontend/src/components/deals/OutcomeDialog.tsx`

**Change:** Submit status/outcome together through CRM-001; other requires note; unknown allows save.

**Why:** Two separate client calls could lose the reason.

**Verify immediately:** One transaction response confirms both changes.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/app/(v2)/deals/[id]/page.tsx`

**Change:** Render correction history and allow append correction; on409 preserve reason and refresh record.

**Why:** Editing history must be visible.

**Verify immediately:** Reload matches server outcome and status.
...
**Purpose:** Prove the boundary named “Cancel” for add outcome forms to deal and shortlist views.
...
**Setup:** Open loss dialog.

**Action:** Cancel.

**Expected result:** Deal remains open; no event.
...
**Why this test matters:** Unknown is a valid answer. Requiring a guess would pollute later product analytics and model evaluation. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Avoid status update on modal open.

### Test 2 — Unknown

**Purpose:** Prove the boundary named “Unknown” for add outcome forms to deal and shortlist views.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Choose unknown.

**Action:** Save.

**Expected result:** Unknown displayed verbatim after reload.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Unknown is a valid answer. Requiring a guess would pollute later product analytics and model evaluation. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** No inferred reason.

### Test 3 — Conflict

**Purpose:** Prove the boundary named “Conflict” for add outcome forms to deal and shortlist views.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Record version changes.

**Action:** Save old dialog.

**Expected result:** 409, edits retained and no false closed badge.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Unknown is a valid answer. Requiring a guess would pollute later product analytics and model evaluation. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Handle error before cache mutation.
...
npm --prefix Frontend test -- e2e/outcomes.spec.ts --project=chromium
```
...
- [ ] One transaction response confirms both changes.
- [ ] Reload matches server outcome and status.
