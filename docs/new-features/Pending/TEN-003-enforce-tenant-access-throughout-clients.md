# TEN-003 — Enforce tenant access throughout clients
...
Enforce tenant access throughout clients. A Client and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contacts, requirements, shortlists inherit parent access.

## 2. Current behavior and evidence

clients routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.
...
A Client and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contacts, requirements, shortlists inherit parent access.
...
Chosen approach: A Client and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contacts, requirements, shortlists inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/test/clients-tenant.e2e-spec.ts`                       | Proposed — create here or consume dependency-created file |
...
**Blocks:** [PROP-001](../Pending/PROP-001-make-proposal-creator-and-geography-access-explicit.md), [DATA-001](../Pending/DATA-001-exclude-archived-clients-from-normal-reads.md), [DATA-006](../Pending/DATA-006-enforce-shortlist-entity-relationships.md), [REQ-002](../Pending/REQ-002-expose-atomic-brief-read-and-replacement-endpoints.md), [AI-003](../Pending/AI-003-accept-text-intake-with-atomic-quotas-and-idempotency.md), [CRM-002](../Pending/CRM-002-record-authorized-interaction-history-and-corrections.md), [CRM-004](../Pending/CRM-004-assign-leads-with-explicit-response-deadlines.md)
...
**Change:** Conjoin organization and existing role/geography restrictions for list/count/detail through client. Resolve each supplied parent before create or reparent.
...
**Change:** Authorize contacts, requirements, shortlists via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/clients-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout clients.
...
**Why this test matters:** A Client and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contacts, requirements, shortlists inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout clients.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout clients.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/clients-tenant.e2e-spec.ts
```
