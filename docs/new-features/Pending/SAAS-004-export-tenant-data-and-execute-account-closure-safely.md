# SAAS-004 — Export tenant data and execute account closure safely
...
Export tenant data and execute account closure safely. Closure is a durable sequence with an auditable manifest. Deleting master rows alone leaves private derived data behind.

## 2. Current behavior and evidence

Media/source retention and soft archive policies need a coordinated organization closure process.
...
Closure is a durable sequence with an auditable manifest. Deleting master rows alone leaves private derived data behind.
...
Chosen approach: Closure is a durable sequence with an auditable manifest. Deleting master rows alone leaves private derived data behind. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915002500_account_lifecycle/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/organizations/data-lifecycle.service.ts`              | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/organizations/data-lifecycle.worker.ts`               | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/organizations/organizations.controller.ts`            | Proposed — create here or consume dependency-created file |
| `Backend/test/data-lifecycle.integration.e2e-spec.ts`                      | Proposed — create here or consume dependency-created file |
...
**Depends On:** [DEC-005](../Pending/DEC-005-approve-sharing-retention-and-customer-data-use.md), [SAAS-001](../Pending/SAAS-001-invite-and-offboard-organization-members-explicitly.md), [OPS-003](../Pending/OPS-003-dispatch-outbox-events-with-retry-safe-claims.md), [MEDIA-005](../Pending/MEDIA-005-clean-expired-pending-uploads-without-deleting-valid-files.md), [AI-001](../Pending/AI-001-create-private-source-job-and-review-persistence.md)
...
Wire worker/service through organizations.module.ts. Production closure is not authorized by completing the code ticket; run only on synthetic fixtures until an actual authorized closure request.
...
**Change:** Add AccountLifecycleJob(id, organizationId, type export|closure, status, phase, manifestJson, errorCode, requestedBy, confirmedAt, createdAt). POST /organizations/current/data-export ADMIN creates bounded private archive job; include domain CSV/JSON, source manifest, job/review history and explicit exclusions under DEC-005.

**Why:** Export promises must be measurable against database counts.

**Verify immediately:** No other tenant records; download grant expires15min.
...
**Where:** `Backend/src/modules/organizations/data-lifecycle.service.ts`, `Backend/src/modules/organizations/data-lifecycle.worker.ts`, `Backend/src/modules/organizations/organizations.controller.ts`

**Change:** Closure request requires explicit organization-name confirmation and DEC-005 approved delay/retention policy. Immediately stop new jobs/invitations/share access; worker phases revoke, purge derived indexes/cache, delete blobs, delete/anonymize records per policy, verify.

**Why:** A failed phase must not restore old access or lose progress.

**Verify immediately:** Each phase checkpoint repeat-safe; legal/retention hold blocks relevant purge explicitly.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/organizations/data-lifecycle.service.ts`

**Change:** Record counts/hashes and residual scan; expire exports and backup retention according to approved provider runbook. Never claim backups instantly deleted without evidence.

**Why:** Operational retention must match the policy users accepted.

**Verify immediately:** Completion requires reconciled residual report; failed cleanup stays failed/retryable.
...
### Test 1 — Foreign export

**Purpose:** Prove the boundary named “Foreign export” for export tenant data and execute account closure safely.
...
**Setup:** Two tenants with similar IDs.

**Action:** ExportA.

**Expected result:** OnlyA content/counts.
...
**Why this test matters:** Closure is a durable sequence with an auditable manifest. Deleting master rows alone leaves private derived data behind. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Scope every included table and object prefix.

### Test 2 — Worker crash

**Purpose:** Prove the boundary named “Worker crash” for export tenant data and execute account closure safely.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Crash after blob purge checkpoint.

**Action:** Resume.

**Expected result:** No duplicate destructive phase; proceeds to remaining work.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Closure is a durable sequence with an auditable manifest. Deleting master rows alone leaves private derived data behind. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Persist phase completion before next phase.

### Test 3 — Derived residue

**Purpose:** Prove the boundary named “Derived residue” for export tenant data and execute account closure safely.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Source chunk remains after primary deletion.

**Action:** Verify closure.

**Expected result:** Job fails verification until chunk purged.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Closure is a durable sequence with an auditable manifest. Deleting master rows alone leaves private derived data behind. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Enumerate derived stores, not only Prisma tables.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/data-lifecycle.integration.e2e-spec.ts
...
- [ ] No other tenant records; download grant expires15min.
- [ ] Each phase checkpoint repeat-safe; legal/retention hold blocks relevant purge explicitly.
- [ ] Completion requires reconciled residual report; failed cleanup stays failed/retryable.
