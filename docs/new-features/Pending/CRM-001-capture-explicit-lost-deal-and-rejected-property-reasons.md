# CRM-001 — Capture explicit lost deal and rejected property reasons
...
Capture explicit lost deal and rejected property reasons. An outcome describes what the person reported. It must not silently label a customer or manufacture training data.

## 2. Current behavior and evidence

Deal stages and shortlist status exist, but reliable structured reasons and immutable correction history are not established.
...
An outcome describes what the person reported. It must not silently label a customer or manufacture training data.
...
Chosen approach: An outcome describes what the person reported. It must not silently label a customer or manufacture training data. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915001400_outcome_events/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/deals/outcome.service.ts`                          | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/deals/deals.controller.ts`                         | Existing — inspect before editing                         |
| `Backend/src/modules/deals/__tests__/outcome.spec.ts`                   | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-007](../Pending/TEN-007-enforce-tenant-access-throughout-deals.md), [DATA-002](../Pending/DATA-002-exclude-archived-deals-from-normal-reads.md), [DATA-006](../Pending/DATA-006-enforce-shortlist-entity-relationships.md)

**Blocks:** [CRM-007](../Pending/CRM-007-add-outcome-forms-to-deal-and-shortlist-views.md), [AI-025](../Pending/AI-025-run-a-rules-baseline-for-deal-prioritization.md)
...
Wire service via deals.module.ts; shortlist route uses a dedicated OutcomeController in this module. UI is CRM-007.
...
**Change:** Add OutcomeEvent(id, organizationId, dealId nullable, shortlistId nullable, reason enum price, location, size, timing, condition, competition, other, unknown, note nullable, actorId, clientEventId, createdAt, supersedesId nullable); exactly one target, unique organizationId/clientEventId.

**Why:** Structured outcomes need provenance and correction.

**Verify immediately:** Other requires note; unknown is valid and excluded from predictive labels.
...
**Where:** `Backend/src/modules/deals/outcome.service.ts`, `Backend/src/modules/deals/deals.controller.ts`

**Change:** POST /deals/:id/outcomes and /shortlists/:id/outcomes accept reason, note<=1000, clientEventId, expectedUpdatedAt; close/lost or reject mutation and outcome in one transaction.

**Why:** Status without reason history cannot be reconciled.

**Verify immediately:** Stale target409; cross-client shortlist404.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/deals/outcome.service.ts`

**Change:** Return current outcome plus append-only history; correction references prior event and does not delete it.

**Why:** Changing the explanation must not erase historical evidence.

**Verify immediately:** No model receives notes until a separately approved evaluation dataset export.
...
**Purpose:** Prove the boundary named “Stale close” for capture explicit lost deal and rejected property reasons.
...
**Setup:** Another user advances deal.

**Action:** Submit old expectedUpdatedAt.

**Expected result:** 409; no close/outcome.
...
**Why this test matters:** An outcome describes what the person reported. It must not silently label a customer or manufacture training data. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Guard both writes in transaction.

### Test 2 — Unknown reason

**Purpose:** Prove the boundary named “Unknown reason” for capture explicit lost deal and rejected property reasons.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** User explicitly selects unknown.

**Action:** Record lost deal.

**Expected result:** Stored unknown, not price/default.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An outcome describes what the person reported. It must not silently label a customer or manufacture training data. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Avoid default categorical labels.

### Test 3 — Correction

**Purpose:** Prove the boundary named “Correction” for capture explicit lost deal and rejected property reasons.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Outcome price corrected to timing.

**Action:** Append correction.

**Expected result:** Both events retained; current reason timing.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An outcome describes what the person reported. It must not silently label a customer or manufacture training data. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Never update historical label in place.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/deals/__tests__/outcome.spec.ts
...
- [ ] Other requires note; unknown is valid and excluded from predictive labels.
- [ ] Stale target409; cross-client shortlist404.
- [ ] No model receives notes until a separately approved evaluation dataset export.
