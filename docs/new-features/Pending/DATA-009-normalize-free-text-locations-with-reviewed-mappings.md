# DATA-009 — Normalize free text locations with reviewed mappings
...
Normalize free text locations with reviewed mappings. Capture may be permissive; matching needs canonical references. Ambiguous names remain unresolved instead of being assigned to the first city with that name.

## 2. Current behavior and evidence

Building stores cityName/localityName beside canonical cityId/localityId; current intake deliberately accepts free text.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/reference/reference.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/reference/reference.service.ts) — exists at the baseline commit.
...
Capture may be permissive; matching needs canonical references. Ambiguous names remain unresolved instead of being assigned to the first city with that name.
...
Chosen approach: Capture may be permissive; matching needs canonical references. Ambiguous names remain unresolved instead of being assigned to the first city with that name. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                 | Baseline state                                            |
| ---------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/reference/reference.service.ts` | Existing — inspect before editing                         |
| `Backend/src/modules/buildings/buildings.service.ts` | Existing — inspect before editing                         |
| `Backend/scripts/normalize-locations.ts`             | Proposed — create here or consume dependency-created file |
| `Backend/test/location-normalization.e2e-spec.ts`    | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md)

**Blocks:** [INV-003](../Pending/INV-003-preview-duplicate-inventory-candidates-without-merging.md), [AI-024](../Pending/AI-024-validate-permissioned-comparable-data-before-summaries.md)
...
**Where:** `Backend/src/modules/reference/reference.service.ts`

**Change:** Produce a dry-run mapping of free text to existing city/locality IDs within the recorded state/city context.

**Why:** Locality names are not globally unique.

**Verify immediately:** Ambiguous and no-match rows are counted separately.
...
**Change:** Require approved mapping entries {buildingId, expectedUpdatedAt, cityId, localityId}; apply only internally consistent parent relationships.

**Why:** Human mapping is the authority, not string similarity.

**Verify immediately:** Conflicts remain unchanged; original free text is retained.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/scripts/normalize-locations.ts`

**Change:** Return canonical labels and unresolved flags without replacing intake text; repeat apply is idempotent.

**Why:** Existing intake work must remain usable.

**Verify immediately:** Reload displays accepted canonical location and original provenance.
...
### Test 1 — Ambiguous city

**Purpose:** Prove the boundary named “Ambiguous city” for normalize free text locations with reviewed mappings.
...
**Setup:** Two cities have same name in different states.

**Action:** Run mapping dry-run.

**Expected result:** No automatic mapping; row flagged ambiguous; zero writes.
...
**Why this test matters:** Capture may be permissive; matching needs canonical references. Ambiguous names remain unresolved instead of being assigned to the first city with that name. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check parent filters and normalization collisions.

### Test 2 — Stale approved mapping

**Purpose:** Prove the boundary named “Stale approved mapping” for normalize free text locations with reviewed mappings.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Building updated after dry-run.

**Action:** Apply reviewed mapping.

**Expected result:** 409-style conflict report and no overwrite.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Capture may be permissive; matching needs canonical references. Ambiguous names remain unresolved instead of being assigned to the first city with that name. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check expectedUpdatedAt predicate.

### Test 3 — Replay mapping

**Purpose:** Prove the boundary named “Replay mapping” for normalize free text locations with reviewed mappings.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Already mapped row and original free text.

**Action:** Apply twice.

**Expected result:** Same IDs; original text retained; no duplicate references.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Capture may be permissive; matching needs canonical references. Ambiguous names remain unresolved instead of being assigned to the first city with that name. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check no-op branch.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/location-normalization.e2e-spec.ts
```
...
- [ ] Ambiguous and no-match rows are counted separately.
- [ ] Conflicts remain unchanged; original free text is retained.
- [ ] Reload displays accepted canonical location and original provenance.
