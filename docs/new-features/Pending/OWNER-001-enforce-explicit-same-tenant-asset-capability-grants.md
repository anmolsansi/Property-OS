# OWNER-001 — Enforce explicit same tenant asset capability grants
...
Enforce explicit same tenant asset capability grants. An asset grant narrows an authenticated person’s allowed actions on one asset. It never widens the organization boundary.

## 2. Current behavior and evidence

Organization and geography guards do not express owner lease capabilities.
...
An asset grant narrows an authenticated person’s allowed actions on one asset. It never widens the organization boundary.
...
Chosen approach: An asset grant narrows an authenticated person’s allowed actions on one asset. It never widens the organization boundary. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915002000_asset_grants/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/asset-grants/asset-grants.service.ts`            | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/asset-grants/asset-grants.controller.ts`         | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/asset-grants/asset-grants.module.ts`             | Proposed — create here or consume dependency-created file |
| `Backend/src/app.module.ts`                                           | Existing — inspect before editing                         |
| `Backend/test/asset-grants.e2e-spec.ts`                               | Proposed — create here or consume dependency-created file |
...
**Depends On:** [DEC-007](../Pending/DEC-007-approve-owner-roles-and-asset-grant-policy.md), [TEN-001](../Pending/TEN-001-create-an-explicit-server-derived-access-context.md), [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md), [AUTH-006](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/docs/tickets/Pending/AUTH-006-enforce-clerk-id-identity-mapping.md)

**Blocks:** [LEASE-002](../Pending/LEASE-002-review-and-activate-lease-versions-atomically.md), [OWNER-002](../Pending/OWNER-002-let-granted-owners-propose-availability-corrections.md), [OWNER-003](../Pending/OWNER-003-display-lease-register-obligations-and-occupancy.md), [AI-017](../Pending/AI-017-extract-lease-clauses-into-evidence-linked-drafts.md), [FIN-002](../Pending/FIN-002-export-reconciled-rent-rolls-with-source-identities.md), [SHARE-002](../Pending/SHARE-002-design-cross-organization-broker-owner-exchange-contracts.md), [AI-020](../Pending/AI-020-index-authorized-source-chunks-for-cited-retrieval.md)
...
**Change:** Add AssetGrant(id, organizationId, buildingId, userId, capabilities Json, expiresAt, revokedAt, version, createdBy); unique active buildingId/userId partial index. Capabilities exactly DEC-007 allowlist.

**Why:** No free-form role string should become authority.

**Verify immediately:** Unknown capability rejected; unit access resolves granted building.
...
**Where:** `Backend/src/modules/asset-grants/asset-grants.service.ts`, `Backend/src/modules/asset-grants/asset-grants.controller.ts`

**Change:** POST /buildings/:id/asset-grants {userId, capabilities, expiresAt}, DELETE /asset-grants/:id; ADMIN issuer in same tenant only. Max365-day lifetime.

**Why:** Invited users cannot self-grant private lease access.

**Verify immediately:** Inactive/foreign user404; no grant created.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/asset-grants/asset-grants.service.ts`

**Change:** Implement requireCapability(ctx, buildingId, capability) reloaded at every write/read/job; revocation increments version and emits audit/outbox invalidation.

**Why:** Stored links/jobs must not freeze old access.

**Verify immediately:** Expired/revoked grants fail before loading lease/source payload.
...
### Test 1 — Foreign invite

**Purpose:** Prove the boundary named “Foreign invite” for enforce explicit same tenant asset capability grants.
...
**Setup:** OrgAadmin targets OrgBuser.

**Action:** Create grant.

**Expected result:** 404; no grant.
...
**Why this test matters:** An asset grant narrows an authenticated person’s allowed actions on one asset. It never widens the organization boundary. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not equate email match with membership.

### Test 2 — Revoked job

**Purpose:** Prove the boundary named “Revoked job” for enforce explicit same tenant asset capability grants.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Lease job queued then grant revoked.

**Action:** Execute.

**Expected result:** Denied; no provider payload.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An asset grant narrows an authenticated person’s allowed actions on one asset. It never widens the organization boundary. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Recheck capability at execution.

### Test 3 — Narrow capability

**Purpose:** Prove the boundary named “Narrow capability” for enforce explicit same tenant asset capability grants.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** User only view_availability.

**Action:** Read lease.

**Expected result:** 403/404 per resource policy; no terms.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An asset grant narrows an authenticated person’s allowed actions on one asset. It never widens the organization boundary. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** No implied broad owner role.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/asset-grants.e2e-spec.ts
...
- [ ] Unknown capability rejected; unit access resolves granted building.
- [ ] Inactive/foreign user404; no grant created.
- [ ] Expired/revoked grants fail before loading lease/source payload.
