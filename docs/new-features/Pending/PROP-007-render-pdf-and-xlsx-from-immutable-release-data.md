# PROP-007 — Render PDF and XLSX from immutable release data

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Proposal rendering  
**Assessment:** D08, D09, A03  
...
Render PDF and XLSX from immutable release data. Rendering is a pure presentation transformation of approved data. It must not independently query current rents or unrestricted media.

## 2. Current behavior and evidence

Legacy PDF renderer ends an empty document; live XLSX has presentation work that should be preserved where compatible.
...
Rendering is a pure presentation transformation of approved data. It must not independently query current rents or unrestricted media.
...
Chosen approach: Rendering is a pure presentation transformation of approved data. It must not independently query current rents or unrestricted media. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                               | Baseline state                                            |
| ------------------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/proposals/release-renderer.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/proposals/__tests__/release-renderer.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Blocks:** [PROP-008](../Pending/PROP-008-generate-artifact-jobs-without-changing-deal-status.md)
...
**Where:** `Backend/src/modules/proposals/release-renderer.service.ts`

**Change:** Implement render(release, format, authorizedMediaLoader) using existing PDFKit/ExcelJS dependencies and a pinned templateVersion1. Include title, date, unit identity, selected values and source-verification caveats.

**Why:** Reuse the stack and preserve a reviewable disclosure contract.

**Verify immediately:** PDF has real text/pages; XLSX cells match snapshot.
...
**Where:** `Backend/src/modules/proposals/__tests__/release-renderer.spec.ts`

**Change:** Limit50 items,10 images/item,10MiB fetched media/image,5s per fetch and60s total; image failures render labeled placeholders without invented images.

**Why:** A large photo must not exhaust memory or stall export indefinitely.

**Verify immediately:** Oversized/timeout inputs produce bounded outcomes.
...
**Change:** Distinguish zero/null using explicit checks; escape formula-like user text as string cells; do not alter approved selected main-media ordering.

**Why:** Ground-floor0 and financial0 are valid; CSV/Excel content is untrusted.

**Verify immediately:** Images/zero values correct; no active formula injection.
...
**Purpose:** Prove the boundary named “Nonempty PDF” for render pdf and xlsx from immutable release data.
...
**Setup:** Two-item public snapshot.

**Action:** Render PDF.

**Expected result:** Extracted text contains expected items and values; readable page count.
...
**Why this test matters:** Rendering is a pure presentation transformation of approved data. It must not independently query current rents or unrestricted media. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not merely assert Buffer.length.

### Test 2 — Image failure

**Purpose:** Prove the boundary named “Image failure” for render pdf and xlsx from immutable release data.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** One image times out/oversized.

**Action:** Render both formats.

**Expected result:** Bounded duration and placeholder; no leaked private URL.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Rendering is a pure presentation transformation of approved data. It must not independently query current rents or unrestricted media. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Enforce streaming byte limit and abort.

### Test 3 — Snapshot exactness

**Purpose:** Prove the boundary named “Snapshot exactness” for render pdf and xlsx from immutable release data.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Zero floor/rent and text beginning=.

**Action:** Render XLSX.

**Expected result:** Zero preserved; text is not formula; no live price query.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Rendering is a pure presentation transformation of approved data. It must not independently query current rents or unrestricted media. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use nullish checks and text cell type.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/proposals/__tests__/release-renderer.spec.ts
```
...
- [ ] PDF has real text/pages; XLSX cells match snapshot.
- [ ] Oversized/timeout inputs produce bounded outcomes.
- [ ] Images/zero values correct; no active formula injection.
