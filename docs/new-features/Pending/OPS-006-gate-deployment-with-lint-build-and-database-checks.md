# OPS-006 — Gate deployment with lint build and database checks
...
Gate deployment with lint build and database checks. A green local command is evidence about that command. Deployment must prove the exact candidate commit passed required hosted checks.

## 2. Current behavior and evidence

Historical lint failed and CI lacked full build/browser/migration acceptance. Hosted branch protection and Render gating were not verified.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [.github/workflows/ci.yml](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/.github/workflows/ci.yml) — exists at the baseline commit.
- [Backend/package.json](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/package.json) — exists at the baseline commit.
- [Frontend/package.json](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Frontend/package.json) — exists at the baseline commit.
...
A green local command is evidence about that command. Deployment must prove the exact candidate commit passed required hosted checks.
...
Chosen approach: A green local command is evidence about that command. Deployment must prove the exact candidate commit passed required hosted checks. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                          | Baseline state                                            |
| --------------------------------------------- | --------------------------------------------------------- |
| `.github/workflows/ci.yml`                    | Existing — inspect before editing                         |
| `Backend/package.json`                        | Existing — inspect before editing                         |
| `Frontend/package.json`                       | Existing — inspect before editing                         |
| `render.yaml`                                 | Existing — inspect before editing                         |
| `docs/new-features/runbooks/release-gates.md` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [OPS-005](../Pending/OPS-005-replace-superficial-search-and-proposal-acceptance-checks.md)
...
This ticket owns CI/configuration only. Any current source lint repair outside listed files must receive a small child ticket with exact diagnostics before edit; do not perform a blanket refactor. Hosted changes are a separate recorded operational step.
...
**Change:** Run current lint and record exact errors; fix each actual error in a bounded linked repair before enabling fail gate. Do not disable rules or turn errors into warnings.

**Why:** Historical counts cannot identify current failing lines.

**Verify immediately:** Attach new file/line diagnostics; separate preexisting warnings.
...
**Change:** Add backend/frontend typecheck+build, lint, DB migrate from empty and seeded prior schema, unit tests and OPS-005 browser/API job. Use test services and least-privilege permissions.

**Why:** Schema compatibility and runtime journeys require different checks.

**Verify immediately:** Failed required job blocks release of candidate SHA.
...
**Change:** Document required GitHub checks and verify hosted Render/GitHub settings with authorized read access; deploy exact verified commit with health check and rollback to last healthy artifact.

**Why:** A YAML file alone does not configure branch protection.

**Verify immediately:** Report hosted setting evidence or explicit external blocker; no fake deployed status.
...
### Test 1 — Broken migration

**Purpose:** Prove the boundary named “Broken migration” for gate deployment with lint build and database checks.
...
**Setup:** Invalid FK fixture migration.

**Action:** CI.

**Expected result:** Migration job fails; no release.
...
**Why this test matters:** A green local command is evidence about that command. Deployment must prove the exact candidate commit passed required hosted checks. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Ensure job required by release dependency.

### Test 2 — Lint regression

**Purpose:** Prove the boundary named “Lint regression” for gate deployment with lint build and database checks.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Introduce known error in disposable fixture branch.

**Action:** CI.

**Expected result:** Lint gate fails.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A green local command is evidence about that command. Deployment must prove the exact candidate commit passed required hosted checks. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not tolerate exit failure.

### Test 3 — Wrong SHA

**Purpose:** Prove the boundary named “Wrong SHA” for gate deployment with lint build and database checks.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Deployment candidate differs from tested head.

**Action:** Evaluate release gate.

**Expected result:** Blocked until sameSHA checks.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A green local command is evidence about that command. Deployment must prove the exact candidate commit passed required hosted checks. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Tie artifact to commit metadata.
...
- [ ] Attach new file/line diagnostics; separate preexisting warnings.
- [ ] Failed required job blocks release of candidate SHA.
- [ ] Report hosted setting evidence or explicit external blocker; no fake deployed status.
