# REQ-002 — Expose atomic brief read and replacement endpoints

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Requirements API  
**Assessment:** P04  
...
Expose atomic brief read and replacement endpoints. Version check, parent authorization, brief replacement and version increment form one operation.

## 2. Current behavior and evidence

Current requirements are nested under clients; REQ-001 supplies a new normalized/versioned brief schema.
...
Version check, parent authorization, brief replacement and version increment form one operation.
...
Chosen approach: Version check, parent authorization, brief replacement and version increment form one operation. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/src/modules/requirements/requirements.service.ts`                | Proposed — create here or consume dependency-created file |
| `Backend/src/app.module.ts`                                               | Existing — inspect before editing                         |
| `Backend/src/modules/requirements/__tests__/requirements.service.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [REQ-001](../Pending/REQ-001-persist-a-versioned-commercial-requirement-brief.md), [TEN-003](../Pending/TEN-003-enforce-tenant-access-throughout-clients.md)

**Blocks:** [REQ-003](../Pending/REQ-003-build-a-requirement-editor-that-preserves-unknown-values.md), [MATCH-001](../Pending/MATCH-001-persist-and-compute-authorized-match-runs.md), [AI-003](../Pending/AI-003-accept-text-intake-with-atomic-quotas-and-idempotency.md), [AI-005](../Pending/AI-005-apply-reviewed-requirement-drafts-exactly-once.md), [TYPE-001](../Pending/TYPE-001-validate-transport-contracts-at-risky-frontend-boundaries.md)
...
**Where:** `Backend/src/modules/requirements/requirements.controller.ts`, `Backend/src/modules/requirements/requirements.service.ts`

**Change:** Register RequirementsModule and implement GET /requirements/:id/brief ->200 {id, version, brief, status} using client ownership.

**Why:** A dedicated read contract avoids frontend guessing from client adapters.

**Verify immediately:** Foreign requirement404; no client list scan.
...
**Change:** Implement PUT /requirements/:id/brief with expectedVersion and the complete validated brief; use a transaction for scalar/locality updates and version+1.

**Why:** Replacement avoids leftover locality memberships.

**Verify immediately:** Concurrent PUTs with same version yield one200 and one409.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/requirements/requirements.service.ts`

**Change:** Preserve existing createRequirement and status transitions; expose schema-backed manual create through existing client route.

**Why:** Do not replace unrelated clients API behavior.

**Verify immediately:** New and legacy creates remain distinguishable with explicit basis fields.
...
### Test 1 — Concurrent replacement

**Purpose:** Prove the boundary named “Concurrent replacement” for expose atomic brief read and replacement endpoints.
...
**Setup:** Requirement version1.

**Action:** Two PUTs expectedVersion1.

**Expected result:** One version2; stale request409 with no partial localities.
...
**Why this test matters:** Version check, parent authorization, brief replacement and version increment form one operation. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check compare-and-swap transaction.

### Test 2 — Unknown parent

**Purpose:** Prove the boundary named “Unknown parent” for expose atomic brief read and replacement endpoints.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Foreign client owns requirement.

**Action:** GET/PUT brief.

**Expected result:** 404; no update.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Version check, parent authorization, brief replacement and version increment form one operation. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Authorize through requirement.client.

### Test 3 — Clearing a field

**Purpose:** Prove the boundary named “Clearing a field” for expose atomic brief read and replacement endpoints.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Versioned brief had parkingSpaces4.

**Action:** PUT full brief with parking absent.

**Expected result:** Stored null, not stale4; version increments once.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Version check, parent authorization, brief replacement and version increment form one operation. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not spread partial fields for replacement.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/requirements/__tests__/requirements.service.spec.ts
```
...
- [ ] Foreign requirement404; no client list scan.
- [ ] Concurrent PUTs with same version yield one200 and one409.
- [ ] New and legacy creates remain distinguishable with explicit basis fields.
