# CRM-006 — Show timelines visit calendar and saved searches

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Broker workspace UI  
**Assessment:** P08, P10, P11, P14  
...
Show timelines visit calendar and saved searches. A workspace displays server-owned workflow state and preserves user edits when concurrent changes occur.

## 2. Current behavior and evidence

Backend workflow slices must land first; the current client page is the integration point.
...
A workspace displays server-owned workflow state and preserves user edits when concurrent changes occur.
...
Chosen approach: A workspace displays server-owned workflow state and preserves user edits when concurrent changes occur. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                      | Baseline state                                            |
| --------------------------------------------------------- | --------------------------------------------------------- |
| `Frontend/src/hooks/use-client-workflows.ts`              | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/clients/InteractionTimeline.tsx` | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/clients/VisitCalendar.tsx`       | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/search/SavedSearchControls.tsx`  | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(v2)/clients/[id]/page.tsx`             | Existing — inspect before editing                         |
| `Frontend/e2e/client-workflows.spec.ts`                   | Proposed — create here or consume dependency-created file |
...
**Depends On:** [CRM-002](../Pending/CRM-002-record-authorized-interaction-history-and-corrections.md), [CRM-003](../Pending/CRM-003-schedule-visits-with-conflict-and-reminder-semantics.md), [CRM-004](../Pending/CRM-004-assign-leads-with-explicit-response-deadlines.md), [CRM-005](../Pending/CRM-005-save-and-evaluate-access-scoped-searches.md)
...
**Where:** `Frontend/src/hooks/use-client-workflows.ts`

**Change:** Create typed hooks for CRM-002/003/004/005 and mount client timeline/calendar plus saved-search controls on client detail. Display server deadline and timezone explicitly.

**Why:** The frontend must not independently calculate ownership/deadline truth.

**Verify immediately:** Reload shows same persisted state.
...
**Where:** `Frontend/src/components/clients/InteractionTimeline.tsx`

**Change:** Provide note/correction, schedule/reschedule/cancel, and disable search forms with pending locks and409 reload preserving input.

**Why:** A failed request should not erase a carefully entered note.

**Verify immediately:** No optimistic completed/sent state.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/components/clients/VisitCalendar.tsx`

**Change:** Render empty, loading, offline,403/404 and terminal errors; clear queries after access loss; dates use stored display timezone.

**Why:** Stale cached private data must leave the screen after denial.

**Verify immediately:** Keyboard navigation and accessible field errors pass browser checks.
...
**Purpose:** Prove the boundary named “Conflict” for show timelines visit calendar and saved searches.
...
**Setup:** Schedule POST409.

**Action:** Submit edited interval.

**Expected result:** Input retained; visible conflict, no calendar success.
...
**Why this test matters:** A workspace displays server-owned workflow state and preserves user edits when concurrent changes occur. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate optimistic draft from confirmed visit.

### Test 2 — Disable

**Purpose:** Prove the boundary named “Disable” for show timelines visit calendar and saved searches.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Saved search enabled.

**Action:** Disable then reload.

**Expected result:** Disabled server state and no pending alerts shown as active.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A workspace displays server-owned workflow state and preserves user edits when concurrent changes occur. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Invalidate relevant cache.

### Test 3 — Correction

**Purpose:** Prove the boundary named “Correction” for show timelines visit calendar and saved searches.
...
**Expected result:** Original and correction visible after reload.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A workspace displays server-owned workflow state and preserves user edits when concurrent changes occur. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not replace timeline entry locally.
...
npm --prefix Frontend test -- e2e/client-workflows.spec.ts --project=chromium
```
...
- [ ] Reload shows same persisted state.
- [ ] No optimistic completed/sent state.
- [ ] Keyboard navigation and accessible field errors pass browser checks.
