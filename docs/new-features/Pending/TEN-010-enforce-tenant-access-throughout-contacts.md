# TEN-010 — Enforce tenant access throughout contacts
...
Enforce tenant access throughout contacts. A Contact and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contact details and reveal actions inherit parent access.

## 2. Current behavior and evidence

contacts routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/contacts/contacts.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/contacts/contacts.controller.ts) — exists at the baseline commit.
- [Backend/src/modules/contacts/contacts.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/contacts/contacts.service.ts) — exists at the baseline commit.
- [Backend/src/modules/contacts/**tests**/contacts.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/contacts/__tests__/contacts.service.spec.ts) — exists at the baseline commit.
...
A Contact and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contact details and reveal actions inherit parent access.
...
Chosen approach: A Contact and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contact details and reveal actions inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                              | Baseline state                                            |
| ----------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/contacts/contacts.controller.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/contacts/contacts.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/contacts/__tests__/contacts.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/contacts-tenant.e2e-spec.ts`                        | Proposed — create here or consume dependency-created file |
...
**Where:** `Backend/src/modules/contacts/contacts.controller.ts`
...
**Where:** `Backend/src/modules/contacts/contacts.service.ts`

**Change:** Conjoin organization and existing role/geography restrictions for list/count/detail through building/floor/unit. Resolve each supplied parent before create or reparent.
...
**Change:** Authorize contact details and reveal actions via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/contacts-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout contacts.
...
**Why this test matters:** A Contact and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. contact details and reveal actions inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout contacts.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout contacts.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/contacts/__tests__/contacts.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/contacts-tenant.e2e-spec.ts
```
