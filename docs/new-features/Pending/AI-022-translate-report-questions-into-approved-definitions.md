# AI-022 — Translate report questions into approved definitions
...
Translate report questions into approved definitions. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

REPORT-001 supplies trusted computations; natural-language input needs an explicit confirmation step.
...
| File                                                     | Baseline state                                            |
| -------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/report-intent.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/report-intent.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/report-intent.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [REPORT-001](../Pending/REPORT-001-compute-allowlisted-portfolio-reports.md), [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md)
...
Command API: POST /api/v1/ai/report-intent with Idempotency-Key; input {question:string<=2000}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {reportRequest:ReportRequest|null, questions:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it. Use REPORT-v1 in full from the shared contract in this ticket; the model never receives database credentials.
...
**Where:** `Backend/src/modules/ai/report-intent.service.ts`

**Change:** Map question only to REPORT-v1 enum and bounded filters; ambiguous date/metric asks clarification.

**Why:** Natural language is not an SQL permission.

**Verify immediately:** Provider output containing SQL/unknown key rejected.
...
**Where:** `Backend/src/modules/ai/dto/report-intent.schema.ts`

**Change:** Return proposed report definition/filters for user confirmation; no report execution inside intent command.

**Why:** User should see what question will be computed.

**Verify immediately:** No invented portfolio result in draft.
...
**Change:** Confirmed UI calls REPORT-001 and renders structured totals; any narrative only summarizes result snapshot with source IDs.

**Why:** Arithmetic stays deterministic.

**Verify immediately:** Changed filter creates new report, not edited past snapshot.
...
### Test 1 — Ambiguous period

**Purpose:** Prove the boundary named “Ambiguous period” for translate report questions into approved definitions.
...
**Setup:** Question says recently.

**Action:** Parse.

**Expected result:** Clarifying question, no default hidden range.
...
**Likely failure diagnosis:** Require explicit period.

### Test 2 — Arbitrary SQL

**Purpose:** Prove the boundary named “Arbitrary SQL” for translate report questions into approved definitions.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model outputs SQL.
...
**Likely failure diagnosis:** Strict schema.

### Test 3 — Unauthorized asset

**Purpose:** Prove the boundary named “Unauthorized asset” for translate report questions into approved definitions.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model includes other tenant building.

**Action:** Confirm report.

**Expected result:** 404, no totals.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Report service rechecks access.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/report-intent.spec.ts
```
...
- [ ] Provider output containing SQL/unknown key rejected.
- [ ] No invented portfolio result in draft.
- [ ] Changed filter creates new report, not edited past snapshot.
