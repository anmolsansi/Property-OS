# PROP-005 — Persist immutable releases artifact jobs and share grants

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Proposal schema  
**Assessment:** D08, P06, P07  
...
Persist immutable releases artifact jobs and share grants. A proposal draft changes. A released proposal is the exact disclosure a client received and must remain reproducible.

## 2. Current behavior and evidence

Proposal has legacy unitIds and live ProposalItems; snapshotJson exists but exports read current entities. No release/grant/artifact-job tables exist.
...
A proposal draft changes. A released proposal is the exact disclosure a client received and must remain reproducible.
...
Chosen approach: A proposal draft changes. A released proposal is the exact disclosure a client received and must remain reproducible. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915001100_proposal_releases/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/test/proposal-release-schema.e2e-spec.ts`                         | Proposed — create here or consume dependency-created file |
...
**Depends On:** [PROP-004](../Pending/PROP-004-enforce-proposal-item-uniqueness-under-concurrency.md), [DEC-005](../Pending/DEC-005-approve-sharing-retention-and-customer-data-use.md)

**Blocks:** [PROP-006](../Pending/PROP-006-create-immutable-reviewed-proposal-releases.md)
...
**Change:** Add ProposalRelease, ArtifactJob, ShareGrant and Feedback exactly as contracts; add unique proposalId/idempotencyKey on releases with requestHash.

**Why:** A retried release must identify the same immutable version.

**Verify immediately:** Schema forbids duplicate version/key and dangling references.
...
**Where:** `Backend/prisma/schema.prisma`, `Backend/prisma/migrations/20260915001100_proposal_releases/migration.sql`

**Change:** Add restrictive deletion for releases/grants and canonical snapshot hashing; do not backfill old proposals as if their historical exports were known.

**Why:** An old mutable row cannot reconstruct a historical document.

**Verify immediately:** Existing proposals remain drafts/history-unknown until explicitly released.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/test/proposal-release-schema.e2e-spec.ts`

**Change:** Write an audit-only legacy unitIds inventory and migration note: retain legacy column until mapped to validated relational items through a separate reviewed operation.

**Why:** Do not silently discard older selected units.

**Verify immediately:** Dirty IDs reported; no invented snapshots or removed data.
...
### Test 1 — Unique release version

**Purpose:** Prove the boundary named “Unique release version” for persist immutable releases artifact jobs and share grants.
...
**Setup:** Two rows same proposal/version.

**Action:** Insert concurrently.

**Expected result:** One accepted, one unique conflict.
...
**Why this test matters:** A proposal draft changes. A released proposal is the exact disclosure a client received and must remain reproducible. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check composite constraint.

### Test 2 — Legacy unknown history

**Purpose:** Prove the boundary named “Legacy unknown history” for persist immutable releases artifact jobs and share grants.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Old proposal unitIds without snapshot.

**Action:** Migrate.

**Expected result:** No fabricated release; existing data retained.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A proposal draft changes. A released proposal is the exact disclosure a client received and must remain reproducible. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not equate updatedAt with original export time.

### Test 3 — Grant ownership

**Purpose:** Prove the boundary named “Grant ownership” for persist immutable releases artifact jobs and share grants.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Grant references absent release.

**Action:** Insert.

**Expected result:** FK failure and no orphan.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A proposal draft changes. A released proposal is the exact disclosure a client received and must remain reproducible. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check restrictive FK and transaction.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/proposal-release-schema.e2e-spec.ts
...
- [ ] Schema forbids duplicate version/key and dangling references.
- [ ] Existing proposals remain drafts/history-unknown until explicitly released.
- [ ] Dirty IDs reported; no invented snapshots or removed data.
