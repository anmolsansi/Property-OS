# DATA-010 — Commit change approval and snapshot atomically
...
Commit change approval and snapshot atomically. Approval is one semantic transition. A field cannot be approved in the queue while its master record failed to change.

## 2. Current behavior and evidence

approveItems creates a snapshot, updates change items, changes master data and writes audit in separate operations.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/change-requests/change-requests.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/change-requests/change-requests.service.ts) — exists at the baseline commit.
- [Backend/src/modules/change-requests/dto/change-requests.schema.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/change-requests/dto/change-requests.schema.ts) — exists at the baseline commit.
- [Backend/src/modules/change-requests/**tests**/change-requests.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/change-requests/__tests__/change-requests.service.spec.ts) — exists at the baseline commit.
...
Approval is one semantic transition. A field cannot be approved in the queue while its master record failed to change.
...
Chosen approach: Approval is one semantic transition. A field cannot be approved in the queue while its master record failed to change. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                            | Baseline state                                            |
| ------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/change-requests/change-requests.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/change-requests/dto/change-requests.schema.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/change-requests/__tests__/change-requests.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/approval-atomicity.e2e-spec.ts`                                   | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-001](../Pending/TEN-001-create-an-explicit-server-derived-access-context.md), [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md)

**Blocks:** [INV-002](../Pending/INV-002-expose-the-verification-queue-and-review-actions.md), [AI-011](../Pending/AI-011-apply-reviewed-inventory-drafts-through-existing-intake.md), [OWNER-002](../Pending/OWNER-002-let-granted-owners-propose-availability-corrections.md)
...
**Where:** `Backend/src/modules/change-requests/change-requests.service.ts`

**Change:** Validate every selected item belongs to the request and is pending; reject unknown IDs rather than skip them.

**Why:** Skipping invalid IDs creates misleading partial approval.

**Verify immediately:** Invalid selection performs no writes.
...
**Where:** `Backend/src/modules/change-requests/dto/change-requests.schema.ts`

**Change:** Use one transaction client for locked request/master lookup, snapshot, selected item changes, master update and semantic audit; compare current master value to submitted base values.

**Why:** Concurrent reviewers cannot both apply stale facts.

**Verify immediately:** One reviewer wins; conflicting approval returns409 and no partial audit.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/change-requests/__tests__/change-requests.service.spec.ts`

**Change:** On storage/database failure roll back all operations; emit notification only after commit through outbox when available.

**Why:** A message must not announce a rolled-back approval.

**Verify immediately:** Injected failure leaves master, review flags and snapshots unchanged.
...
### Test 1 — Mid-approval failure

**Purpose:** Prove the boundary named “Mid-approval failure” for commit change approval and snapshot atomically.
...
**Setup:** Pending request, original master values; fail master update.

**Action:** Approve selected fields.

**Expected result:** All review/snapshot/audit writes rolled back; no notification.
...
**Why this test matters:** Approval is one semantic transition. A field cannot be approved in the queue while its master record failed to change. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find queries using root Prisma instead of tx.

### Test 2 — Concurrent reviewers

**Purpose:** Prove the boundary named “Concurrent reviewers” for commit change approval and snapshot atomically.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Two approvals based on same original value.

**Action:** Run simultaneously.

**Expected result:** One result applied; stale attempt409; no lost update.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Approval is one semantic transition. A field cannot be approved in the queue while its master record failed to change. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check row locking and base-value comparison.

### Test 3 — Foreign change item

**Purpose:** Prove the boundary named “Foreign change item” for commit change approval and snapshot atomically.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** RequestA receives itemId from requestB.

**Action:** Approve.

**Expected result:** 400; neither request changes.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Approval is one semantic transition. A field cannot be approved in the queue while its master record failed to change. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not continue silently on missing selection.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/change-requests/__tests__/change-requests.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/approval-atomicity.e2e-spec.ts
```
...
- [ ] Invalid selection performs no writes.
- [ ] One reviewer wins; conflicting approval returns409 and no partial audit.
- [ ] Injected failure leaves master, review flags and snapshots unchanged.
