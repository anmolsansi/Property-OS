# DATA-003 — Exclude archived tasks from normal reads
...
Exclude archived tasks from normal reads. Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence.

## 2. Current behavior and evidence

tasks has softDelete/restore, but ordinary reads do not consistently predicate deletedAt:null.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/tasks/tasks.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/tasks/tasks.service.ts) — exists at the baseline commit.
- [Backend/src/modules/tasks/tasks.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/tasks/tasks.controller.ts) — exists at the baseline commit.
- [Backend/src/modules/tasks/**tests**/tasks.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/tasks/__tests__/tasks.service.spec.ts) — exists at the baseline commit.
...
| File                                                        | Baseline state                    |
| ----------------------------------------------------------- | --------------------------------- |
| `Backend/src/modules/tasks/tasks.service.ts`                | Existing — inspect before editing |
| `Backend/src/modules/tasks/tasks.controller.ts`             | Existing — inspect before editing |
| `Backend/src/modules/tasks/__tests__/tasks.service.spec.ts` | Existing — inspect before editing |
...
**Depends On:** [TEN-006](../Pending/TEN-006-enforce-tenant-access-throughout-tasks.md)
...
**Where:** `Backend/src/modules/tasks/tasks.service.ts`
...
**Where:** `Backend/src/modules/tasks/tasks.controller.ts`
...
**Purpose:** Prove the boundary named “Archive survives reload” for exclude archived tasks from normal reads.
...
**Purpose:** Prove the boundary named “Restore remains scoped” for exclude archived tasks from normal reads.
...
**Purpose:** Prove the boundary named “Repeat lifecycle is stable” for exclude archived tasks from normal reads.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/tasks/__tests__/tasks.service.spec.ts
```
