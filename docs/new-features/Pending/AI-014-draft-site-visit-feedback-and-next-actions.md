# AI-014 — Draft site visit feedback and next actions
...
Draft site visit feedback and next actions. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

SiteVisits store notes and status; Tasks support follow-ups. Neither links a reviewed debrief extraction command.
...
| File                                                     | Baseline state                                            |
| -------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/visit-debrief.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/visit-debrief.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/visit-debrief.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [TEN-008](../Pending/TEN-008-enforce-tenant-access-throughout-site-visits.md), [CRM-002](../Pending/CRM-002-record-authorized-interaction-history-and-corrections.md)
...
Command API: POST /api/v1/ai/visit-debrief with Idempotency-Key; input {siteVisitId:UUID, text:string<=20000}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {summary:string<=2000, outcomes:string[], proposedTasks:[{title:string<=200, dueDate:ISO8601|null, reason:string<=500}], questions:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/visit-debrief.service.ts`

**Change:** Load only authorized visit/client/building facts and submitted notes; label notes as unverified source.

**Why:** A visit can mention private client preferences.

**Verify immediately:** Foreign visit rejected before generation.
...
**Where:** `Backend/src/modules/ai/dto/visit-debrief.schema.ts`

**Change:** Extract reasons and possible actions with unresolved dates left null; never infer attendance/status from model prose.

**Why:** Task due dates and completed visits have operational consequences.

**Verify immediately:** No SiteVisit.status mutation.
...
**Change:** Return draft actions that users accept individually through normal task endpoints; attach originating job ID in audit evidence.

**Why:** Generated next steps are suggestions.

**Verify immediately:** No task appears until explicit acceptance; repeated acceptance must use one idempotency key/action.
...
### Test 1 — No explicit due date

**Purpose:** Prove the boundary named “No explicit due date” for draft site visit feedback and next actions.
...
**Setup:** Notes say follow up soon.

**Action:** Generate.

**Expected result:** dueDate=null; asks for date.
...
**Likely failure diagnosis:** Do not invent tomorrow.

### Test 2 — Cancelled visit

**Purpose:** Prove the boundary named “Cancelled visit” for draft site visit feedback and next actions.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Visit was cancelled.

**Action:** Draft from old notes.

**Expected result:** Warn stale/cancelled context; no automatic completion or reminder.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Recheck visit status.

### Test 3 — Repeated action acceptance

**Purpose:** Prove the boundary named “Repeated action acceptance” for draft site visit feedback and next actions.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same task action clicked twice.

**Action:** Accept with same action key.

**Expected result:** One task through CRM-002 command boundary.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use job/action unique key.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/visit-debrief.spec.ts
```
...
- [ ] Foreign visit rejected before generation.
- [ ] No SiteVisit.status mutation.
- [ ] No task appears until explicit acceptance; repeated acceptance must use one idempotency key/action.
