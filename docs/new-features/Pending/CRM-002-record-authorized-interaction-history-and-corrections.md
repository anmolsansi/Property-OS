# CRM-002 — Record authorized interaction history and corrections
...
Record authorized interaction history and corrections. A timeline is an append-only account of approved actions linked to the correct client, not an unfiltered activity log.

## 2. Current behavior and evidence

Notes and workflow entities exist in separate modules; no unified permissioned interaction ledger exists.
...
A timeline is an append-only account of approved actions linked to the correct client, not an unfiltered activity log.
...
Chosen approach: A timeline is an append-only account of approved actions linked to the correct client, not an unfiltered activity log. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915001500_interactions/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/interactions/interactions.service.ts`            | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/interactions/interactions.controller.ts`         | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/interactions/interactions.module.ts`             | Proposed — create here or consume dependency-created file |
| `Backend/test/interactions.e2e-spec.ts`                               | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-003](../Pending/TEN-003-enforce-tenant-access-throughout-clients.md), [TEN-007](../Pending/TEN-007-enforce-tenant-access-throughout-deals.md), [TEN-008](../Pending/TEN-008-enforce-tenant-access-throughout-site-visits.md)

**Blocks:** [AI-014](../Pending/AI-014-draft-site-visit-feedback-and-next-actions.md), [AI-015](../Pending/AI-015-draft-contextual-follow-ups-with-suppression-checks.md), [CRM-004](../Pending/CRM-004-assign-leads-with-explicit-response-deadlines.md), [CRM-006](../Pending/CRM-006-show-timelines-visit-calendar-and-saved-searches.md), [METRIC-001](../Pending/METRIC-001-measure-the-intake-to-visit-funnel-without-private-payloads.md), [WORKFLOW-001](../Pending/WORKFLOW-001-specify-versioned-broker-and-owner-workflow-templates.md), [AI-029](../Pending/AI-029-apply-reviewed-visit-and-voice-action-drafts-once.md)
...
**Change:** Create EVENTS-v1 tables and strict per-type metadata schemas; register module. Add supersedesId nullable self FK.

**Why:** Common history still needs typed event payloads.

**Verify immediately:** No arbitrary JSON body can smuggle role, tenant or contact secrets.
...
**Where:** `Backend/src/modules/interactions/interactions.service.ts`, `Backend/src/modules/interactions/interactions.controller.ts`

**Change:** Implement POST and client timeline GET with parent relationship validation and request replay digest. Bound occurredAt to no more than24h future.

**Why:** An accessible but unrelated deal must not be attached to another client.

**Verify immediately:** Inconsistent parent IDs400; inaccessible parent404; zero writes.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/interactions/interactions.service.ts`

**Change:** Add correction route POST /interactions/:id/corrections with full replacement content and new event identity, preserving immutable original.

**Why:** Corrections must be visible and attributable.

**Verify immediately:** Tenant-scoped chronological pagination remains stable.
...
### Test 1 — Parent mismatch

**Purpose:** Prove the boundary named “Parent mismatch” for record authorized interaction history and corrections.
...
**Setup:** Authorized clientA, deal belongs clientB.

**Action:** Post combined event.

**Expected result:** 400; no event.
...
**Why this test matters:** A timeline is an append-only account of approved actions linked to the correct client, not an unfiltered activity log. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Validate relationship as well as access.

### Test 2 — Duplicate timeout

**Purpose:** Prove the boundary named “Duplicate timeout” for record authorized interaction history and corrections.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same event key posted twice.

**Action:** Retry exact payload.

**Expected result:** One event; original result.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A timeline is an append-only account of approved actions linked to the correct client, not an unfiltered activity log. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Unique constraint plus payload digest.

### Test 3 — Correct history

**Purpose:** Prove the boundary named “Correct history” for record authorized interaction history and corrections.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Original note and correction.

**Action:** Read timeline.

**Expected result:** Both shown with supersedes relation; no overwritten text.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A timeline is an append-only account of approved actions linked to the correct client, not an unfiltered activity log. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Append rather than update.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/interactions.e2e-spec.ts
...
- [ ] No arbitrary JSON body can smuggle role, tenant or contact secrets.
- [ ] Inconsistent parent IDs400; inaccessible parent404; zero writes.
- [ ] Tenant-scoped chronological pagination remains stable.
