# AI-008 — Explain matching results using retrieved facts
...
Explain matching results using retrieved facts. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

MATCH-v1 produces deterministic reasons; no current LLM explanation exists.
...
| File                                                         | Baseline state                                            |
| ------------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/ai/match-explanation.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/match-explanation.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/match-explanation.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [MATCH-001](../Pending/MATCH-001-persist-and-compute-authorized-match-runs.md), [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md)
...
Command API: POST /api/v1/ai/match-explanation with Idempotency-Key; input {matchRunId:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {matchRunId:UUID, items:[{unitId:UUID, summary:string<=1000, evidence:Evidence[]}], warnings:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it. No API mutates the match score, candidate classification or order; numeric claims should be rendered from structured fields rather than free-form generated numbers.
...
**Where:** `Backend/src/modules/ai/match-explanation.service.ts`

**Change:** Load only authorized persisted match run and allowed public candidate facts; reject changed requirement.

**Why:** An explanation must not search a broader inventory.

**Verify immediately:** Provider input contains no hidden candidate/client information.
...
**Where:** `Backend/src/modules/ai/dto/match-explanation.schema.ts`

**Change:** Ask for short reasons citing existing matched/tradeoff facts; validate unit IDs and every numeric statement against run snapshot.

**Why:** A model must not upgrade a needs-verification unit to eligible.

**Verify immediately:** Classification and ordering remain byte-equivalent to input run.
...
**Change:** Return draft explanations alongside deterministic reasons; on failure retain original rule explanation.

**Why:** Provider outage should not disable matching.

**Verify immediately:** No new shortlist or property mutation.
...
### Test 1 — Invented amenity

**Purpose:** Prove the boundary named “Invented amenity” for explain matching results using retrieved facts.
...
**Setup:** Model adds parking not in run.

**Action:** Validate output.

**Expected result:** Reject draft statement; rule reasons remain.
...
**Likely failure diagnosis:** Check claim-to-fact validation.

### Test 2 — Hidden candidate

**Purpose:** Prove the boundary named “Hidden candidate” for explain matching results using retrieved facts.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Output includes unit outside run.

**Action:** Validate.

**Expected result:** INVALID_OUTPUT; no leaked candidate fetched.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use allowlisted unit IDs.

### Test 3 — Provider outage

**Purpose:** Prove the boundary named “Provider outage” for explain matching results using retrieved facts.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Match run valid, provider503.

**Action:** Request explanation.

**Expected result:** Failed draft; deterministic matches still usable.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not couple core query to AI availability.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/match-explanation.spec.ts
```
...
- [ ] Provider input contains no hidden candidate/client information.
- [ ] Classification and ordering remain byte-equivalent to input run.
- [ ] No new shortlist or property mutation.
