# PROP-001 — Make proposal creator and geography access explicit
...
Make proposal creator and geography access explicit. Proposal ownership and item permission are separate checks. A creator may edit their same-tenant empty draft, but may not acquire another geography by adding its items.

## 2. Current behavior and evidence

Proposal access expects geographicScope.userId although GeographyGuard does not provide it; denyAll can produce OR:[{}]. Detail access based on any one building can expose other items.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/proposals/proposals.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/proposals/proposals.controller.ts) — exists at the baseline commit.
...
- [Backend/src/modules/proposals/**tests**/proposals.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/proposals/__tests__/proposals.service.spec.ts) — exists at the baseline commit.
...
Proposal ownership and item permission are separate checks. A creator may edit their same-tenant empty draft, but may not acquire another geography by adding its items.
...
Chosen approach: Proposal ownership and item permission are separate checks. A creator may edit their same-tenant empty draft, but may not acquire another geography by adding its items. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                | Baseline state                                            |
| ------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/proposals/proposals.controller.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/proposals/proposals.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/proposals/__tests__/proposals.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/proposal-access.e2e-spec.ts`                          | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-001](../Pending/TEN-001-create-an-explicit-server-derived-access-context.md), [TEN-002](../Pending/TEN-002-audit-and-backfill-organization-ownership-safely.md), [TEN-003](../Pending/TEN-003-enforce-tenant-access-throughout-clients.md)

**Blocks:** [PROP-002](../Pending/PROP-002-validate-proposal-item-hierarchy-before-writes.md), [PROP-003](../Pending/PROP-003-authorize-every-proposal-artifact-export.md)
...
**Where:** `Backend/src/modules/proposals/proposals.controller.ts`

**Change:** Require AccessContext on every proposal CRUD/items handler; pass actorId explicitly instead of casting geographicScope.

**Why:** Undefined userId must never enter a Prisma OR branch.

**Verify immediately:** All actor IDs originate in verified context.
...
**Change:** Filter by organization first. ADMIN can read same-tenant proposals; WORKER reads own draft or proposals whose active items are all permitted, never an any-item widening. A zero-item non-owned draft is denied.

**Why:** A mixed proposal is one disclosure package.

**Verify immediately:** No OR:[{}]; denyAll worker sees only own empty draft metadata.
...
**Change:** On detail/getItems/update/remove authorize the proposal, then all active item parents before returning or mutating. On create authorize client and requirement ownership.

**Why:** List-only checks do not protect direct IDs.

**Verify immediately:** Forbidden item data is never serialized, even to a creator.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 4 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/test/proposal-access.e2e-spec.ts`

**Change:** Run creator/noncreator, empty, removed-only, mixed-geography and cross-tenant fixtures.

**Why:** Edge cases previously changed authorization silently.

**Verify immediately:** Consistent denial and no mutation on every route.
...
### Test 1 — Missing geography cannot become empty OR

**Purpose:** Prove the boundary named “Missing geography cannot become empty OR” for make proposal creator and geography access explicit.
...
**Setup:** No assignments; other-worker proposals exist.

**Action:** List proposals.

**Expected result:** No other-worker proposals; own empty draft allowed; no empty OR branch.
...
**Why this test matters:** Proposal ownership and item permission are separate checks. A creator may edit their same-tenant empty draft, but may not acquire another geography by adding its items. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check actor source and empty item semantics.

### Test 2 — Mixed proposal cannot disclose one hidden item

**Purpose:** Prove the boundary named “Mixed proposal cannot disclose one hidden item” for make proposal creator and geography access explicit.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Proposal has one permitted and one forbidden active building.

**Action:** Read detail/items as worker.

**Expected result:** 404 for the package; no hidden building data.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Proposal ownership and item permission are separate checks. A creator may edit their same-tenant empty draft, but may not acquire another geography by adding its items. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Replace any-item authorization with all-active-item check.

### Test 3 — Creator does not bypass tenant

**Purpose:** Prove the boundary named “Creator does not bypass tenant” for make proposal creator and geography access explicit.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Worker owns historical proposal now attached to orgB.

**Action:** Read/update as orgA context.

**Expected result:** 404 and no writes.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Proposal ownership and item permission are separate checks. A creator may edit their same-tenant empty draft, but may not acquire another geography by adding its items. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Organization predicate must precede creator branch.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/proposals/__tests__/proposals.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/proposal-access.e2e-spec.ts
```
...
- [ ] All actor IDs originate in verified context.
- [ ] No OR:[{}]; denyAll worker sees only own empty draft metadata.
- [ ] Forbidden item data is never serialized, even to a creator.
- [ ] Consistent denial and no mutation on every route.
