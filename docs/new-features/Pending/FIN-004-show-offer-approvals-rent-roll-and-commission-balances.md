# FIN-004 — Show offer approvals rent roll and commission balances
...
Show offer approvals rent roll and commission balances. The screen explains terms and balances calculated by the server and makes approval versus external acceptance explicit.

## 2. Current behavior and evidence

FIN-001/002/003 provide reviewed financial contracts; the existing UI must consume decimal strings without recreating ledger logic.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Frontend/src/app/(v2)/deals/[id]/page.tsx](<https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Frontend/src/app/(v2)/deals/[id]/page.tsx>) — exists at the baseline commit.
...
The screen explains terms and balances calculated by the server and makes approval versus external acceptance explicit.
...
Chosen approach: The screen explains terms and balances calculated by the server and makes approval versus external acceptance explicit. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                 | Baseline state                                            |
| ---------------------------------------------------- | --------------------------------------------------------- |
| `Frontend/src/hooks/use-financial-workflows.ts`      | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/leases/OfferComparison.tsx` | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(v2)/owner/leases/[id]/page.tsx`   | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/deals/CommissionLedger.tsx` | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(v2)/deals/[id]/page.tsx`          | Existing — inspect before editing                         |
| `Frontend/e2e/financial-workflows.spec.ts`           | Proposed — create here or consume dependency-created file |
...
**Depends On:** [FIN-001](../Pending/FIN-001-version-leasing-offers-and-enforce-approval-limits.md), [FIN-002](../Pending/FIN-002-export-reconciled-rent-rolls-with-source-identities.md), [FIN-003](../Pending/FIN-003-track-approved-commission-splits-and-receipt-reversals.md), [OWNER-003](../Pending/OWNER-003-display-lease-register-obligations-and-occupancy.md)
...
**Where:** `Frontend/src/hooks/use-financial-workflows.ts`

**Change:** Mount offer revision comparison and internal approval actions on lease detail; display revision/policy version and excluded components.

**Why:** Users must approve the exact compared offer.

**Verify immediately:** Changed revision invalidates approval controls until refresh.
...
**Where:** `Frontend/src/components/leases/OfferComparison.tsx`

**Change:** Add rent-roll export action with manifest totals/exclusions and commission ledger with partial receipt/reversal forms on deal detail.

**Why:** A download or paid badge alone hides reconciliation gaps.

**Verify immediately:** Unknown amounts stay unknown; server outstanding displayed.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/app/(v2)/owner/leases/[id]/page.tsx`

**Change:** Preserve draft amounts on400/409, require confirmation for approval/reversal, disable duplicate pending actions.

**Why:** Financial retries need stable intent.

**Verify immediately:** No client Number arithmetic for totals.
...
**Purpose:** Prove the boundary named “Precision” for show offer approvals rent roll and commission balances.
...
**Setup:** Amount0.10 plus0.20.

**Action:** Display server balance.

**Expected result:** 0.30 with no floating artifacts.
...
**Why this test matters:** The screen explains terms and balances calculated by the server and makes approval versus external acceptance explicit. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use formatted decimal strings.

### Test 2 — Version conflict

**Purpose:** Prove the boundary named “Version conflict” for show offer approvals rent roll and commission balances.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Offer changed while dialog open.

**Action:** Approve.

**Expected result:** 409 with no approved badge.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The screen explains terms and balances calculated by the server and makes approval versus external acceptance explicit. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Bind action to revision.

### Test 3 — Partial balance

**Purpose:** Prove the boundary named “Partial balance” for show offer approvals rent roll and commission balances.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Receipt less than entitlement.

**Action:** Reload deal.

**Expected result:** Partial balance shown, not fully paid.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The screen explains terms and balances calculated by the server and makes approval versus external acceptance explicit. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use outstanding from ledger.
...
npm --prefix Frontend test -- e2e/financial-workflows.spec.ts --project=chromium
```
...
- [ ] Changed revision invalidates approval controls until refresh.
- [ ] Unknown amounts stay unknown; server outstanding displayed.
- [ ] No client Number arithmetic for totals.
