# DATA-005 — Make import row application atomic and resumable
...
Make import row application atomic and resumable. A source row has one durable result. Either the entity and row result commit together or neither does. Partial imports are visible and retryable.

## 2. Current behavior and evidence

ImportsService.confirm creates an entity then separately marks its row completed and eventually calls the whole import completed even when rows fail.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/imports/imports.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/imports/imports.service.ts) — exists at the baseline commit.
...
- [Backend/src/modules/imports/**tests**/imports.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/imports/__tests__/imports.service.spec.ts) — exists at the baseline commit.
...
A source row has one durable result. Either the entity and row result commit together or neither does. Partial imports are visible and retryable.
...
Chosen approach: A source row has one durable result. Either the entity and row result commit together or neither does. Partial imports are visible and retryable. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                         | Baseline state                                            |
| ---------------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/imports/imports.service.ts`                             | Existing — inspect before editing                         |
| `Backend/prisma/schema.prisma`                                               | Existing — inspect before editing                         |
| `Backend/prisma/migrations/20260915000200_import_row_identity/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/imports/__tests__/imports.service.spec.ts`              | Existing — inspect before editing                         |
| `Backend/test/import-retry.e2e-spec.ts`                                      | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-001](../Pending/TEN-001-create-an-explicit-server-derived-access-context.md), [TEN-002](../Pending/TEN-002-audit-and-backfill-organization-ownership-safely.md), [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md), [TEN-005](../Pending/TEN-005-enforce-tenant-access-throughout-units.md)

**Blocks:** [AI-012](../Pending/AI-012-suggest-csv-mappings-without-applying-imported-rows.md), [PRODUCT-001](../Pending/PRODUCT-001-guide-onboarding-to-the-first-useful-shortlist.md)
...
**Change:** Add Import.organizationId required for new imports; ImportRow.resultEntityId UUID nullable and resultEntityType String nullable; unique importId/rowNumber. Backfill ownership only via TEN-002 reviewed mapping.

**Why:** A durable row outcome makes retries unambiguous.

**Verify immediately:** Unowned imports are quarantined; duplicate row numbers stop migration.
...
**Where:** `Backend/src/modules/imports/imports.service.ts`

**Change:** Claim each validated row with transaction row lock; create entity via transaction client and mark completed/result ID in the same transaction. Batch at most100 rows/request; concurrent callers skip locked/completed rows.

**Why:** Crashes cannot leave an untracked created entity.

**Verify immediately:** Fault injected after entity creation rolls back both effects.
...
**Change:** Confirm only same-tenant ADMIN imports; whitelist mapped fields, resolve parents and reuse DTO validation. Return imported/failed/pending counts; only all-terminal-success means completed, otherwise failed with retained successful rows.

**Why:** Importer must not become a privileged arbitrary field writer.

**Verify immediately:** Retry processes failed/validated rows explicitly, never completed rows.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 4 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/test/import-retry.e2e-spec.ts`

**Change:** Run concurrent confirmation and crash/restart with real PostgreSQL.

**Why:** Exactly-once domain effects require DB evidence.

**Verify immediately:** Every completed row points to exactly one entity.
...
### Test 1 — Crash rolls back row

**Purpose:** Prove the boundary named “Crash rolls back row” for make import row application atomic and resumable.
...
**Setup:** Validated row; inject DB failure before row outcome.

**Action:** Confirm then retry.

**Expected result:** One eventual entity; no orphan from failed transaction.
...
**Why this test matters:** A source row has one durable result. Either the entity and row result commit together or neither does. Partial imports are visible and retryable. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Ensure both queries use transaction client.

### Test 2 — Concurrent confirmation

**Purpose:** Prove the boundary named “Concurrent confirmation” for make import row application atomic and resumable.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same import confirmed on two connections.

**Action:** Run both calls together.

**Expected result:** One entity per source row; correct counts.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A source row has one durable result. Either the entity and row result commit together or neither does. Partial imports are visible and retryable. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect lock scope and completed predicate.

### Test 3 — Untrusted mapped ownership

**Purpose:** Prove the boundary named “Untrusted mapped ownership” for make import row application atomic and resumable.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** CSV maps role or organizationId or foreign parent.

**Action:** Validate/confirm as orgA admin.

**Expected result:** Rejected row; no foreign write; safe failure summary.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A source row has one durable result. Either the entity and row result commit together or neither does. Partial imports are visible and retryable. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Use allowlisted entity DTOs instead of spread mappedData.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/imports/__tests__/imports.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/import-retry.e2e-spec.ts
...
- [ ] Unowned imports are quarantined; duplicate row numbers stop migration.
- [ ] Fault injected after entity creation rolls back both effects.
- [ ] Retry processes failed/validated rows explicitly, never completed rows.
- [ ] Every completed row points to exactly one entity.
