# AI-012 — Suggest CSV mappings without applying imported rows
...
Suggest CSV mappings without applying imported rows. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

ImportsService supports user column mapping and validation; those steps are currently deterministic/manual.
...
| File                                                   | Baseline state                                            |
| ------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/ai/csv-mapping.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/csv-mapping.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/csv-mapping.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [DATA-005](../Pending/DATA-005-make-import-row-application-atomic-and-resumable.md)
...
Command API: POST /api/v1/ai/csv-mapping with Idempotency-Key; input {importId:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {mapping:Record<allowedSystemField, string|null>, warnings:string[], unmappedColumns:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/csv-mapping.service.ts`

**Change:** Load only headers and up to20 redacted sample rows from a permitted import; cap100 columns.

**Why:** Column mapping does not require sending the entire customer dataset.

**Verify immediately:** Provider input bounded and no secrets/contact values where unnecessary.
...
**Where:** `Backend/src/modules/ai/dto/csv-mapping.schema.ts`

**Change:** Validate proposed target fields against the current entity import allowlist; forbid organizationId/role/status privilege fields.

**Why:** Model suggestions cannot expand import authority.

**Verify immediately:** Unknown target fields rejected; unmapped columns remain explicit.
...
**Change:** Present mapping as a draft accepted through existing mapColumns and validate steps; do not call confirm from AI command.

**Why:** Mapping confidence does not prove each row valid.

**Verify immediately:** No entity creation before independent user confirmation.
...
### Test 1 — Unknown columns

**Purpose:** Prove the boundary named “Unknown columns” for suggest csv mappings without applying imported rows.
...
**Setup:** CSV has rent and admin_role.

**Action:** Suggest mapping.

**Expected result:** admin_role unmapped; no privilege target.
...
**Likely failure diagnosis:** Use entity allowlist.

### Test 2 — Small sample

**Purpose:** Prove the boundary named “Small sample” for suggest csv mappings without applying imported rows.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** 10000-row CSV.

**Action:** Capture request payload.

**Expected result:** At most20 samples/100 headers; full rows not sent.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check sampling boundary.

### Test 3 — Invalid row after map

**Purpose:** Prove the boundary named “Invalid row after map” for suggest csv mappings without applying imported rows.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Header mapping valid but bad amount row later.

**Action:** Run normal validate.

**Expected result:** Row error blocks its import; AI suggestion does not suppress validation.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not mark rows validated from model output.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/csv-mapping.spec.ts
```
...
- [ ] Provider input bounded and no secrets/contact values where unnecessary.
- [ ] Unknown target fields rejected; unmapped columns remain explicit.
- [ ] No entity creation before independent user confirmation.
