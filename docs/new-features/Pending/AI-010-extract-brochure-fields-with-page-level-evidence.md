# AI-010 — Extract brochure fields with page level evidence
...
Extract brochure fields with page level evidence. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

Media uploads and property intake exist; no OCR/document extraction model is registered.
...
| File                                                          | Baseline state                                            |
| ------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/inventory-document.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/inventory-document.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/inventory-document.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md), [MEDIA-002](../Pending/MEDIA-002-verify-actual-uploaded-bytes-before-completion.md), [MEDIA-003](../Pending/MEDIA-003-serve-private-media-with-short-lived-authorized-urls.md), [INV-003](../Pending/INV-003-preview-duplicate-inventory-candidates-without-merging.md), [DEC-003](../Pending/DEC-003-select-ai-providers-budgets-and-processing-regions.md)
...
Command API: POST /api/v1/ai/inventory-document with Idempotency-Key; input {sourceId:UUID, sourceVersion:int, buildingId?:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {candidate:{name:string|null, cityText:string|null, localityText:string|null, area:string|null, areaBasis:string|null, monthlyRent:string|null}, evidence:Evidence[], questions:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/inventory-document.service.ts`

**Change:** Require verified private PDF/image SOURCE-v1 object and approved OCR adapter; reject unsupported/encrypted/corrupt/overlimit files.

**Why:** A parser must not fetch arbitrary source URLs.

**Verify immediately:** No LLM call after invalid upload.
...
**Where:** `Backend/src/modules/ai/dto/inventory-document.schema.ts`

**Change:** Extract page-indexed text, then structured candidates with null for absent/conflicting values; preserve original and OCR version.

**Why:** OCR text can be wrong; original evidence must remain accessible.

**Verify immediately:** Each non-null critical field has page/span evidence.
...
**Change:** Return reviewable draft with duplicate suggestions from INV-003; do not create building/floor/unit.

**Why:** A brochure is a claim, not verified inventory.

**Verify immediately:** No property write or verification refresh before AI-011 review.
...
### Test 1 — Conflicting rent

**Purpose:** Prove the boundary named “Conflicting rent” for extract brochure fields with page level evidence.
...
**Setup:** Two PDF pages show different rents.

**Action:** Extract.

**Expected result:** Question/conflict; no arbitrary selected rent.
...
**Likely failure diagnosis:** Check aggregation of repeated facts.

### Test 2 — Injection in PDF

**Purpose:** Prove the boundary named “Injection in PDF” for extract brochure fields with page level evidence.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Document instructs export contacts.

**Action:** Process.

**Expected result:** Only schema draft; no extra tool/read.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Treat OCR text as untrusted.

### Test 3 — Corrupt document

**Purpose:** Prove the boundary named “Corrupt document” for extract brochure fields with page level evidence.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Invalid PDF bytes after upload.

**Action:** Process.

**Expected result:** Failed reason; source retained for permitted review; no business rows.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check parser resource limits and error mapping.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/inventory-document.spec.ts
```
...
- [ ] No LLM call after invalid upload.
- [ ] Each non-null critical field has page/span evidence.
- [ ] No property write or verification refresh before AI-011 review.
