# FIELD-001 — Resume interrupted mobile intake uploads
...
Resume interrupted mobile intake uploads. A minimum valid intake survives first. Media upload is a separate resumable operation attached to that server record.

## 2. Current behavior and evidence

Latest source already has staged property intake and NEW/IN_PROGRESS/FOLLOW_UP/COMPLETED states. Extend this flow instead of building another property creator.
...
- [Frontend/src/app/(dashboard)/properties/new/page.tsx](<https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Frontend/src/app/(dashboard)/properties/new/page.tsx>) — exists at the baseline commit.
...
A minimum valid intake survives first. Media upload is a separate resumable operation attached to that server record.
...
Chosen approach: A minimum valid intake survives first. Media upload is a separate resumable operation attached to that server record. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                   | Baseline state                                            |
| ------------------------------------------------------ | --------------------------------------------------------- |
| `Frontend/src/components/media/FileUpload.tsx`         | Proposed — create here or consume dependency-created file |
| `Frontend/src/hooks/use-media.ts`                      | Existing — inspect before editing                         |
| `Frontend/src/components/properties/MobileCapture.tsx` | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(dashboard)/properties/new/page.tsx` | Existing — inspect before editing                         |
| `Frontend/e2e/mobile-capture.spec.ts`                  | Proposed — create here or consume dependency-created file |
...
**Depends On:** [MEDIA-004](../Pending/MEDIA-004-show-real-upload-progress-and-retry-states.md), [MEDIA-005](../Pending/MEDIA-005-clean-expired-pending-uploads-without-deleting-valid-files.md), [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md)

**Blocks:** [FIELD-002](../Pending/FIELD-002-save-offline-text-drafts-with-explicit-conflict-review.md)
...
True multipart byte-range resume is excluded until storage-specific capability is verified. This ticket delivers durable record capture plus resumable user workflow with full-file retries.
...
**Where:** `Frontend/src/components/media/FileUpload.tsx`

**Change:** Mount a narrow-screen capture form reusing existing CreateBuildingSchema minimum required fields and intake endpoint. Save server record before upload.

**Why:** An interrupted image must not lose all address work.

**Verify immediately:** One client request identity reused on duplicate submit; persisted intake ID retained.
...
**Change:** Persist nonsecret upload intent metadata per account/intake; browser File bytes require explicit reselection after reload. Retry failed whole-file PUT with a refreshed grant; do not claim byte-range resumability.

**Why:** The current signed PUT transport does not establish multipart resumability.

**Verify immediately:** UI says reselect/retry when bytes are unavailable; completed uploads not repeated.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/components/properties/MobileCapture.tsx`

**Change:** Use actual MEDIA-004 progress, cancellation and status recheck; clear local intent at logout/closure.

**Why:** Progress must reflect transport and tenant context.

**Verify immediately:** New account cannot resume prior account upload.
...
### Test 1 — Upload interruption

**Purpose:** Prove the boundary named “Upload interruption” for resume interrupted mobile intake uploads.
...
**Setup:** Intake saved; network drops halfway.

**Action:** Reconnect and retry chosen file.

**Expected result:** Same intake, one completed attachment; honest restart progress.
...
**Why this test matters:** A minimum valid intake survives first. Media upload is a separate resumable operation attached to that server record. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate intake creation from upload retry.

### Test 2 — Reload

**Purpose:** Prove the boundary named “Reload” for resume interrupted mobile intake uploads.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Pending file not available after reload.

**Action:** Open draft.

**Expected result:** Reselect file prompt; no fake resumed byte count.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A minimum valid intake survives first. Media upload is a separate resumable operation attached to that server record. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not serialize File into JSON.

### Test 3 — Account switch

**Purpose:** Prove the boundary named “Account switch” for resume interrupted mobile intake uploads.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** AccountA pending intent.

**Action:** Logout/loginB.

**Expected result:** NoA draft or upload request.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A minimum valid intake survives first. Media upload is a separate resumable operation attached to that server record. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Partition and clear local state.
...
npm --prefix Frontend test -- e2e/mobile-capture.spec.ts --project=chromium
```
...
- [ ] One client request identity reused on duplicate submit; persisted intake ID retained.
- [ ] UI says reselect/retry when bytes are unavailable; completed uploads not repeated.
- [ ] New account cannot resume prior account upload.
