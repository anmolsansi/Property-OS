# TEN-006 — Enforce tenant access throughout tasks
...
Enforce tenant access throughout tasks. A Task and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. follow-ups inherit parent access.

## 2. Current behavior and evidence

tasks routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.
...
A Task and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. follow-ups inherit parent access.
...
Chosen approach: A Task and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. follow-ups inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/test/tasks-tenant.e2e-spec.ts`                     | Proposed — create here or consume dependency-created file |
...
**Blocks:** [DATA-003](../Pending/DATA-003-exclude-archived-tasks-from-normal-reads.md), [AI-016](../Pending/AI-016-summarize-a-deterministic-daily-work-briefing.md), [AI-029](../Pending/AI-029-apply-reviewed-visit-and-voice-action-drafts-once.md)
...
**Change:** Conjoin organization and existing role/geography restrictions for list/count/detail through organization and optional client/building. Resolve each supplied parent before create or reparent.
...
**Change:** Authorize follow-ups via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/tasks-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout tasks.
...
**Why this test matters:** A Task and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. follow-ups inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout tasks.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout tasks.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/tasks-tenant.e2e-spec.ts
```
