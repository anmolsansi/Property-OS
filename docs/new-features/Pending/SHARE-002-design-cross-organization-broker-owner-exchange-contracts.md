# SHARE-002 — Design cross organization broker owner exchange contracts
...
Design cross organization broker owner exchange contracts. Deciding a safe operating contract is work. The output is a signed decision record; writing this ticket does not grant approval or implement a feature.
...
| `docs/new-features/decisions/cross-organization-sharing.md` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [OWNER-001](../Pending/OWNER-001-enforce-explicit-same-tenant-asset-capability-grants.md), [DEC-005](../Pending/DEC-005-approve-sharing-retention-and-customer-data-use.md)
...
- Two-tenant read/download/revoke matrix has zero hidden field leakage
- Ownership of recipient annotations and deletion policy signed
- Fields and source freshness labels are explicit
- Separate schema/API/UI implementation tickets required after customer exchange pilot validates need
...
**Where:** `docs/new-features/decisions/cross-organization-sharing.md`

**Change:** Write the proposed decision: Use an explicit shared snapshot with issuerOrganizationId, recipientOrganizationId, field allowlist, source attribution, expiresAt andrevokedAt. A recipient grant never grants direct source-table access. Refresh creates a new version; revocation stops reads/downloads/jobs. Do not enable cross-organization writes in initial exchange.
...
**Change:** Run the decision review against: Two-tenant read/download/revoke matrix has zero hidden field leakage; Ownership of recipient annotations and deletion policy signed; Fields and source freshness labels are explicit; Separate schema/API/UI implementation tickets required after customer exchange pilot validates need
...
**Purpose:** Prove the boundary named “Evidence is traceable” for design cross organization broker owner exchange contracts.
...
**Purpose:** Prove the boundary named “Decision covers failure and recovery” for design cross organization broker owner exchange contracts.
...
**Purpose:** Prove the boundary named “No premature activation” for design cross organization broker owner exchange contracts.
