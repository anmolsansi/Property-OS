# MEDIA-001 — Authorize upload completion and downloads by parent
...
Authorize upload completion and downloads by parent. A storage key is not an access grant. Every media operation resolves its real building, even when attached through floor or unit.

## 2. Current behavior and evidence

Upload, complete-upload and download-url handlers do not currently pass actor/geography scope; direct media reads can lack a building relation.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/media/media.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/media/media.controller.ts) — exists at the baseline commit.
...
- [Backend/src/modules/media/dto/media.schema.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/media/dto/media.schema.ts) — exists at the baseline commit.
- [Backend/src/modules/media/**tests**/media.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/media/__tests__/media.service.spec.ts) — exists at the baseline commit.
...
A storage key is not an access grant. Every media operation resolves its real building, even when attached through floor or unit.
...
Chosen approach: A storage key is not an access grant. Every media operation resolves its real building, even when attached through floor or unit. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                        | Baseline state                                            |
| ----------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/media/media.controller.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/media/media.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/media/dto/media.schema.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/media/__tests__/media.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/media-access.e2e-spec.ts`                     | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-001](../Pending/TEN-001-create-an-explicit-server-derived-access-context.md), [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md), [TEN-005](../Pending/TEN-005-enforce-tenant-access-throughout-units.md), [TEN-009](../Pending/TEN-009-enforce-tenant-access-throughout-floors.md)

**Blocks:** [MEDIA-002](../Pending/MEDIA-002-verify-actual-uploaded-bytes-before-completion.md), [MEDIA-003](../Pending/MEDIA-003-serve-private-media-with-short-lived-authorized-urls.md), [MEDIA-004](../Pending/MEDIA-004-show-real-upload-progress-and-retry-states.md)
...
**Where:** `Backend/src/modules/media/media.controller.ts`

**Change:** Pass AccessContext through upload/complete/download and metadata/delete/restore handlers.

**Why:** Protection must cover every object lifecycle action.

**Verify immediately:** Denied actors never receive signed URLs.
...
**Change:** Resolve supplied building/floor/unit lineage; require exactly one valid target association and derive building scope; persist uploadedBy from actor.

**Why:** Ownership must not rely on user-supplied metadata.

**Verify immediately:** Mismatched parent IDs are400; inaccessible parents404.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/media/dto/media.schema.ts`

**Change:** Authorize the stored parent on completion/download; only uploader or same-tenant ADMIN completes pending media; require completed/nondeleted for download.

**Why:** An attacker cannot complete another upload or download a pending object.

**Verify immediately:** No S3 signing/HEAD call before authorization.
...
### Test 1 — Foreign media URL

**Purpose:** Prove the boundary named “Foreign media URL” for authorize upload completion and downloads by parent.
...
**Action:** OrgA requests download-url.

**Expected result:** 404; no getSignedUrl call.
...
**Why this test matters:** A storage key is not an access grant. Every media operation resolves its real building, even when attached through floor or unit. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check stored parent resolution.

### Test 2 — Complete another upload

**Purpose:** Prove the boundary named “Complete another upload” for authorize upload completion and downloads by parent.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Same org worker B, uploader A.

**Action:** B submits complete-upload.

**Expected result:** 403; media remains pending; no HEAD.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A storage key is not an access grant. Every media operation resolves its real building, even when attached through floor or unit. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check actor ownership on completion.

### Test 3 — Indirect parent

**Purpose:** Prove the boundary named “Indirect parent” for authorize upload completion and downloads by parent.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Media on a unit without direct buildingId.

**Action:** Read as assigned/unassigned workers.

**Expected result:** Actual unit building determines access; no missing-parent bypass.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A storage key is not an access grant. Every media operation resolves its real building, even when attached through floor or unit. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Resolve relations consistently.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/media/__tests__/media.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/media-access.e2e-spec.ts
```
...
- [ ] Denied actors never receive signed URLs.
- [ ] Mismatched parent IDs are400; inaccessible parents404.
- [ ] No S3 signing/HEAD call before authorization.
