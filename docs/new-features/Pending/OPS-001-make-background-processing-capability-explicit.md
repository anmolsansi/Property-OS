# OPS-001 — Make background processing capability explicit
...
Make background processing capability explicit. A disabled capability is not a successful delivery. Startup/health and callers must distinguish durable acceptance from unavailable infrastructure.

## 2. Current behavior and evidence

NotificationsModule disables queues when REDIS_HOST is missing/disabled; sendEmail/sendSms log and return. Render config sets Redis disabled.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/notifications/notifications.module.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/notifications/notifications.module.ts) — exists at the baseline commit.
...
- [render.yaml](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/render.yaml) — exists at the baseline commit.
- [Backend/src/modules/notifications/**tests**/notifications.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/notifications/__tests__/notifications.service.spec.ts) — exists at the baseline commit.
...
A disabled capability is not a successful delivery. Startup/health and callers must distinguish durable acceptance from unavailable infrastructure.
...
Chosen approach: A disabled capability is not a successful delivery. Startup/health and callers must distinguish durable acceptance from unavailable infrastructure. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                        | Baseline state                    |
| --------------------------------------------------------------------------- | --------------------------------- |
| `Backend/src/modules/notifications/notifications.module.ts`                 | Existing — inspect before editing |
...
| `render.yaml`                                                               | Existing — inspect before editing |
| `Backend/src/modules/notifications/__tests__/notifications.service.spec.ts` | Existing — inspect before editing |
...
**Depends On:** [DEC-003](../Pending/DEC-003-select-ai-providers-budgets-and-processing-regions.md)

**Blocks:** [OPS-003](../Pending/OPS-003-dispatch-outbox-events-with-retry-safe-claims.md)
...
**Where:** `Backend/src/modules/notifications/notifications.service.ts`, `Backend/src/health/health.controller.ts`

**Change:** Validate NOTIFICATIONS_REQUIRED boolean at startup; required+disabled Redis fails readiness; optional mode reports disabled capability explicitly.

**Why:** Operators must see deployment capability mismatch.

**Verify immediately:** Health payload adds capabilities.notifications=ready|disabled|unavailable without secrets.
...
**Change:** Return structured acceptance {accepted, deliveryId:null|string, reason:null|QUEUE_DISABLED}; callers cannot claim sent when accepted=false.

**Why:** A warning log is invisible to a user awaiting a reminder.

**Verify immediately:** Disabled queue creates no success delivery state.
...
**Change:** Document worker/Redis deployment and failover; replace free-tier disabled default only when approved infrastructure exists.

**Why:** A config string cannot provision a reliable worker.

**Verify immediately:** Staging outage/reconnect demonstrated and main API manual flows remain usable.
...
### Test 1 — Required queue absent

**Purpose:** Prove the boundary named “Required queue absent” for make background processing capability explicit.
...
**Setup:** NOTIFICATIONS_REQUIRED=true, Redis disabled.

**Action:** Start/readiness probe.

**Expected result:** Not ready with safe reason; no silent delivery acceptance.
...
**Why this test matters:** A disabled capability is not a successful delivery. Startup/health and callers must distinguish durable acceptance from unavailable infrastructure. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check config validation before queue creation.

### Test 2 — Optional mode

**Purpose:** Prove the boundary named “Optional mode” for make background processing capability explicit.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Optional Redis disabled.

**Action:** Request notification.

**Expected result:** accepted=false reason QUEUE_DISABLED; no sent marker.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A disabled capability is not a successful delivery. Startup/health and callers must distinguish durable acceptance from unavailable infrastructure. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect optional injection fallback.

### Test 3 — Redis recovery

**Purpose:** Prove the boundary named “Redis recovery” for make background processing capability explicit.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Configured Redis loses connectivity then returns.

**Action:** Queue known notification.

**Expected result:** Durable work eventually processed once logically; capability recovers.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A disabled capability is not a successful delivery. Startup/health and callers must distinguish durable acceptance from unavailable infrastructure. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check reconnect and retry status.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/notifications/__tests__/notifications.service.spec.ts
```
...
- [ ] Health payload adds capabilities.notifications=ready|disabled|unavailable without secrets.
- [ ] Disabled queue creates no success delivery state.
- [ ] Staging outage/reconnect demonstrated and main API manual flows remain usable.
