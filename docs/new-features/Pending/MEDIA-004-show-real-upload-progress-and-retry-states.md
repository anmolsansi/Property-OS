# MEDIA-004 — Show real upload progress and retry states
...
Show real upload progress and retry states. Selected, transferred and server-verified are different states. Success means the completion API accepted the actual file.

## 2. Current behavior and evidence

FileUpload simulates progress with a timer; onUpload is optional. Existing use-media implements storage-related API calls.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Frontend/src/components/shared/FileUpload.tsx](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Frontend/src/components/shared/FileUpload.tsx) — exists at the baseline commit.
...
Selected, transferred and server-verified are different states. Success means the completion API accepted the actual file.
...
Chosen approach: Selected, transferred and server-verified are different states. Success means the completion API accepted the actual file. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                            | Baseline state                                            |
| ----------------------------------------------- | --------------------------------------------------------- |
| `Frontend/src/components/shared/FileUpload.tsx` | Existing — inspect before editing                         |
| `Frontend/src/hooks/use-media.ts`               | Existing — inspect before editing                         |
| `Frontend/e2e/media-upload.spec.ts`             | Proposed — create here or consume dependency-created file |
...
**Depends On:** [MEDIA-001](../Pending/MEDIA-001-authorize-upload-completion-and-downloads-by-parent.md), [MEDIA-002](../Pending/MEDIA-002-verify-actual-uploaded-bytes-before-completion.md)

**Blocks:** [FIELD-001](../Pending/FIELD-001-resume-interrupted-mobile-intake-uploads.md)
...
**Where:** `Frontend/src/components/shared/FileUpload.tsx`

**Change:** Require an upload callback and distinguish selected/uploading/verifying/complete/error; remove simulated interval progress and mock success.

**Why:** False completion leads users to discard their source file.

**Verify immediately:** No handler configured shows disabled upload with explanatory text.
...
**Change:** Report transfer progress from XMLHttpRequest or the existing transport callback; call complete-upload only after PUT succeeds; keep mediaId for retry of completion.

**Why:** Retried verification must not create duplicate pending media.

**Verify immediately:** Failed PUT never calls completion; failed completion retries same ID.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/e2e/media-upload.spec.ts`

**Change:** Allow cancel/retry per file; revoke object URLs on removal/unmount; preserve existing successfully uploaded files when another fails.

**Why:** A batch is not all-or-nothing UI state.

**Verify immediately:** Mixed batch shows accurate per-file outcomes after refresh.
...
### Test 1 — Transfer fails

**Purpose:** Prove the boundary named “Transfer fails” for show real upload progress and retry states.
...
**Setup:** Two files, first PUT500 second200.

**Action:** Upload batch.

**Expected result:** Only second may complete; first shows retry; no fake100%.
...
**Why this test matters:** Selected, transferred and server-verified are different states. Success means the completion API accepted the actual file. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Remove timer-based success.

### Test 2 — Completion fails

**Purpose:** Prove the boundary named “Completion fails” for show real upload progress and retry states.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** PUT succeeds, completion503 then200.

**Action:** Retry failed file.

**Expected result:** Same mediaId reused; one stored completed media.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Selected, transferred and server-verified are different states. Success means the completion API accepted the actual file. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check retry stage persistence.

### Test 3 — Cancel

**Purpose:** Prove the boundary named “Cancel” for show real upload progress and retry states.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Large file uploading with local preview.

**Action:** Cancel and unmount.

**Expected result:** Request aborted; no completion; object URL released.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Selected, transferred and server-verified are different states. Success means the completion API accepted the actual file. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect cancellation cleanup.
...
npm --prefix Frontend test -- e2e/media-upload.spec.ts --project=chromium
```
...
- [ ] No handler configured shows disabled upload with explanatory text.
- [ ] Failed PUT never calls completion; failed completion retries same ID.
- [ ] Mixed batch shows accurate per-file outcomes after refresh.
