# INTEGRATION-001 — Specify scoped integration API and webhook delivery
...
Specify scoped integration API and webhook delivery. Deciding a safe operating contract is work. The output is a signed decision record; writing this ticket does not grant approval or implement a feature.
...
- [Backend/src/main.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/main.ts) — exists at the baseline commit.
...
| File                                            | Baseline state                                            |
| ----------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/main.ts`                           | Existing — inspect before editing                         |
| `Backend/prisma/schema.prisma`                  | Existing — inspect before editing                         |
| `docs/MARKET_AI_ASSESSMENT.md`                  | Existing — inspect before editing                         |
| `docs/new-features/decisions/integration-v1.md` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md), [SAAS-002](../Pending/SAAS-002-enforce-pilot-entitlements-and-durable-usage-limits.md), [DEC-008](../Pending/DEC-008-choose-lease-offer-approval-and-accounting-boundaries.md)
...
- Name partner sandbox and exact payload fixtures without secrets
- Define60second signature replay tolerance, rotation overlap, rate limits, retry/dead-letter policy
- Reject loopback/private/link-local destinations and redirects to them
- No generic write API or accounting push before approved reconciliation contract
- Create bounded implementation tickets only after sandbox contract approval
...
**Where:** `docs/new-features/decisions/integration-v1.md`

**Change:** Write the proposed decision: Start with read-only inventory and event webhooks for named pilot partner. API keys are hashed, scoped to organization and capabilities, rotatable andrevocable. Webhooks use timestamped HMAC signature, eventId/schemaVersion and per-destination idempotent delivery logs. Partner endpoints must pass SSRF controls and DNS rebinding protection.
...
**Change:** Run the decision review against: Name partner sandbox and exact payload fixtures without secrets; Define60second signature replay tolerance, rotation overlap, rate limits, retry/dead-letter policy; Reject loopback/private/link-local destinations and redirects to them; No generic write API or accounting push before approved reconciliation contract; Create bounded implementation tickets only after sandbox contract approval
...
**Purpose:** Prove the boundary named “Evidence is traceable” for specify scoped integration api and webhook delivery.
...
**Purpose:** Prove the boundary named “Decision covers failure and recovery” for specify scoped integration api and webhook delivery.
...
**Purpose:** Prove the boundary named “No premature activation” for specify scoped integration api and webhook delivery.
