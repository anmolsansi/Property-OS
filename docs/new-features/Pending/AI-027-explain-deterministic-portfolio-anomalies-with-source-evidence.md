# AI-027 — Explain deterministic portfolio anomalies with source evidence
...
Explain deterministic portfolio anomalies with source evidence. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

Reconciled report snapshots and lease versions are prerequisites; current listing count is not a reliable financial anomaly baseline.
...
| File                                                           | Baseline state                                            |
| -------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/ai/portfolio-anomalies.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/portfolio-anomalies.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/portfolio-anomalies.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [REPORT-001](../Pending/REPORT-001-compute-allowlisted-portfolio-reports.md), [FIN-002](../Pending/FIN-002-export-reconciled-rent-rolls-with-source-identities.md), [AI-021](../Pending/AI-021-answer-document-questions-with-verifiable-citations.md)
...
Command API: POST /api/v1/ai/portfolio-anomalies with Idempotency-Key; input {currentReportId:UUID, previousReportId:UUID}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {changes:[{metric:string, delta:string, sourceIds:UUID[], explanation:string|null, unknownCause:boolean}], warnings:string[]}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/portfolio-anomalies.service.ts`

**Change:** Require same organization, definition, basis and comparable periods; code computes deltas using decimals.

**Why:** Different denominators can create false anomalies.

**Verify immediately:** Incompatible reports409 with explanation.
...
**Where:** `Backend/src/modules/ai/dto/portfolio-anomalies.schema.ts`

**Change:** Retrieve documented lease changes, verified vacancies and reconciled rows explaining deltas; model summarizes only those drivers.

**Why:** Correlation does not establish a cause.

**Verify immediately:** Unexplained residual remains unknownCause=true.
...
**Change:** Persist draft with report/source version IDs; no automatic repricing, collections, owner notices or lease edits.

**Why:** An alert is a review aid, not an autonomous financial action.

**Verify immediately:** Provider failure leaves deterministic delta table available.
...
### Test 1 — Changed denominator

**Purpose:** Prove the boundary named “Changed denominator” for explain deterministic portfolio anomalies with source evidence.
...
**Setup:** Current report includes new assets.

**Action:** Compare.

**Expected result:** Basis warning/reject or separately explained scope change, not rent collapse.
...
**Likely failure diagnosis:** Compare filters and source set.

### Test 2 — No cause

**Purpose:** Prove the boundary named “No cause” for explain deterministic portfolio anomalies with source evidence.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Delta has no documented event.

**Action:** Explain.

**Expected result:** Unknown cause, not invented tenant default.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Require driver evidence.

### Test 3 — Numerical drift

**Purpose:** Prove the boundary named “Numerical drift” for explain deterministic portfolio anomalies with source evidence.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Model alters delta.

**Action:** Validate.

**Expected result:** Reject; render code-computed value.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Keep arithmetic outside prose.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/portfolio-anomalies.spec.ts
```
...
- [ ] Incompatible reports409 with explanation.
- [ ] Unexplained residual remains unknownCause=true.
- [ ] Provider failure leaves deterministic delta table available.
