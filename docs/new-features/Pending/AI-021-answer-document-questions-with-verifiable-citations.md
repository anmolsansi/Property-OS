# AI-021 — Answer document questions with verifiable citations
...
Answer document questions with verifiable citations. This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent.

## 2. Current behavior and evidence

AI-020 returns scoped chunks; no document answer should bypass that retrieval boundary.
...
| File                                                         | Baseline state                                            |
| ------------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/ai/document-question.service.ts`        | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/dto/document-question.schema.ts`     | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/ai/__tests__/document-question.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-020](../Pending/AI-020-index-authorized-source-chunks-for-cited-retrieval.md), [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [AI-007](../Pending/AI-007-gate-prompt-and-model-changes-with-held-out-evaluations.md)

**Blocks:** [AI-027](../Pending/AI-027-explain-deterministic-portfolio-anomalies-with-source-evidence.md), [AI-028](../Pending/AI-028-launch-and-review-advanced-ai-commands-in-a-typed-console.md)
...
Command API: POST /api/v1/ai/document-question with Idempotency-Key; input {contextType:client|building|lease, contextId:UUID, question:string<=2000}. Return202 {id, status:queued}, then GET /ai/intakes/:id using AIJOB-v1. Output stored under strict outputJson schema: {answer:string<=3000, citations:Evidence[], abstained:boolean, reasonCode:string|null}. Register the handler in ai/ai.module.ts and the closed ai/command-registry.ts created by AI-001; kind names use underscores. Preserve all other command shapes. Add this command’s cases to the shared held-out evaluation corpus before enabling it.
...
**Where:** `Backend/src/modules/ai/document-question.service.ts`

**Change:** Retrieve at most8 chunks through AI-020; no external search or arbitrary tools. If insufficient evidence return abstained=true.

**Why:** Knowledge from model training is not evidence about this lease/property.

**Verify immediately:** Every factual answer span has a source citation.
...
**Where:** `Backend/src/modules/ai/dto/document-question.schema.ts`

**Change:** Validate citation source/version/page/offset against retrieved chunk allowlist; reject fabricated citations and conflicting critical facts.

**Why:** Citation-shaped text can still be invented.

**Verify immediately:** No answer when cited text fails source match.
...
**Change:** Recheck permissions/expiry before returning; display source excerpts and uncertainty, not legal conclusions. Add50 held-out answerable/unanswerable/injection fixtures.

**Why:** Access may change during generation.

**Verify immediately:** A revoked source yields no generated answer payload.
...
**Purpose:** Prove the boundary named “No evidence” for answer document questions with verifiable citations.
...
**Setup:** Question asks unstated lease option.

**Action:** Answer.

**Expected result:** Abstain, no invented clause.
...
**Likely failure diagnosis:** Set evidence threshold.

### Test 2 — Fake citation

**Purpose:** Prove the boundary named “Fake citation” for answer document questions with verifiable citations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Provider cites unreturned page.

**Action:** Validate.

**Expected result:** INVALID_OUTPUT.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Exact allowlist match.

### Test 3 — Revoked midflight

**Purpose:** Prove the boundary named “Revoked midflight” for answer document questions with verifiable citations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Source revoked during provider call.

**Action:** Return.

**Expected result:** Denied, answer not exposed.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** This command produces a reviewable draft. It reuses the durable job boundary; it never becomes an independent privileged agent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Final authorization check.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/document-question.spec.ts
```
...
- [ ] Every factual answer span has a source citation.
- [ ] No answer when cited text fails source match.
- [ ] A revoked source yields no generated answer payload.
