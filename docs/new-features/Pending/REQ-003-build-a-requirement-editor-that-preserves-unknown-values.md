# REQ-003 — Build a requirement editor that preserves unknown values

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Requirements UI  
**Assessment:** P04  
...
Build a requirement editor that preserves unknown values. The form edits a confirmed brief, not inferred defaults. A failed save retains user input and never displays success.

## 2. Current behavior and evidence

Current client adapter fills several missing requirement values with zeros/defaults, which cannot express unknown commercial requirements.
...
The form edits a confirmed brief, not inferred defaults. A failed save retains user input and never displays success.
...
Chosen approach: The form edits a confirmed brief, not inferred defaults. A failed save retains user input and never displays success. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                            | Baseline state                                            |
| --------------------------------------------------------------- | --------------------------------------------------------- |
| `Frontend/src/hooks/use-requirement-brief.ts`                   | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/requirements/RequirementBriefForm.tsx` | Proposed — create here or consume dependency-created file |
| `Frontend/src/app/(v2)/requirements/[id]/page.tsx`              | Existing — inspect before editing                         |
| `Frontend/e2e/requirement-brief.spec.ts`                        | Proposed — create here or consume dependency-created file |
...
**Depends On:** [REQ-002](../Pending/REQ-002-expose-atomic-brief-read-and-replacement-endpoints.md)

**Blocks:** [MATCH-002](../Pending/MATCH-002-expose-match-runs-and-comparison-ui.md), [AI-006](../Pending/AI-006-build-source-side-by-side-draft-review-ui.md), [PRODUCT-001](../Pending/PRODUCT-001-guide-onboarding-to-the-first-useful-shortlist.md)
...
**Where:** `Frontend/src/hooks/use-requirement-brief.ts`

**Change:** Add typed GET/PUT hooks for BRIEF-v1 keyed by organization/requirement ID; model optional numbers as null/empty until user enters them.

**Why:** Zero is a meaningful parking/rent value.

**Verify immediately:** Loading placeholders do not prefill assumed values.
...
**Where:** `Frontend/src/components/requirements/RequirementBriefForm.tsx`

**Change:** Show area/budget basis selectors, locality multi-select, date, furnishing, parking and must-have controls; use exact backend limits.

**Why:** The user must resolve ambiguous input before matching.

**Verify immediately:** Incomplete required basis shows Needs clarification instead of Match ready.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/app/(v2)/requirements/[id]/page.tsx`

**Change:** On409 preserve draft and display server version with Reload latest action; no automatic overwrite. Invalidate brief and matching cache only after confirmed success.

**Why:** Concurrent edits need deliberate resolution.

**Verify immediately:** Failed save leaves values and server unchanged.
...
### Test 1 — Unknown remains blank

**Purpose:** Prove the boundary named “Unknown remains blank” for build a requirement editor that preserves unknown values.
...
**Setup:** GET brief has null parking/bases.

**Action:** Open editor.

**Expected result:** Blank/unknown state, no invented0 or carpet basis.
...
**Why this test matters:** The form edits a confirmed brief, not inferred defaults. A failed save retains user input and never displays success. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect adapter defaults.

### Test 2 — Stale save

**Purpose:** Prove the boundary named “Stale save” for build a requirement editor that preserves unknown values.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Editor version1, server advanced2.

**Action:** Save.

**Expected result:** 409 conflict UI; entered values retained; no success toast.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The form edits a confirmed brief, not inferred defaults. A failed save retains user input and never displays success. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Handle mutation error before reset.

### Test 3 — Successful save and reload

**Purpose:** Prove the boundary named “Successful save and reload” for build a requirement editor that preserves unknown values.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Valid explicit brief.

**Action:** Save and reload page.

**Expected result:** Persisted values/version visible; matching cache refreshed.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** The form edits a confirmed brief, not inferred defaults. A failed save retains user input and never displays success. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check response unwrap and query keys.
...
npm --prefix Frontend test -- e2e/requirement-brief.spec.ts --project=chromium
```
...
- [ ] Loading placeholders do not prefill assumed values.
- [ ] Incomplete required basis shows Needs clarification instead of Match ready.
- [ ] Failed save leaves values and server unchanged.
