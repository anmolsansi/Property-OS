# CRM-004 — Assign leads with explicit response deadlines

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Lead operations  
**Assessment:** P11  
...
Assign leads with explicit response deadlines. A response SLA is a stored due time with an owner. Assignment does not mean a response was sent.

## 2. Current behavior and evidence

Current client/deal assignees do not provide a durable response clock and absence/reassignment policy.
...
A response SLA is a stored due time with an owner. Assignment does not mean a response was sent.
...
Chosen approach: A response SLA is a stored due time with an owner. Assignment does not mean a response was sent. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                          | Baseline state                                            |
| ------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/lead-routing/lead-routing.service.ts`    | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/lead-routing/lead-routing.controller.ts` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/lead-routing/lead-routing.module.ts`     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/lead-routing/lead-routing.schema.ts`     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/lead-routing/lead-routing.spec.ts`       | Proposed — create here or consume dependency-created file |
...
**Depends On:** [CRM-002](../Pending/CRM-002-record-authorized-interaction-history-and-corrections.md), [TEN-003](../Pending/TEN-003-enforce-tenant-access-throughout-clients.md), [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md)
...
Additional allowed files: Backend/prisma/schema.prisma; Backend/prisma/migrations/20260915001700_assignment_events/migration.sql; Backend/src/app.module.ts. No automatic reassignment or inbox integration in this ticket.
...
**Where:** `Backend/src/modules/lead-routing/lead-routing.service.ts`

**Change:** Implement pure policy computeDeadline(assignedAt, timeZone, workingHours, holidayDates) using versioned team settings. Pilot uses weekdays09:00–18:00 Asia/Kolkata,4 working hours, no hidden holidays.

**Why:** Elapsed clock time differs from working time.

**Verify immediately:** Friday17:00 deadline Monday12:00 with no holiday.
...
**Where:** `Backend/src/modules/lead-routing/lead-routing.controller.ts`

**Change:** Persist AssignmentEvent(id, organizationId, clientId, assigneeId, policyVersion, dueAt, firstResponseAt nullable, version) through new Prisma model/migration; POST /clients/:id/assignment {assigneeId, expectedVersion} ADMIN only. Validate active same-tenant assignee.

**Why:** Reassignment needs durable evidence and a compare boundary.

**Verify immediately:** Closed client or inactive assignee rejected; only one current owner.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/lead-routing/lead-routing.service.ts`, `Backend/src/modules/lead-routing/lead-routing.controller.ts`

**Change:** On deadline use OPS outbox to create one internal escalation task keyed assignmentId/version; approved contact_attempt with confirmed delivery ends SLA, draft creation does not.

**Why:** An AI draft cannot satisfy customer response.

**Verify immediately:** Reassignment invalidates old escalation; absence reassignment stays explicit.
...
### Test 1 — Weekend clock

**Purpose:** Prove the boundary named “Weekend clock” for assign leads with explicit response deadlines.
...
**Setup:** Friday17:00 assignment.

**Action:** Compute4-hour deadline.

**Expected result:** Monday12:00 local.
...
**Why this test matters:** A response SLA is a stored due time with an owner. Assignment does not mean a response was sent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check business time iteration.

### Test 2 — Draft response

**Purpose:** Prove the boundary named “Draft response” for assign leads with explicit response deadlines.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** AI followup generated only.

**Action:** Check SLA.

**Expected result:** firstResponseAt remains null.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A response SLA is a stored due time with an owner. Assignment does not mean a response was sent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Require confirmed contact evidence.

### Test 3 — Reassignment race

**Purpose:** Prove the boundary named “Reassignment race” for assign leads with explicit response deadlines.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Old escalation queued then new owner assigned.

**Action:** Run old event.

**Expected result:** Suppressed; new owner clock shown.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A response SLA is a stored due time with an owner. Assignment does not mean a response was sent. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Compare assignment version.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/lead-routing/lead-routing.spec.ts
```
...
- [ ] Friday17:00 deadline Monday12:00 with no holiday.
- [ ] Closed client or inactive assignee rejected; only one current owner.
- [ ] Reassignment invalidates old escalation; absence reassignment stays explicit.
