# OPS-005 — Replace superficial search and proposal acceptance checks
...
Replace superficial search and proposal acceptance checks. A test proves the returned records and produced artifact, not that a mock method happened to be called.

## 2. Current behavior and evidence

Historical tests asserted Prisma invocation and proposal browser route/CSV expectations were stale; Render uses Clerk while browser fixtures use test-login.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Frontend/e2e/proposals.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Frontend/e2e/proposals.spec.ts) — exists at the baseline commit.
- [Frontend/playwright.config.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Frontend/playwright.config.ts) — exists at the baseline commit.
- [Backend/test/jest-e2e.json](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/test/jest-e2e.json) — exists at the baseline commit.
...
A test proves the returned records and produced artifact, not that a mock method happened to be called.
...
Chosen approach: A test proves the returned records and produced artifact, not that a mock method happened to be called. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                       | Baseline state                                            |
| ------------------------------------------ | --------------------------------------------------------- |
| `Backend/test/search-access.e2e-spec.ts`   | Proposed — create here or consume dependency-created file |
| `Backend/test/proposal-export.e2e-spec.ts` | Proposed — create here or consume dependency-created file |
| `Frontend/e2e/proposals.spec.ts`           | Existing — inspect before editing                         |
| `Frontend/playwright.config.ts`            | Existing — inspect before editing                         |
| `Backend/test/jest-e2e.json`               | Existing — inspect before editing                         |
...
**Depends On:** [SEARCH-002](../Pending/SEARCH-002-merge-rent-and-area-bounds-without-dropping-filters.md), [PROP-003](../Pending/PROP-003-authorize-every-proposal-artifact-export.md), [PROP-004](../Pending/PROP-004-enforce-proposal-item-uniqueness-under-concurrency.md), [AUTH-015](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/docs/tickets/Pending/AUTH-015-add-jwt-auth-guard-regression-tests.md), [AUTH-017](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/docs/tickets/Pending/AUTH-017-add-auth-hardening-e2e-regression-suite.md)

**Blocks:** [OPS-006](../Pending/OPS-006-gate-deployment-with-lint-build-and-database-checks.md)
...
**Where:** `Backend/test/search-access.e2e-spec.ts`

**Change:** Seed isolated PostgreSQL tenants and real related inventory; assert text+geography+min/max+same-unit predicates against actual result IDs.

**Why:** Mock call shape can pass while authorization is wrong.

**Verify immediately:** Hidden IDs absent in payload/counts; empty scope returns zero.
...
**Where:** `Backend/test/proposal-export.e2e-spec.ts`

**Change:** Update proposal browser journey to current route and XLSX/PDF capability; parse artifact content to verify allowed rows/columns and forbidden fields.

**Why:** Download success alone does not prove correct disclosure.

**Verify immediately:** Actual MIME/extension/content agree; forbidden export leaves no artifact.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/e2e/proposal.spec.ts`

**Change:** Use explicit test auth mode only in isolated E2E server; production-mode boot test rejects test-login. Link AUTH-017 for real Clerk acceptance rather than fabricating it.

**Why:** Local bypass cannot stand in for hosted identity behavior.

**Verify immediately:** Report local DB/browser and protected Clerk results separately.
...
### Test 1 — Hidden result

**Purpose:** Prove the boundary named “Hidden result” for replace superficial search and proposal acceptance checks.
...
**Setup:** Matching text on hidden building.

**Action:** Run DB search.

**Expected result:** No hidden rows/counts.
...
**Why this test matters:** A test proves the returned records and produced artifact, not that a mock method happened to be called. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check composed predicates.

### Test 2 — Wrong export

**Purpose:** Prove the boundary named “Wrong export” for replace superficial search and proposal acceptance checks.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Restricted field selected.

**Action:** Download via UI/API.

**Expected result:** Denied; no file/job.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A test proves the returned records and produced artifact, not that a mock method happened to be called. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Assert absence, not just status.

### Test 3 — Prod test login

**Purpose:** Prove the boundary named “Prod test login” for replace superficial search and proposal acceptance checks.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Production config.

**Action:** Call test-login.

**Expected result:** Unavailable/denied and no token.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A test proves the returned records and produced artifact, not that a mock method happened to be called. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Validate mode gate.
...
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/search-access.e2e-spec.ts test/proposal-export.e2e-spec.ts
npm --prefix Frontend run typecheck
npm --prefix Frontend run lint
npm --prefix Frontend run build
npm --prefix Frontend test -- e2e/proposals.spec.ts --project=chromium
```
...
- [ ] Hidden IDs absent in payload/counts; empty scope returns zero.
- [ ] Actual MIME/extension/content agree; forbidden export leaves no artifact.
- [ ] Report local DB/browser and protected Clerk results separately.
