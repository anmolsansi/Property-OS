# AI-007 — Gate prompt and model changes with held out evaluations
...
Gate prompt and model changes with held out evaluations. Model changes change behavior. Passing TypeScript cannot detect invented amounts, unsupported citations or degraded Hindi extraction.

## 2. Current behavior and evidence

Current CI covers typecheck/lint/backend tests, with no model/prompt evaluation set.
...
Model changes change behavior. Passing TypeScript cannot detect invented amounts, unsupported citations or degraded Hindi extraction.
...
Chosen approach: Model changes change behavior. Passing TypeScript cannot detect invented amounts, unsupported citations or degraded Hindi extraction. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/evals/run-ai-evals.ts`   | Proposed — create here or consume dependency-created file |
| `Backend/evals/README.md`         | Proposed — create here or consume dependency-created file |
| `Backend/package.json`            | Existing — inspect before editing                         |
| `.github/workflows/ai-evals.yml`  | Proposed — create here or consume dependency-created file |
...
Model outputs are nondeterministic: store per-case output IDs and distributions for repeated live runs; do not require byte-identical narratives. Evaluation approval does not authorize deployment.
...
**Where:** `Backend/evals/ai-fixtures.jsonl`

**Change:** Author at least100 permissioned/synthetic labeled requirement messages with separate development/held-out sets, field ground truth and evidence spans. Include English/Hindi/Hinglish ambiguity and malicious instructions.

**Why:** A benchmark cannot train and test on the same examples.

**Verify immediately:** No real customer PII or copied private documents in repository.
...
**Where:** `Backend/evals/run-ai-evals.ts`

**Change:** Implement deterministic score report for critical-field accuracy, unsupported values, citation correctness, latency and cost; record model/prompt/schema IDs.

**Why:** A single aggregate score hides dangerous errors.

**Verify immediately:** Report by field/language/input type and retain failures.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/evals/README.md`

**Change:** CI runs schema/mocked adversarial checks on every PR; paid live evaluation is a separately authorized protected job with fixed cap. Gate release at >=95% explicit-field accuracy and zero unsupported critical claims in held-out fixtures.

**Why:** Do not expose secrets or incur unbounded spend on fork PRs.

**Verify immediately:** Regression fails release; no silently relabeled fixtures.
...
### Test 1 — Known wrong critical field

**Purpose:** Prove the boundary named “Known wrong critical field” for gate prompt and model changes with held out evaluations.
...
**Setup:** Fixture budget300000; output3000000.

**Action:** Score run.

**Expected result:** Critical-field failure with exact case ID.
...
**Why this test matters:** Model changes change behavior. Passing TypeScript cannot detect invented amounts, unsupported citations or degraded Hindi extraction. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check numeric unit normalization.

### Test 2 — Unanswerable input

**Purpose:** Prove the boundary named “Unanswerable input” for gate prompt and model changes with held out evaluations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** No area stated.

**Action:** Score output inventing area.

**Expected result:** Unsupported-value failure, even if shape valid.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Model changes change behavior. Passing TypeScript cannot detect invented amounts, unsupported citations or degraded Hindi extraction. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check abstention metric.

### Test 3 — Untrusted PR

**Purpose:** Prove the boundary named “Untrusted PR” for gate prompt and model changes with held out evaluations.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Fork PR triggers workflow.

**Action:** Inspect/run CI dry configuration.

**Expected result:** No provider secret/live call; deterministic checks only.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Model changes change behavior. Passing TypeScript cannot detect invented amounts, unsupported citations or degraded Hindi extraction. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Separate workflow permissions and protected environments.
...
- [ ] No real customer PII or copied private documents in repository.
- [ ] Report by field/language/input type and retain failures.
- [ ] Regression fails release; no silently relabeled fixtures.
