# FIELD-002 — Save offline text drafts with explicit conflict review
...
Save offline text drafts with explicit conflict review. An offline draft is a local proposal to change data. The server remains authoritative when the device reconnects.

## 2. Current behavior and evidence

FIELD-001 preserves server intake/upload intents but does not authorize offline canonical writes.
...
An offline draft is a local proposal to change data. The server remains authoritative when the device reconnects.
...
Chosen approach: An offline draft is a local proposal to change data. The server remains authoritative when the device reconnects. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                         | Baseline state                                            |
| ------------------------------------------------------------ | --------------------------------------------------------- |
| `Frontend/src/lib/offline-drafts.ts`                         | Proposed — create here or consume dependency-created file |
| `Frontend/src/hooks/use-offline-draft.ts`                    | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/properties/DraftConflictReview.tsx` | Proposed — create here or consume dependency-created file |
| `Frontend/src/components/properties/MobileCapture.tsx`       | Proposed — create here or consume dependency-created file |
| `Frontend/e2e/offline-drafts.spec.ts`                        | Proposed — create here or consume dependency-created file |
...
**Depends On:** [DEC-006](../Pending/DEC-006-approve-offline-data-and-synchronization-boundaries.md), [FIELD-001](../Pending/FIELD-001-resume-interrupted-mobile-intake-uploads.md)
...
**Where:** `Frontend/src/lib/offline-drafts.ts`

**Change:** Use IndexedDB store drafts key accountId/entityId with text fields, baseVersion, expiresAt, requestId;7-day TTL, max20 drafts/account and1MiB total. No auth headers or documents.

**Why:** Bounded local data limits device exposure and storage failure.

**Verify immediately:** Quota failure shows unsaved state and does not overwrite previous draft.
...
**Where:** `Frontend/src/hooks/use-offline-draft.ts`

**Change:** On explicit sync reload current entity; compare baseVersion. If changed, show base/local/current per field and require choices; submit server expectedVersion.

**Why:** Last-writer-wins would silently destroy colleague edits.

**Verify immediately:** 409 preserves local draft and restarts comparison.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Frontend/src/components/properties/DraftConflictReview.tsx`

**Change:** After confirmed server save delete local draft; logout purges account partition; offline screens label locally saved versus server saved.

**Why:** User needs to distinguish device persistence from team-visible data.

**Verify immediately:** No background upload or automatic merge.
...
### Test 1 — Concurrent edit

**Purpose:** Prove the boundary named “Concurrent edit” for save offline text drafts with explicit conflict review.
...
**Setup:** Offline rent text changed; server updated meantime.

**Action:** Sync.

**Expected result:** Conflict review; no silent overwrite.
...
**Why this test matters:** An offline draft is a local proposal to change data. The server remains authoritative when the device reconnects. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Compare server version before mutation.

### Test 2 — Storage full

**Purpose:** Prove the boundary named “Storage full” for save offline text drafts with explicit conflict review.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** IndexedDB rejects put.

**Action:** Edit draft.

**Expected result:** Unsaved warning and previous draft retained.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An offline draft is a local proposal to change data. The server remains authoritative when the device reconnects. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not claim success before storage transaction.

### Test 3 — Logout

**Purpose:** Prove the boundary named “Logout” for save offline text drafts with explicit conflict review.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Two drafts in active account.

**Action:** Logout.

**Expected result:** Partition removed; no sync later.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** An offline draft is a local proposal to change data. The server remains authoritative when the device reconnects. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Await purge and cancel pending sync.
...
npm --prefix Frontend test -- e2e/offline-drafts.spec.ts --project=chromium
```
...
- [ ] Quota failure shows unsaved state and does not overwrite previous draft.
- [ ] 409 preserves local draft and restarts comparison.
- [ ] No background upload or automatic merge.
