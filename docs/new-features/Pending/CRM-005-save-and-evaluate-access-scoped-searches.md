# CRM-005 — Save and evaluate access scoped searches
...
Save and evaluate access scoped searches. A saved search stores filter intent. It must recompute today’s permissions each time it runs.

## 2. Current behavior and evidence

Search filters exist but do not define stored search ownership, disable behavior or alert deduplication.
...
- [Backend/src/modules/search/search.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/search/search.controller.ts) — exists at the baseline commit.
- [Backend/src/modules/search/search.module.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/search/search.module.ts) — exists at the baseline commit.
...
A saved search stores filter intent. It must recompute today’s permissions each time it runs.
...
Chosen approach: A saved search stores filter intent. It must recompute today’s permissions each time it runs. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915001800_saved_searches/migration.sql` | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/search/saved-search.service.ts`                    | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/search/search.controller.ts`                       | Existing — inspect before editing                         |
| `Backend/src/modules/search/search.module.ts`                           | Existing — inspect before editing                         |
| `Backend/src/modules/search/__tests__/saved-search.spec.ts`             | Proposed — create here or consume dependency-created file |
...
**Depends On:** [SEARCH-002](../Pending/SEARCH-002-merge-rent-and-area-bounds-without-dropping-filters.md), [TEN-005](../Pending/TEN-005-enforce-tenant-access-throughout-units.md), [OPS-004](../Pending/OPS-004-persist-notification-outcomes-and-replay-controls.md), [INV-001](../Pending/INV-001-store-per-field-inventory-verification-evidence.md)
...
**Change:** Add SavedSearch(id, organizationId, ownerId, name, filterVersion default1, filtersJson, enabled, version, lastRunAt) and SearchAlert(searchId, unitId, availabilityVersion) unique tuple. POST /search/saved and PATCH /search/saved/:id use strict existing filter DTO and expectedVersion.

**Why:** Saved JSON cannot become arbitrary Prisma conditions.

**Verify immediately:** Unknown filter fields and invalid ranges rejected.
...
**Where:** `Backend/src/modules/search/saved-search.service.ts`, `Backend/src/modules/search/search.controller.ts`

**Change:** Hourly worker reloads active user/context and uses SEARCH fixed predicates in batches200; max10000 candidates with visible truncated state. Notify only newly available verified unit versions.

**Why:** An access change must take effect before delivery.

**Verify immediately:** No hidden unit enters persisted recipient payload.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/search/saved-search.service.ts`

**Change:** Disabling/deleting search increments version; dispatcher rechecks enabled/version before sending. Delivery idempotency key searchId/unitId/availabilityVersion.

**Why:** A queue must not ignore unsubscribe.

**Verify immediately:** Repeated hourly runs create one alert per availability version.
...
### Test 1 — Scope revoked

**Purpose:** Prove the boundary named “Scope revoked” for save and evaluate access scoped searches.
...
**Setup:** User loses locality after search save.

**Action:** Run hourly evaluation.

**Expected result:** No alerts or hidden IDs.
...
**Why this test matters:** A saved search stores filter intent. It must recompute today’s permissions each time it runs. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Rebuild context at execution.

### Test 2 — Disable race

**Purpose:** Prove the boundary named “Disable race” for save and evaluate access scoped searches.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Alert queued then search disabled.

**Action:** Dispatch.

**Expected result:** Suppressed; no send.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A saved search stores filter intent. It must recompute today’s permissions each time it runs. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Reload search version.

### Test 3 — Same availability

**Purpose:** Prove the boundary named “Same availability” for save and evaluate access scoped searches.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Three runs same unit version.

**Action:** Evaluate all.

**Expected result:** One alert row/delivery.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A saved search stores filter intent. It must recompute today’s permissions each time it runs. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Enforce DB uniqueness.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/search/__tests__/saved-search.spec.ts
...
- [ ] Unknown filter fields and invalid ranges rejected.
- [ ] No hidden unit enters persisted recipient payload.
- [ ] Repeated hourly runs create one alert per availability version.
