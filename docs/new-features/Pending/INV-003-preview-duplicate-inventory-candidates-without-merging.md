# INV-003 — Preview duplicate inventory candidates without merging
...
Preview duplicate inventory candidates without merging. A similar address is evidence for review, not proof that two properties are identical.

## 2. Current behavior and evidence

Buildings contain free-text and canonical location fields; imports/intake can create similar records without an audited duplicate review.
...
A similar address is evidence for review, not proof that two properties are identical.
...
Chosen approach: A similar address is evidence for review, not proof that two properties are identical. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                               | Baseline state                                            |
| ------------------------------------------------------------------ | --------------------------------------------------------- |
| `Backend/src/modules/buildings/duplicate-review.service.ts`        | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/buildings/__tests__/duplicate-review.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [TEN-004](../Pending/TEN-004-enforce-tenant-access-throughout-buildings.md), [DATA-009](../Pending/DATA-009-normalize-free-text-locations-with-reviewed-mappings.md)

**Blocks:** [AI-010](../Pending/AI-010-extract-brochure-fields-with-page-level-evidence.md), [INV-004](../Pending/INV-004-merge-reviewed-building-duplicates-transactionally.md)
...
**Where:** `Backend/src/modules/buildings/duplicate-review.service.ts`

**Change:** Implement GET /buildings/:id/duplicate-candidates returning at most20 same-tenant permitted buildings with matching canonical locality and normalized exact name/address tokens. Score is rule evidence, not identity probability.

**Why:** Avoid disclosing hidden properties through duplicate hints.

**Verify immediately:** Cross-tenant and inaccessible IDs never appear in candidates or counts.
...
**Change:** Return differences and link counts for floors, units, media, proposals, shortlists, leases if enabled; never load secret contact fields.

**Why:** The reviewer needs to see what a merge would affect.

**Verify immediately:** Read-only preview makes zero domain mutations.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/buildings/duplicate-review.service.ts`, `Backend/src/modules/buildings/buildings.controller.ts`

**Change:** Keep conflicting canonical IDs and ambiguous hierarchy in blockedReasons; link to manual correction rather than auto-selecting a survivor.

**Why:** Address normalization can collapse distinct towers or units.

**Verify immediately:** Same building name in another locality is excluded.
...
**Purpose:** Prove the boundary named “Two towers” for preview duplicate inventory candidates without merging.
...
**Setup:** Same name, different locality IDs.

**Action:** Get candidates.

**Expected result:** No proposed match across canonical locality.
...
**Why this test matters:** A similar address is evidence for review, not proof that two properties are identical. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check normalization scope.

### Test 2 — Hidden duplicate

**Purpose:** Prove the boundary named “Hidden duplicate” for preview duplicate inventory candidates without merging.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** OrgB copy of OrgA building.

**Action:** Query as OrgA.

**Expected result:** No OrgB ID, count or field.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A similar address is evidence for review, not proof that two properties are identical. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Scope before scoring.

### Test 3 — Preview purity

**Purpose:** Prove the boundary named “Preview purity” for preview duplicate inventory candidates without merging.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Candidate has100 links.

**Action:** Call twice.

**Expected result:** All records and versions unchanged.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** A similar address is evidence for review, not proof that two properties are identical. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Remove writes from inspection path.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/buildings/__tests__/duplicate-review.spec.ts
```
...
- [ ] Cross-tenant and inaccessible IDs never appear in candidates or counts.
- [ ] Read-only preview makes zero domain mutations.
- [ ] Same building name in another locality is excluded.
