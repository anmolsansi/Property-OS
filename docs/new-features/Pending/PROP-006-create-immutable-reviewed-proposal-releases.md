# PROP-006 — Create immutable reviewed proposal releases
...
Create immutable reviewed proposal releases. Release is an explicit review action. The snapshot is the artifact source; later property edits must not change prior releases.

## 2. Current behavior and evidence

Current export fetches live items; PROP-005 supplies immutable release storage.
...
Release is an explicit review action. The snapshot is the artifact source; later property edits must not change prior releases.
...
Chosen approach: Release is an explicit review action. The snapshot is the artifact source; later property edits must not change prior releases. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                                       | Baseline state                                            |
| -------------------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/proposals/proposal-release.service.ts`                | Proposed — create here or consume dependency-created file |
...
| `Backend/src/modules/proposals/dto/proposal-release.schema.ts`             | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/proposals/__tests__/proposal-release.service.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [PROP-005](../Pending/PROP-005-persist-immutable-releases-artifact-jobs-and-share-grants.md), [PROP-003](../Pending/PROP-003-authorize-every-proposal-artifact-export.md)

**Blocks:** [PROP-007](../Pending/PROP-007-render-pdf-and-xlsx-from-immutable-release-data.md), [PROP-010](../Pending/PROP-010-issue-revoke-and-read-scoped-client-share-links.md), [PROP-012](../Pending/PROP-012-version-brand-templates-without-changing-old-releases.md), [AI-009](../Pending/AI-009-draft-proposal-rationale-from-approved-public-facts.md), [METRIC-001](../Pending/METRIC-001-measure-the-intake-to-visit-funnel-without-private-payloads.md)
...
**Where:** `Backend/src/modules/proposals/proposal-release.service.ts`

**Change:** Implement POST release after current proposal/item/field access checks and expectedUpdatedAt comparison.

**Why:** An earlier preview is not authority to disclose changed data.

**Verify immediately:** Foreign, stale and empty proposals fail before snapshot.
...
**Change:** Inside transaction allocate version with proposal row lock, whitelist public values/media and persist hash/request identity.

**Why:** Concurrent release cannot duplicate versions or mix snapshots.

**Verify immediately:** No internal data in snapshot, even for ADMIN client-facing release.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/proposals/proposal-release.service.ts`, `Backend/src/modules/proposals/proposals.controller.ts`

**Change:** Expose GET /proposal-releases/:id ->200 immutable release payload for authorized members; repeat key returns same release.

**Why:** Preview and later artifact use one canonical dataset.

**Verify immediately:** Editing master rent afterward leaves old release unchanged.
...
### Test 1 — Master changes after release

**Purpose:** Prove the boundary named “Master changes after release” for create immutable reviewed proposal releases.
...
**Setup:** Release rent1000 then master rent1200.

**Action:** GET old release.

**Expected result:** Still1000 and same hash.
...
**Why this test matters:** Release is an explicit review action. The snapshot is the artifact source; later property edits must not change prior releases. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check live joins at read/render time.

### Test 2 — Private field exclusion

**Purpose:** Prove the boundary named “Private field exclusion” for create immutable reviewed proposal releases.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** ADMIN asks internalNotes for client release.

**Action:** Create release.

**Expected result:** 403/400; no snapshot includes it.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Release is an explicit review action. The snapshot is the artifact source; later property edits must not change prior releases. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Client-facing policy must be stricter than internal export role.

### Test 3 — Concurrent releases

**Purpose:** Prove the boundary named “Concurrent releases” for create immutable reviewed proposal releases.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Two same-idempotency requests.

**Action:** Run concurrently.

**Expected result:** One release/version; stable response.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Release is an explicit review action. The snapshot is the artifact source; later property edits must not change prior releases. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check lock and unique key.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/proposals/__tests__/proposal-release.service.spec.ts
```
...
- [ ] Foreign, stale and empty proposals fail before snapshot.
- [ ] No internal data in snapshot, even for ADMIN client-facing release.
- [ ] Editing master rent afterward leaves old release unchanged.
