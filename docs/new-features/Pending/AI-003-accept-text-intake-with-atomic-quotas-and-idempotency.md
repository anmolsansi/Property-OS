# AI-003 — Accept text intake with atomic quotas and idempotency
...
Accept text intake with atomic quotas and idempotency. Accepting a job means durable intent, not a completed requirement. One click creates one source/job/event despite HTTP retries.

## 2. Current behavior and evidence

Clients/requirements can be created manually; there is no current AI intake endpoint.
...
Accepting a job means durable intent, not a completed requirement. One click creates one source/job/event despite HTTP retries.
...
Chosen approach: Accepting a job means durable intent, not a completed requirement. One click creates one source/job/event despite HTTP retries. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/src/modules/ai/dto/intake.schema.ts`             | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/ai.module.ts`                     | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/__tests__/intake.service.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-001](../Pending/AI-001-create-private-source-job-and-review-persistence.md), [OPS-003](../Pending/OPS-003-dispatch-outbox-events-with-retry-safe-claims.md), [TEN-003](../Pending/TEN-003-enforce-tenant-access-throughout-clients.md), [REQ-002](../Pending/REQ-002-expose-atomic-brief-read-and-replacement-endpoints.md)

**Blocks:** [AI-004](../Pending/AI-004-run-recoverable-ai-extraction-jobs.md), [SAAS-003](../Pending/SAAS-003-wire-plan-limits-into-jobs-uploads-and-admin-screens.md)
...
**Change:** Implement POST/GET/cancel AIJOB-v1 routes, strict text length, client ownership and same-tenant source hash.

**Why:** A client ID is a required authority boundary.

**Verify immediately:** Foreign client404 before provider or storage work.
...
**Change:** Lock an organization quota row or organization row in transaction, check active/day counts and approved cost reservations, then commit SourceRecord/job/outbox together.

**Why:** Concurrent requests must not bypass limits.

**Verify immediately:** At most configured active jobs are accepted under parallel requests.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/ai/dto/intake.schema.ts`

**Change:** Return same result for identical replay and409 for changed payload key; cancellation preserves cost and source history, marks running result for discard.

**Why:** Cancel cannot promise a refund or undo an applied record.

**Verify immediately:** No requirement exists before apply.
...
### Test 1 — Retry same click

**Purpose:** Prove the boundary named “Retry same click” for accept text intake with atomic quotas and idempotency.
...
**Setup:** Two same-key same-input POSTs.

**Action:** Send simultaneously.

**Expected result:** One job/source/event; both reference same ID.
...
**Why this test matters:** Accepting a job means durable intent, not a completed requirement. One click creates one source/job/event despite HTTP retries. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check uniqueness plus payload hash.

### Test 2 — Quota race

**Purpose:** Prove the boundary named “Quota race” for accept text intake with atomic quotas and idempotency.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Workspace has9 active jobs, cap10; two POSTs.

**Action:** Run concurrently.

**Expected result:** Only one new job accepted; other429; no orphan source.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Accepting a job means durable intent, not a completed requirement. One click creates one source/job/event despite HTTP retries. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check quota lock inside transaction.

### Test 3 — Cancellation after apply

**Purpose:** Prove the boundary named “Cancellation after apply” for accept text intake with atomic quotas and idempotency.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Applied job and linked requirement.

**Action:** POST cancel.

**Expected result:** 409; requirement and usage unchanged.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Accepting a job means durable intent, not a completed requirement. One click creates one source/job/event despite HTTP retries. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Do not implement cancellation as deletion.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/intake.service.spec.ts
```
...
- [ ] Foreign client404 before provider or storage work.
- [ ] At most configured active jobs are accepted under parallel requests.
- [ ] No requirement exists before apply.
