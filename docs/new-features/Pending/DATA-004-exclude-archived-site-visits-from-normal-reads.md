# DATA-004 — Exclude archived site-visits from normal reads

**Status:** Pending  
**Readiness:** Blocked by dependencies; revalidate after they land  
**Type:** Implementation  
**Priority:** P1  
**Area:** Archive lifecycle  
**Assessment:** D10  
...
Exclude archived site-visits from normal reads. Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence.

## 2. Current behavior and evidence

site-visits has softDelete/restore, but ordinary reads do not consistently predicate deletedAt:null.
...
- [Backend/src/modules/site-visits/**tests**/site-visits.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/site-visits/__tests__/site-visits.service.spec.ts) — exists at the baseline commit.
...
Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence.
...
Chosen approach: Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/src/modules/site-visits/__tests__/site-visits.service.spec.ts` | Existing — inspect before editing |
...
**Depends On:** [TEN-008](../Pending/TEN-008-enforce-tenant-access-throughout-site-visits.md)
...
**Change:** Add deletedAt:null to ordinary lists/counts/details and reject normal updates of archived records. Preserve historical references via explicit projection.

**Why:** An archived record should not reappear after refresh.

**Verify immediately:** List totals and detail behavior agree.
...
**Change:** Separate archive-aware lookup for authorized restore; make archive and restore repeated calls idempotent.

**Why:** Using normal findOne for restore makes recovery impossible.

**Verify immediately:** Restore works for permitted archived records and never across tenant.

**Checkpoint:** capture the focused assertion/output proving this result before continuing. If it fails, inspect this boundary and its transaction/context inputs; do not bypass validation to reach the next step.

### Step 3 — CHANGE, TEST and CHECKPOINT

**Where:** `Backend/src/modules/site-visits/site-visits.controller.ts`

**Change:** Keep delete/restore role restrictions; do not add includeDeleted to public query parameters.

**Why:** Trash access is a distinct authority.

**Verify immediately:** Worker cannot use an invented query flag to recover hidden records.
...
### Test 1 — Archive survives reload

**Purpose:** Prove the boundary named “Archive survives reload” for exclude archived site-visits from normal reads.
...
**Setup:** One active permitted record.

**Action:** Archive, list, detail, restart API then list.

**Expected result:** Record absent/404; deletedAt remains stored.
...
**Why this test matters:** Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check missing predicates and in-memory filtering.

### Test 2 — Restore remains scoped

**Purpose:** Prove the boundary named “Restore remains scoped” for exclude archived site-visits from normal reads.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Archived record in orgB.

**Action:** OrgA requests restore.

**Expected result:** 404 and no update.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Inspect special archive-aware lookup.

### Test 3 — Repeat lifecycle is stable

**Purpose:** Prove the boundary named “Repeat lifecycle is stable” for exclude archived site-visits from normal reads.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Permitted record.

**Action:** Archive twice; restore twice.

**Expected result:** No duplicate side effects; original relationships preserved.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Archive changes normal visibility; it does not grant a user a way around access control or erase historical evidence. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check state transition handling.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/site-visits/__tests__/site-visits.service.spec.ts
```
...
- [ ] List totals and detail behavior agree.
- [ ] Restore works for permitted archived records and never across tenant.
- [ ] Worker cannot use an invented query flag to recover hidden records.
