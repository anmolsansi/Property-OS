# AI-020 — Index authorized source chunks for cited retrieval
...
Index authorized source chunks for cited retrieval. Permission filtering happens before retrieval and again before answering. A vector similarity score would not authorize a document.

## 2. Current behavior and evidence

SourceRecord provides immutable evidence but there is no authorized chunk index. Start with PostgreSQL lexical retrieval; embeddings are not required for a safe first slice.
...
Permission filtering happens before retrieval and again before answering. A vector similarity score would not authorize a document.
...
Chosen approach: Permission filtering happens before retrieval and again before answering. A vector similarity score would not authorize a document. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| `Backend/prisma/migrations/20260915002700_source_chunks/migration.sql`  | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/source-retrieval.service.ts`                    | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/ai.module.ts`                                   | Proposed — create here or consume dependency-created file |
| `Backend/src/modules/ai/__tests__/source-retrieval.integration.spec.ts` | Proposed — create here or consume dependency-created file |
...
**Depends On:** [AI-001](../Pending/AI-001-create-private-source-job-and-review-persistence.md), [OWNER-001](../Pending/OWNER-001-enforce-explicit-same-tenant-asset-capability-grants.md), [OPS-003](../Pending/OPS-003-dispatch-outbox-events-with-retry-safe-claims.md)

**Blocks:** [AI-021](../Pending/AI-021-answer-document-questions-with-verifiable-citations.md)
...
**Change:** Add SourceChunk(id, sourceId, sourceVersion, organizationId, ordinal, page, startOffset, endOffset, text, searchVector) unique source/version/ordinal, GIN lexical index. Chunk at1000 chars with100overlap preserving page boundaries.

**Why:** Stable spans make answers inspectable.

**Verify immediately:** Deleted/expired/unscanned sources excluded from indexing.
...
**Where:** `Backend/src/modules/ai/source-retrieval.service.ts`

**Change:** retrieve(ctx, query, allowedParentContext) uses tenant and live parent/asset grants before lexical rank; top8 chunks, max8000 chars. Require explicit client/lease/building context, not whole-company search.

**Why:** Restricted source text must never reach a model prompt.

**Verify immediately:** Unauthorized candidate count/text not exposed.
...
**Change:** Source revoke/delete/expiry makes retrieval unavailable immediately through DB predicate; async purge removes chunk rows with idempotent outbox consumer.

**Why:** Delayed physical purge must not imply delayed permission revocation.

**Verify immediately:** No stale chunk served after primary source expires.
...
**Purpose:** Prove the boundary named “Cross tenant” for index authorized source chunks for cited retrieval.
...
**Setup:** Same text inA andB.

**Action:** RetrieveA.

**Expected result:** OnlyA chunks.
...
**Why this test matters:** Permission filtering happens before retrieval and again before answering. A vector similarity score would not authorize a document. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Apply where before rank.

### Test 2 — Revocation

**Purpose:** Prove the boundary named “Revocation” for index authorized source chunks for cited retrieval.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Chunk indexed before grant revoke.

**Action:** Retrieve.

**Expected result:** No text; no provider call.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Permission filtering happens before retrieval and again before answering. A vector similarity score would not authorize a document. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Live parent policy.

### Test 3 — Expired source

**Purpose:** Prove the boundary named “Expired source” for index authorized source chunks for cited retrieval.

**Level:** Service / integration. Use a real isolated database whenever the expected result depends on locks, constraints, relations or rollback; mocks alone are insufficient.

**Setup:** Source expires between query and answer.

**Action:** Reauthorize chunks.

**Expected result:** Reject answer/abstain.

**Required assertions:** Assert the exact response/result and persisted state described above. Inspect relevant rows/events/jobs before and after; where denied or rolled back, assert no forbidden mutation, artifact or provider/delivery call. Do not assert only that a method was called.

**Why this test matters:** Permission filtering happens before retrieval and again before answering. A vector similarity score would not authorize a document. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Check again before response.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/ai/__tests__/source-retrieval.integration.spec.ts
...
- [ ] Deleted/expired/unscanned sources excluded from indexing.
- [ ] Unauthorized candidate count/text not exposed.
- [ ] No stale chunk served after primary source expires.
