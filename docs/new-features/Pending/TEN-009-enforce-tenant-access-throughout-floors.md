# TEN-009 — Enforce tenant access throughout floors
...
Enforce tenant access throughout floors. A Floor and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. floor detail and writes inherit parent access.

## 2. Current behavior and evidence

floors routes/services currently accept IDs or geography independently; the existing organization field is not a universal query boundary.

The baseline below is source inspection, not production verification. Read these exact files before editing:

- [Backend/src/modules/floors/floors.controller.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/floors/floors.controller.ts) — exists at the baseline commit.
- [Backend/src/modules/floors/floors.service.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/floors/floors.service.ts) — exists at the baseline commit.
- [Backend/src/modules/floors/**tests**/floors.service.spec.ts](https://github.com/anmolsansi/Property-OS/blob/06ce222654fbaed7bfda33802a89c305574f0e95/Backend/src/modules/floors/__tests__/floors.service.spec.ts) — exists at the baseline commit.
...
A Floor and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. floor detail and writes inherit parent access.
...
Chosen approach: A Floor and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. floor detail and writes inherit parent access. The numbered implementation steps below identify the owning boundary and the reason for each choice.
...
| File                                                          | Baseline state                                            |
| ------------------------------------------------------------- | --------------------------------------------------------- |
| `Backend/src/modules/floors/floors.controller.ts`             | Existing — inspect before editing                         |
| `Backend/src/modules/floors/floors.service.ts`                | Existing — inspect before editing                         |
| `Backend/src/modules/floors/__tests__/floors.service.spec.ts` | Existing — inspect before editing                         |
| `Backend/test/floors-tenant.e2e-spec.ts`                      | Proposed — create here or consume dependency-created file |
...
**Blocks:** [MEDIA-001](../Pending/MEDIA-001-authorize-upload-completion-and-downloads-by-parent.md)
...
**Where:** `Backend/src/modules/floors/floors.controller.ts`
...
**Where:** `Backend/src/modules/floors/floors.service.ts`
...
**Change:** Authorize floor detail and writes via parent, set new root organization from context, and perform conditional writes including owner identity. Keep existing state-transition rules.
...
**Where:** `Backend/test/floors-tenant.e2e-spec.ts`
...
**Purpose:** Prove the boundary named “Tenant ID substitution” for enforce tenant access throughout floors.
...
**Why this test matters:** A Floor and its linked records belong to one organization. Knowing an ID is not permission to read or modify it. floor detail and writes inherit parent access. This scenario verifies that rule under a specific failure or lifecycle condition.

**Likely failure diagnosis:** Find unconverted service method or unscoped count.

### Test 2 — Foreign parent on create

**Purpose:** Prove the boundary named “Foreign parent on create” for enforce tenant access throughout floors.
...
**Purpose:** Prove the boundary named “Same tenant role restrictions preserved” for enforce tenant access throughout floors.
...
npm --prefix Backend test -- --runInBand --runTestsByPath src/modules/floors/__tests__/floors.service.spec.ts
npm --prefix Backend run test:e2e -- --runInBand --runTestsByPath test/floors-tenant.e2e-spec.ts
```
